...
public sealed partial class HtmlRenderAdapter
{
    ...
    /// <summary>The dashboard's two-row header, replacing the old single centered project-title band. Row one is
    /// the identity row — the project name on the left and a "Generated by SpecScribe" brand mark pinned to the
    /// upper-right (the same nib geometry as the nav brand, linking to the About / generation-details page; the
    /// generation timestamp itself lives only in the per-page footer). Row two is the Explore Key Views pill menu
    /// promoted from its old bottom-of-page panel into a task-oriented sub-header.</summary>
    private void AppendDashboardHeader(StringBuilder sb, DashboardView view)
    {
        sb.Append("<header class=\"dashboard-header\">\n");
        sb.Append("  <div class=\"dashboard-identity\">\n");
        sb.Append($"    <h1 class=\"project-name\">{PathUtil.Html(view.SiteTitle)}</h1>\n");
        sb.Append($"    <a class=\"specscribe-badge\" href=\"{SiteNav.AboutOutputPath}\" data-tooltip=\"Generated by SpecScribe — view generation details\">"
            ...
            + "<span class=\"specscribe-badge-text\">SpecScribe</span></a>\n");
        sb.Append("  </div>\n");
        AppendKeyViewsSubheader(sb, view.QuickLinks);
        sb.Append("</header>\n\n");
    }
    ...
    private void AppendSummaryBand(StringBuilder sb, DashboardView view, ProgressModel p)
    {
        ...
        AppendOverallProgress(sb, view.ProgressBars);
    ...
    private void AppendOverallProgress(StringBuilder sb, IReadOnlyList<ProgressBarView> bars)
    {
    ...
    /// <summary>The "Explore Key Views" pills, rendered as the dashboard header's second row — a task-oriented
    /// sub-header menu rather than a bordered bottom-of-page panel. Keeps the <c>dashboard-quick-links</c> +
    /// <c>quick-link-pill</c> hooks (so the quick-link parity facts and menu styling are unchanged); the visible
    /// "Explore Key Views" heading is dropped in favor of the accessible nav label, since the icon+label pills are
    /// self-describing. The family/title helpers are pure label mappings.</summary>
    private void AppendKeyViewsSubheader(StringBuilder sb, IReadOnlyList<NavQuickLink> quickLinks)
    {
        if (quickLinks.Count == 0) return;

        sb.Append("  <nav class=\"key-views dashboard-quick-links\" aria-label=\"Explore key views\">\n");
