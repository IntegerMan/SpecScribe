        ...
        SiteNav nav)
    {
        ...
        try
        {
            ...
            foreach (var epic in model.Epics)
            {
                ...
                File.WriteAllText(Path.Combine(epicsDir, $"epic-{epic.Number}.html"), ApplyReferenceLinks(EpicsTemplater.RenderEpic(epic, progressByEpic[epic.Number], nav, _module.Commands, epicRetroPath, EpicPager(model, epic), followUps, unplanned, EpicSubgraph(epic.Number)), $"epics/epic-{epic.Number}.html", skipEpicNumber: epic.Number));
                ...
                foreach (var story in epic.Stories)
                {
                    ...
                    var storyHtml = EpicsTemplater.RenderStory(epic, story, f.ArtifactRelative, f.BlurbHtml, f.RemainderHtml, f.AcceptanceCriteria, f.DevAgentRecord, f.Tasks, f.ReviewFindingsHtml, f.ChangeLogHtml, f.Evidence, f.ChangeSurface, nav, _module.Commands, epicRetroPath, StoryPager(model, story), followUps, StorySubgraph(epic, story, followUps));
    ...
    public WebviewBundle RenderWebviewSurfaces()
    {
        lock (_gate)
        {
            ...
            if (_epicsModel is { } model && _progress is { } progress)
            {
                ...
                {
                    var epicRetroPath = EpicRetroMap.TryGetValue(epic.Number, out var erp) ? erp : null;
                    var epicPage = EpicsTemplater.BuildEpicPage(epic, progressByEpic[epic.Number], nav, _module.Commands, epicRetroPath, EpicPager(model, epic), followUps, unplanned, EpicSubgraph(epic.Number));
                        ...
                        else
                        {
                            try
                            {
                                ...
                                storyPage = EpicsTemplater.BuildStoryPage(
                                    ...
                                    _module.Commands, epicRetroPath, StoryPager(model, story), followUps, StorySubgraph(epic, story, followUps));
                            }
                            catch (Exception ex) when (ex is IOException or UnauthorizedAccessException)
                            {
        ...
        if (_epicsModel is { } model && _progress is { } progress)
        {
            ...
            foreach (var epic in model.Epics)
            {
                ...
                AddSpaSurface(pages, familyPaths,
                    EpicsTemplater.BuildEpicPage(epic, progressByEpic[epic.Number], nav, _module.Commands, epicRetroPath, EpicPager(model, epic), followUps, unplanned, EpicSubgraph(epic.Number)),
