...
public sealed partial class HtmlRenderAdapter : IRenderAdapter
{
    ...
    public string RenderNavMarkup(NavigationView nav)
    {
        ...
        // Delivery — each with an icon), and the "Generated by SpecScribe" badge; beneath it a white KEY-VIEWS
        // band with the Discoverable-document chips (the former dashboard "Explore Key Views" pills).
    ...
    /// <summary>Renders the dark-bar journey menu: Home plus Docs / Architecture / Delivery dropdowns (each with
    /// an icon). Merges top-nav <see cref="NavItem"/>s with the quick-link superset so dropdowns stay complete,
    /// then groups them (docs/onboarding, architecture+code map, delivery) per docs/UserJourneys.md. Code Map
    ...
    /// <summary>The white sub-header band: key-view chips from <see cref="NavigationView.QuickLinks"/> (icon +
    /// label + tooltip description), reachable from every page. Omits the band entirely when there are none.</summary>
    private void AppendKeyViewsBand(StringBuilder sb, NavigationView nav, string prefix)
    {
        ...
        {
            sb.Append($"      <a class=\"quick-link-pill {QuickLinkFamily(label)}\" href=\"{PathUtil.Html(prefix + outputPath)}\" data-tooltip=\"{PathUtil.Html(description)}\">{Icons.ForConcept(label)}{PathUtil.Html(QuickLinkTitle(label))}</a>\n");
        }
        sb.Append("    </div>\n  </div>\n");
    }
    ...
    /// <summary>Ordered journey-themed menu groups for the dark bar. See <see cref="NavGroup"/>.</summary>
    private static readonly string[] NavGroupOrder = { "Docs", "Architecture", "Delivery" };

    /// <summary>Maps a nav/key-view label to its journey-themed dark-bar group, or null for a top-level flat link
    /// (Home). Architecture includes ADRs and Code Map; Delivery is the daily pulse (Epics / Sprint /
