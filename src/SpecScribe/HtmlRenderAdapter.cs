...
public sealed partial class HtmlRenderAdapter : IRenderAdapter
{
    ...
    public string RenderNavMarkup(NavigationView nav)
    {
        ...
        var sb = new StringBuilder();
        // Two-tier chrome (a single sticky <nav class="site-nav"> so the toggle script + webview bridge + mobile
        // rules are unchanged): a dark IDENTITY bar (project name + a "Generated by SpecScribe" attribution badge
        // linking to the About page), and beneath it a white MENU band carrying the journey-grouped hierarchical
        // navigation. Both tiers use the same centered content column as the page body.
        ...
        sb.Append("    <div class=\"site-menu-inner\">\n");
        AppendNavMenu(sb, nav, prefix, current);
        sb.Append("    </div>\n  </div>\n</nav>\n");
        return sb.ToString();
    }

    /// <summary>Renders the white menu band's hierarchical navigation. Merges the top-nav <see cref="NavItem"/>s
    /// with the dashboard <see cref="NavQuickLink"/> superset (deduped by target) so every discoverable key view is
    /// reachable from every page, then arranges them into journey-themed groups (docs/onboarding, architecture,
    /// delivery, codebase — mirroring docs/UserJourneys.md). "Home" and any single-item group render as a flat
    /// link; multi-item groups render as a hover/focus dropdown so the whole menu fits one row. [journey nav]</summary>
    private void AppendNavMenu(StringBuilder sb, NavigationView nav, string prefix, string current)
    {
        ...
        // Home (and anything that maps to no group) leads as a flat link.
        ...
        foreach (var group in NavGroupOrder)
        {
            ...
            // A lone member reads better as a flat link than a one-item dropdown.
            if (members.Count == 1)
            {
            ...
            sb.Append($"        <button class=\"site-menu-trigger\" type=\"button\" aria-haspopup=\"true\">{PathUtil.Html(group)}<span class=\"site-menu-caret\" aria-hidden=\"true\">&#9662;</span></button>\n");
    ...
    /// the entries the menu renders — the shared basis for both <see cref="AppendNavMenu"/> and the render-parity
    /// <c>Nav</c> fact (via <see cref="NavMenuOrder"/>), so the two can never drift.</summary>
    ...
    /// <summary>The (display label, output-relative target) pairs in the exact order the menu emits its anchors —
    /// flat/home links first, then each journey group's members. The render-parity harness reuses this so its
    /// declared nav graph matches the markup the adapter produces. [journey nav]</summary>
    ...
    /// <summary>Ordered journey-themed menu groups. See <see cref="NavGroup"/> for the label→group mapping.</summary>
    private static readonly string[] NavGroupOrder = { "Docs", "Architecture", "Delivery", "Codebase" };

    /// <summary>Maps a nav/key-view label to its journey-themed group, or null for a top-level flat link (Home).
    /// Grouping mirrors docs/UserJourneys.md: Docs = onboarding/vision + traceability sources; Architecture =
    /// how-it's-built; Delivery = daily pulse / work selection / review / requirement traceability; Codebase =
    /// health & hotspots. Unknown labels (e.g. game-mode module docs) fall into Docs so nothing is orphaned.</summary>
    private static string? NavGroup(string label) => label switch
    {
        "Home" => null,
        "Architecture" or "ADRs" => "Architecture",
