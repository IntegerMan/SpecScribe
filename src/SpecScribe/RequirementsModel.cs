...
/// covering epic is fully done); Done (every covering epic is complete); Retired (every covering epic is itself
/// retired — <see cref="StatusStyles.ForEpic"/>'s all-retired terminal tier, Story 8.9/ADR 0025 — so the plan
/// that would have delivered this requirement was abandoned, not merely not-yet-started).
...
/// <para><see cref="Retired"/> is distinct from <see cref="Deferred"/> for the same reason: Deferred is a
/// requirement-level decision to shelve (<see cref="RequirementInfo.Deferred"/>), independent of what any
/// covering epic does; Retired is an epic-level outcome — the covering epic(s) were abandoned — surfaced only
/// once <em>every</em> covering epic resolves to "retired". A requirement covered by one retired epic and one
/// still-progressing epic reads Active/Ready/Done as appropriate, never Retired: this tier fires only when no
/// covering epic offers any other signal. [Story 8.9 review]</para>
...
public enum RequirementStatus { Deferred, Unmapped, Planned, Ready, Active, Done, Retired }
