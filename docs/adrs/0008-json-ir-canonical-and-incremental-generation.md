...
3. **Generation targets an incremental, event-driven model.** The C# core recomputes only the changed scope (operationalizing AD-5, including AD-5's rule that *topology changes may trigger a broader refresh*) and emits **IR deltas** suitable for a future watch / client-server transport (conformant with AD-8).
   ...
   > **The "changed scope" half is operationalized by [ADR 0027](0027-watch-rebuild-scope-is-one-classifier-and-topology-escalates.md)** (Story 22.5): *which* scope a change needs is decided by one named classifier (`SiteGenerator.ClassifyRebuildScope`) consulted before family routing, a content change stays narrow only where it is proven byte-identical to a full rebuild, and a topology change — a file or directory added, renamed or deleted — escalates. That is AD-5's "topology changes may trigger a broader refresh" turned into a testable contract rather than a behaviour implied by omission.
   >
   > **The "IR deltas" half is operationalized by [ADR 0028](0028-delta-transport-is-a-sidecar-and-a-stream-never-a-server.md)** (Accepted 2026-07-29, Story 22.6). The delta transport is AD-8's two named clauses and nothing more — a polled `spa/delta.json` sidecar and delta frames on the NDJSON stdout channel — with **no listener, no port and no long-lived process**. That also answers this ADR's own §Consequences deferral ("a future client/server mode adds a long-lived-process deployment shape (explicitly later; not decided here)") as a **no**: a `specscribe serve` HTTP/SSE server remains out of scope and needs its own ADR.
