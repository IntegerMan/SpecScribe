...
**2. A consumer may FILTER and POST-PROCESS; it may not re-derive.** The webview's entire remaining surface-specific behaviour is, exhaustively: its exclusion set (code pages, commit-day pages, the `commit/` prefix — applied as a skip predicate *inside* `CapturedRegions`, so an excluded page is never sliced), its degrade skip, its JSON-island strip, its `SourcePath` join, its `WrapDocument` entry document, and its **`ProjectOutline`, projected by `BuildOutline` from the same `FamilySurface` sequence** (the sidebar tree is a VS Code affordance with no browser analogue; it is a projection of the seam's output, not a second derivation of it). The SPA consumes the same sequence unfiltered. Anything a consumer needs beyond that is a signal the seam is wrong, not a licence to fork it.

> ⚠️ This list said **five** items until Story 22.4's code review: `BuildOutline` was omitted, while the story's own Completion Notes table listed it as a sixth. An "exhaustively" that is not exhaustive is worse than no list, because §Decision 2's closing sentence makes it normative — on the original wording the shipped seam violated this ADR on its own terms.
