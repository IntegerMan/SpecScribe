...
| --- | --- | --- |
| pages driving extraction | 4 | **the whole site** (see the count note below) |
| rules carried | 880 | **see `web/measurements/ir-content-residue.txt`** |
...
**The residue measurement** (`npm run report:ir-content-residue`, committed at `web/measurements/ir-content-residue.{txt,json}`) buckets every carried rule by what would have to change for it to stop existing.

> ⚠️ **Amended 2026-08-08 by the Story 23.4 code review (findings F-11 and F-13).** Two corrections, both of which move the numbers below and neither of which changes this ADR's conclusion.
>
> 1. **The classifier over-claimed prose.** The `prose` bucket matched the bare substrings `code-` and `pre`, so it captured `.code-line` — the `CodeFileSurface` source-listing gutter, which is neither Markdig prose nor authorable — and anything merely *containing* the letters `pre` (`.preview`, `.prefix`). Since the size of this bucket is the entire argument for D5's first branch being within reach, it is now anchored to class-name boundaries and validated by a self-test the report runs before it prints a verdict. Corrected, `prose` is **43 rules (2.9 %)**, not 93 (6.5 %) — so the first branch is *further* out of reach than the original measurement showed, and this ADR's amendment is more strongly supported, not less.
> 2. **The count was stated four different ways.** Story 23.4 recorded 1,423, this table said 1,416, and the residue report said 1,420 — three figures for one measurement, written in one session, never reconciled; the manifest has since drifted again. **The rule count is deliberately no longer quoted here.** `web/measurements/ir-content-residue.txt` is regenerated by command and is the single answer; nothing gates a number re-typed into prose, which is precisely how four of them came to disagree.

| bucket | share | blocker |
| --- | --- | --- |
| prose | ~3 % | **none — authorable today** |
| chart | ~22 % | Epic 22 — the IR carries no structured chart data |
| card | ~33 % | Epic 22 — the IR carries no per-family view models |
| chrome | ~7 % | **ADR 0024 / 23.4 AC #3, by design — permanent** |
| status | ~6 % | the token bridge — must stay in step with the six `--status-*` tokens |
| other | ~29 % | Epic 22 — uncategorized injected vocabulary |

⚠️ The `other` bucket is roughly a quarter of the residue and its "named blocker" is a **classifier fall-through**, not an inspected cause. AC #4 asks for a named blocker per rule; for this bucket that is satisfied in letter only. Whoever does the Epic 22 view-model work should expect to re-classify it rather than trust the label.
...
**One bucket will never empty, and that changes this ADR's own end state.** The `chrome` rules style the nav, key-views band, breadcrumb/wayfinding, TOC rail and tab strips — markup that owner decision D2 and [ADR 0024](0024-spa-and-webview-are-filtered-projections-of-one-region-seam.md) keep C# composing into the region **permanently**, because the webview and SPA consume that same seam. So §Decision 4's "when it is empty" condition is unreachable as written even after Epic 22 lands: a residue floor exists by design. What those rules need is not deletion but a change of **provenance** — an owned sheet in `web/` rather than a generated extract — which is a smaller, separable piece of work than the other buckets.

**Consequently this ADR stays `Proposed` and its retirement clause is amended:** the layer retires when the Epic 22 view-model work lands for the `chart`/`card`/`other` buckets (roughly **84 %** of the residue between them), the token bridge absorbs `status`, and `chrome` is re-homed to an authored sheet. Until then the layer, its manifest and `npm run check:ir-content` **stay**, and the owner-visible debt figure is whatever `npm run report:ir-content-residue` prints — regenerated by command, never re-typed here.
