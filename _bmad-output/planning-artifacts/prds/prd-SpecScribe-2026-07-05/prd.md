...
## 9. Open Questions
- ~~How should coverage tiers be communicated (fully rendered, summarized, unsupported) so users understand exactly what is and is not interpreted?~~ **ANSWERED 2026-07-27 by Story 18.5.** The three tiers are a real closed type (`CoverageTier` + `CoverageTiers`, one word and one description per value, the single classifier — no surface spells a tier itself), assigned per discovered artifact, and communicated **three ways at once**: a badge on every artifact row that always carries its WORD (never colour alone, UX-DR17); a "How far interpretation goes" legend on the same page that states each tier's promise in a full sentence beside the artifacts it applies to; and per-tier counts on the dashboard's Module Coverage panel. The tiers are defined by INTERPRETATION DEPTH, not by whether a file was found — `rendered` = a full page exists and nothing beyond its prose is interpreted; `summarized` = a structured headline (verdict, coverage figures) is extracted while the file is not fully modelled; `unsupported` = discovered and named, nothing claimed about its contents. The boundary is stated on the surface rather than left implicit in a colour, so an uninterpreted artifact never quietly looks supported.
