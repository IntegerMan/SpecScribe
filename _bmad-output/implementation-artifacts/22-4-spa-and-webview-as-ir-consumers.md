...
Code review 2026-07-28 (`/bmad-code-review 22.4`, three adversarial layers). **Scoped by this story's File List and declared symbols, not by a commit range** (CLAUDE.md § Scoping a code review): sibling Stories 18.2 (ADR 0018), 18.5 (ADR 0020), 25.3 (ADR 0023) and 18.4 are excluded from the same `6017c2c..HEAD` range. Every finding below was re-verified against the working tree before rating; one Blind Hunter finding was **refuted** by a guard it missed and dismissed.

**Owner decisions on the two `decision-needed` findings (2026-07-28):**

- **DR1 — Build the shared instance now, inside this story.** `RenderEpicsPages` must consume the *same* `WorkInventory` / `FollowUpGeometry` instance the seam builds, not an equal-but-separate one. The owner declined both the "amend the task to match what shipped" and the "hand it to 22.5" options. ⚠️ This takes on the nav-gate circularity Story 22.5 documents as its Trap 1.
- **DR2 — The consumer skips degraded pages.** `web/ir/adapter.ts` gets a skip for landmark-less regions instead of a throw, matching the webview's existing `Degraded → continue`. ADR 0024 §Decision 3 stands; one landmark-less page becomes a per-page degrade rather than a whole-site prerender failure.

- [~] [Review][Patch] **[DR1] PARTIALLY APPLIED — 2 of 3 consumers now share one instance; the third is blocked by AC #7.** `BuildSurfacePrelude` is now memoized on `_prelude`, so `BuildSpaBundle` and `RenderWebviewSurfaces` share **one** instance where a `--webview` run previously built two equal ones (the exact "equal-but-separately-constructed" defect Task 3 named, live inside the seam built to end it). Invalidated at the start of every mutating entry point (`GenerateAll`, `GenerateOne`, `RemoveFor`, `RegenerateEpics`, `RegenerateAdrs`; `RegenerateTopology`/`RegenerateFromDataSource` delegate to `GenerateAll`) and never by the pure-read `RenderWebviewSurfaces`, which is what lets it reuse the emit's instance. ⚠️ **`RenderEpicsPages` cannot join without moving the golden fingerprint.** It runs at `GenerateAll:396`, before the pages loop at `:409` fills `_docs`; `ResolveFollowUpWork` therefore returns the source-fallback inventory there and the docs-derived one at the seam. Sharing the early instance would push the lossier inventory into the IR — reversing exactly the convergence 23.3 measured and 22.4 D3 chose — and relocating `RenderEpicsPages` reorders the diagnostics stream, which **AC #7 forbids and Story 22.5 AC #6 forbids again**. That residual third is the nav-gate circularity Story 22.5 documents as its own Trap 1, and `epics.md` now says so [src/SpecScribe/SiteGenerator.cs:2929]
- [x] [Review][Patch] **[DR2]** `splitContentRegion` now returns a `degraded: true` nav-only region instead of throwing, so one landmark-less page degrades itself rather than failing the whole Nuxt prerender; `IrRegion.degraded` added to `types.ts`, consumed by `ir-content-build.mjs`, and `region-split.test.ts`'s refusal case replaced with three assertions on the degraded shape [web/ir/adapter.ts]

- [x] [Review][Patch] AC #5's agreement test now exists — `EverySurface_ReportsTheSameWorkGraphCounts_InTheStaticPageAndTheIr` compares the rendered work-graph summary (node **and** edge counts) between each static page and its IR region. ⚠️ The fixture had to gain a `deferred-work.md` whose item is RESOLVED by a **spec doc rather than a dotted story id** — the exact `ResolvingHref` path the defect nulled — because the test's **vacuity guard** correctly failed first: the old fixture rendered no work graph at all, which is precisely why 22.4 measured AC #5 as zero-delta on the golden fixture [tests/SpecScribe.Tests/SiteGeneratorSpaTests.cs]
- [x] [Review][Patch] `epics.md`'s Story 22.5 gate clause corrected — both false halves called out inline so 22.5 inherits the truth: there is no shared `WorkInventory` across all three builders, and 22.4 **did** touch the pre-nav `_workGraph` build via `ResolveDeferredModel` [_bmad-output/planning-artifacts/epics.md]
- [x] [Review][Patch] A `schemaVersion` **below** `EXPECTED_SCHEMA_VERSION` now **throws**; only an above-expected (additive) bump warns. Closes the warn-then-corrupt path a stale v1 IR opened once the balance repair was deleted [web/ir/adapter.ts]
- [x] [Review][Patch] The story-fragment degrade is no longer silent — it records to `_familyDegradedStories` and drains into **one** `GenerationEvent` after the emit, only when a degrade actually occurred, so the normal event stream (and therefore the fingerprint) is byte-unchanged. The inaccurate "mirrors `RenderEpicsPages`" comment is corrected in place [src/SpecScribe/SiteGenerator.cs]
- [x] [Review][Patch] ADR 0024 §Decision 2 and `docs/adrs/README.md` now list **six** webview behaviours including `BuildOutline`, with a note recording that the original "exhaustively" was not [docs/adrs/0024-spa-and-webview-are-filtered-projections-of-one-region-seam.md]
- [x] [Review][Patch] `CapturedRegions` takes an optional skip predicate applied **before** any slicing; the webview passes its exclusion set, so an excluded page is never sliced. Same set, same output, none of the discarded work [src/SpecScribe/SiteGenerator.cs]
- [x] [Review][Patch] `BuildOutline`'s `stories?.Add(...)` replaced with an explicit invariant throw naming the story and epic, so an out-of-order family sequence fails loudly instead of silently dropping a story from both the outline tree and `BuildOutlineSummary`'s counts [src/SpecScribe/SiteGenerator.cs]

- [x] [Review][Defer] `npm run check:ir-content` ships red under a required CI gate — `web/package.json:25` chains it into `npm run check`, run at `.github/workflows/build-test-analyze.yml:260` with no `continue-on-error`; credibly attributed as pre-existing/baseline-identical, but the promised follow-up exists only as a sentence in the Completion Notes — deferred, pre-existing
- [x] [Review][Defer] `EXPECTED_SCHEMA_VERSION` is duplicated in `web/ir/adapter.ts:66` and `web/ir/adapter.client.ts:36` with the drift hazard documented but nothing testing it; the mismatch path is a `console.warn` in the server adapter and the client constant is never compared to a manifest at all — deferred, pre-existing
- [x] [Review][Defer] Four `BuildHrefMap(_epicsModel, _docs.Values)` call sites remain (`SiteGenerator.cs:4162`, `:4286`, `:4529`, `:4831`); all correct **today** only because their callers run after the pages loop — the guard is call-site ordering, not the code — deferred, pre-existing
- [x] [Review][Defer] Emitter and consumer search different haystacks for the same two markers — `SpaDelivery.ExtractContentRegion` searches the whole captured page, `splitContentRegion` searches the emitted region; equivalent only while no nav markup contains a breadcrumb-classed div — deferred, pre-existing
- [x] [Review][Defer] `EveryWebviewSurface_IncludingCapturedOnes_CarriesNoScript` asserts no `<script` of any kind, but `WebviewRenderAdapter.StripDataIslands` only matches `<script type="application/json">` — the test and the strip assert different contracts — deferred, pre-existing
- [x] [Review][Defer] `.claude/launch.json` carries two port collisions (8109 with `jsoff-20-7`, 8110 with `details-pane-20-8`), lost its trailing newline, and hard-codes other sessions' scratchpad paths — sibling-story content, not 22.4's — deferred, pre-existing
- [x] [Review][Defer] The Completion Notes' recorded fingerprint `06788c0f…` is already stale (now `9bf8ac05…` at `SiteGeneratorAdapterTests.cs:1318`, moved by concurrent Stories 8.9/24.1) — inherently volatile under shared `main`; the story's own "read it from the file" instruction is the durable guidance — deferred, pre-existing

**Dismissed as noise (3):** the claim that `ResolveFollowUpDocPairs` relocates the divergence via retro notes (**refuted** — `SiteGenerator.cs:4704` guards `if (_docs.Count > 0) return _docs.Values…`, so the derived path runs only when `_docs` is empty, and it diverges in the *safe* direction); the claim that the four remaining `_docs.Values` sites sit on empty-`_docs` routes (refuted at every one — the real, narrower point is kept as a defer above); and the observation that the commit range bundles sibling stories (expected under CLAUDE.md, and handled by scoping to the File List).
...
| Date | Note |
|---|---|
| 2026-07-28 | **Code review (`/bmad-code-review 22.4`, three adversarial layers; scoped by File List per CLAUDE.md, sibling Stories 18.2/18.5/25.3/18.4 excluded from the same `6017c2c..HEAD` range).** 2 decision-needed, 7 patch, 7 defer, 3 dismissed. ⚠️ **Two Task 3 subtasks were marked `[x]` but had not been done:** the "share ONE `WorkInventory` instance" requirement and AC #5's required agreement test. Owner resolved both decisions (DR1 build the shared instance; DR2 consumer skips degraded pages) and directed all 9 patches applied. **8 applied in full; DR1 partially** — `BuildSurfacePrelude` is now memoized so the SPA and webview share one instance (a `--webview` run previously built two equal ones), but `RenderEpicsPages` cannot join without moving the golden fingerprint, which AC #7 forbids; that residual is Story 22.5's Trap 1 and `epics.md` now records it. **AC #5's test now exists** and its **vacuity guard fired first**, proving the old fixture rendered no work graph at all — which is exactly why 22.4 measured AC #5 as zero-delta on the golden fixture; the fixture gained a `deferred-work.md` resolved by a spec doc (the real `ResolvingHref` path). Also fixed: a stale-v1 IR now **throws** instead of warn-then-corrupt; the story-fragment degrade emits one `GenerationEvent` instead of forking the IR from the static HTML at `errors=0`; the webview's exclusions moved INSIDE `CapturedRegions` as a skip predicate; `BuildOutline`'s silent story-drop became an invariant throw; ADR 0024's "exhaustively five" corrected to six. ⚠️ **Verification caveat, measured not assumed:** the full suite is **not a usable regression instrument in this tree** — two consecutive unchanged baseline runs failed **15** and **19** tests with barely-overlapping sets (memory `gitmetrics-3s-timeout-silent-deep-git-loss` firing under parallel load). Targeted runs used instead: **92 passed / 0 failed** across SPA, webview, epics-removal, SPA-parity and file-watcher; **88/88** web tests. The golden fingerprint DID move during this session — attributed to a concurrent `specscribe.css` edit at 15:56:40, proven by the fixture never enabling `EmitSpa`/`CapturePages`, so every path touched here is unreachable in it. |
