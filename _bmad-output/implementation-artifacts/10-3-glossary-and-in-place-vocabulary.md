...
- [ ] [Review][Decision] Story's "1588/1588 green" completion claim was false at commit `c7ba041` — Independently reproduced in an isolated worktree checked out at `c7ba04176ecd38df1cd1ea76060952f797815758`: `SiteGeneratorAdapterTests.GenerateAll_GoldenContentFingerprint_IsStableAfterNormalizingVolatileTokens` fails deterministically (committed constant `452785b22c7d243967d353fc6c88bdf3e0394853f9b587a7f4a3e35f2c49c1fa`, actual rendered hash `0c7ad3ea0b6bba37212dc8b6d457a124a13b95c06869b8c33c732f117650b8e5`) — reproduced across a clean rebuild, matching the project's documented "stale-build first-captured-hash trap" pattern. The next story's commit (`26cc545`, "10.5") happened to regenerate this same golden constant, so current `main` is not known-broken by this today — but the verification claim in this story's own Dev Agent Record was not true when written. Needs a human call: correct the historical Completion Notes, and/or decide whether to harden the golden-hash regeneration procedure (always capture from a clean `dotnet test` run, never an incremental build) to stop this recurring.
- [ ] [Review][Patch] `AbbreviationExpander` throws on a future glossary with duplicate acronym `Term`s [src/SpecScribe/AbbreviationExpander.cs:41] — `acronyms.ToDictionary(a => a.Term, ...)` has no dedupe guard; a future module glossary entry sharing a `Term` with an existing one will throw `ArgumentException` and take down page rendering for that module. Not triggered by today's two glossaries but the story explicitly frames this list as extensible.
- [ ] [Review][Patch] `.howtoread-glossary .cap-row` never collapses to one column on mobile [src/SpecScribe/assets/specscribe.css:1878] — this two-class selector has higher specificity than the `@media (max-width: 560px) { .cap-row { ... } }` one-class rule, so it wins at any viewport width regardless of source order. Every other `.cap-row` consumer (e.g. the About page) correctly stacks on narrow screens; the glossary is the one exception, risking crowded/truncated definitions like "Non-Functional Requirement" on phones.
- [ ] [Review][Patch] `SiteNav.HowToReadOutputPath`'s XML doc comment contradicts the code beneath it [src/SpecScribe/SiteNav.cs:61-65, :140] — the doc comment says the page is "deliberately NOT a top-nav item," but `Build()` unconditionally adds it to the top-nav `project` group (a deliberate, correctly-recorded deviation elsewhere in the Dev Agent Record). The XML doc comment itself was never updated to match and is misleading to a future reader relying on it.
- [ ] [Review][Patch] `IconsTests`'s hand-maintained label list omits the new nav entry [tests/SpecScribe.Tests/IconsTests.cs:24-45] — `ForConcept_EveryKnownLabelReturnsAGlyph`'s explicit `[InlineData]` list documents every other nav label (Readme, PRD, Epics, ADRs, Sprint, Code Map, …) but not "How to read this portal," contradicting this story's own completion notes which claim `IconsTests` was updated. Functional coverage exists incidentally via a different, dynamically-driven test, but the explicit documentation list is stale.
