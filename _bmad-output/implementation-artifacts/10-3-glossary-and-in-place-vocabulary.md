...
- [x] [Review][Patch] Correct Completion Notes + harden golden-hash capture — Decision resolved (option 2): the "1588/1588 green" claim at commit `c7ba041` was false (`GenerateAll_GoldenContentFingerprint_IsStableAfterNormalizingVolatileTokens` failed until `26cc545` / 10.5 re-locked the hash). Patch: (a) amend Dev Agent Record Completion Notes / Debug Log to record the inaccurate claim and the stale-build first-captured-hash trap; (b) harden the golden-hash regeneration procedure so the constant is always captured from a clean `dotnet test` run, never an incremental build.
- [x] [Review][Patch] `AbbreviationExpander` throws on a future glossary with duplicate acronym `Term`s [src/SpecScribe/AbbreviationExpander.cs:41] — `acronyms.ToDictionary(a => a.Term, ...)` has no dedupe guard; a future module glossary entry sharing a `Term` with an existing one will throw `ArgumentException` and take down page rendering for that module. Not triggered by today's two glossaries but the story explicitly frames this list as extensible.
- [x] [Review][Patch] `.howtoread-glossary .cap-row` never collapses to one column on mobile [src/SpecScribe/assets/specscribe.css:1878] — this two-class selector has higher specificity than the `@media (max-width: 560px) { .cap-row { ... } }` one-class rule, so it wins at any viewport width regardless of source order. Every other `.cap-row` consumer (e.g. the About page) correctly stacks on narrow screens; the glossary is the one exception, risking crowded/truncated definitions like "Non-Functional Requirement" on phones.
- [x] [Review][Patch] `SiteNav.HowToReadOutputPath`'s XML doc comment contradicts the code beneath it [src/SpecScribe/SiteNav.cs:61-65, :140] — the doc comment says the page is "deliberately NOT a top-nav item," but `Build()` unconditionally adds it to the top-nav `project` group (a deliberate, correctly-recorded deviation elsewhere in the Dev Agent Record). The XML doc comment itself was never updated to match and is misleading to a future reader relying on it.
- [x] [Review][Patch] `IconsTests`'s hand-maintained label list omits the new nav entry [tests/SpecScribe.Tests/IconsTests.cs:24-45] — `ForConcept_EveryKnownLabelReturnsAGlyph`'s explicit `[InlineData]` list documents every other nav label (Readme, PRD, Epics, ADRs, Sprint, Code Map, …) but not "How to read this portal," contradicting this story's own completion notes which claim `IconsTests` was updated. Functional coverage exists incidentally via a different, dynamically-driven test, but the explicit documentation list is stale.
- [x] [Review][Patch] How-to-read intro promises "sections below" even when every section is omitted [src/SpecScribe/HowToReadTemplater.cs:37-43] — when reading-order gates all fail and glossary/commands are empty (`ModuleContext.None` + shallow/empty nav), `AppendReadingOrder`/`AppendGlossary`/`AppendCommandLegend` all return silently while the intro still says the sections below walk you through what to read first; empty orientation page with lying copy.
- [x] [Review][Defer] `\bADR\b` expands mid-token in real "ADR-0005"/"ADR 0005" story-page references [src/SpecScribe/AbbreviationExpander.cs:39] — deferred, pre-existing pattern with no earlier linkifier protecting it; cosmetically odd (tooltip lands on the bare "ADR" adjacent to its number) but arguably the intended first-use behavior and not a functional break. No test currently pins the desired behavior either way.
- [x] [Review][Defer] `AbbreviationExpander` protected-split does not shield HTML comments [src/SpecScribe/AbbreviationExpander.cs:17-20] — deferred, pre-existing; same `RequirementLinkifier` grammar; a bare acronym inside `<!-- … -->` can be rewritten into `<abbr>` markup inside the comment.
...
- **Correction (code review 2026-07-20):** The completion-time claim that the full suite was "1588/1588 green" at commit `c7ba041` was **false**. An isolated clean checkout of that SHA fails `GenerateAll_GoldenContentFingerprint_IsStableAfterNormalizingVolatileTokens` deterministically (committed constant `452785b22c…`, actual `0c7ad3ea0b…`) — the stale-build first-captured-hash trap. Story 10.5 (`26cc545`) later re-locked the golden constant; current `main` is not broken by that historical miss. Procedure hardened in `SiteGeneratorAdapterTests` (clean rebuild + stable multi-run capture before locking).
...
- Regenerated `SiteGeneratorAdapterTests`'s golden content-fingerprint constant and output-inventory list (new `how-to-read.html` page + every page's nav/CSS delta), and updated the `SiteNavTests`/`RenderParityTests` fixtures the new "How to read this portal" nav entry touched. **Correction:** the contemporaneous "1588/1588 green" / `IconsTests` update claims were inaccurate — the golden fingerprint locked at `c7ba041` failed a clean rebuild (stale-build first-captured-hash trap; fixed later by 10.5), and `IconsTests`'s hand-maintained `[InlineData]` list omitted the new label until the 2026-07-20 code-review patch.
...
- 2026-07-20: code-review — applied 6 patches (duplicate-term guard, glossary mobile CSS, SiteNav XML doc, IconsTests InlineData, empty how-to-read intro honesty, Completion Notes + golden-hash clean-rebuild procedure); deferred 2 pre-existing protected-split items. Golden fingerprint re-locked after clean rebuild. Status → done.
