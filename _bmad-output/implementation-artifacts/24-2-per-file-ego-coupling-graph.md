...
1. **Given** a code page for a file with coupling data and JavaScript available
   ...
   **Then** the focal file sits at the center with its coupled neighbors (bounded to a sensible degree, 1–2 hops) as a force-directed node-link graph, nodes sized by co-change support and edges width-**banded** by shared commits with confidence carried on the node's radial distance plus the tooltip and the twin (cross-boundary edges emphasized by dash pattern), with rich hover/focus tooltips (UX-DR19) and nodes linking to their own code pages

   > **AC #1 amendment, owner-approved at code review 2026-08-07.** The original wording said "nodes sized by change
   > frequency and edges weighted/colored by the Story 24.1 confidence". That contradicted this story's own Task 3,
   > which forbids encoding confidence in stroke width — ADR 0030 §5 forces per-edge style into trace-level classes,
   > which quantizes width into bands, and a banded channel cannot honestly carry a continuous metric. The shipped
   > encoding is: width band = shared commits, radius = confidence, dash = cross-boundary/process, shape = node kind.
   > Citing artifacts carry no change-frequency signal of their own and render at a fixed base size. The
   > implementation was right and the AC text was stale; the text is what moved.
...
2. **Given** a JavaScript-off, reduced-motion, or assistive-technology visitor (NFR8)
   ...
   **And** a file with no qualifying couples renders **no relationship surface at all** — no host, no island, no inert controls, no empty framed panel — never a broken or misleading empty graph.

   > **AC #2 amendment, owner-approved at code review 2026-08-07.** The original wording asked for "a designed empty
   > state". The shipped behaviour is designed *omission*: `RelationshipGraph.Render` returns `string.Empty` for a
   > zero-node model and `BuildRelationshipsPanel` drops the tab entirely, pinned by
   > `RelationshipGraphTests.Render_EmptyModel_EmitsNothingAtAll`. That is the better reading of NFR8 — an empty
   > framed panel promising a chart that is not coming is itself the misleading state the clause exists to prevent.
   > Ratified rather than re-implemented. Note this is distinct from a PARTIALLY empty instance, which *is* handled
   > by conditional suppression: a file with citers but no couples renders citations-only, with the coupling framing
   > sentence and the coupling legend rows suppressed.
...
Code review 2026-08-07 (`/bmad-code-review 24.2`). Scoped to this story's own File List and hunks in commit
`6df8e0d` ("Today's pulse") plus `5a78ee7` (the ir-content regen) — **NOT** the commit range. `6df8e0d` also carried
create-story 24.3, `epics.md`, ADR 0033 and a 23.x story; all excluded by hunk. Twelve sibling commits have since
touched these files (24.1's review patches, 23.2's review patches, 23.6, the sunburst fixes) and are likewise
excluded. Four layers: Blind Hunter, Edge Case Hunter, Acceptance Auditor A (ACs + owner decisions), Acceptance
Auditor B (ADR conformance, completed inline after three environmental agent failures).

**Compliance:** D1 / D2 / D3 **MET**. AC #1 / #2 / #3 **PARTIAL**. ADR 0030 §2/§3/§4/§5 **MET**, ADR 0012 §3/§6
**MET**, ADR 0013 §2 **PARTIAL** (lift is `title`-attribute-only). D2's index alignment at the wider cap was
independently re-verified and genuinely holds — all three consumers iterate the same uncapped `insight.CoupledFiles`,
no builder hardcodes 8, and there is still exactly one git call and one commit scan.

- [x] [Review][Decision] **`.code-relationships` is dead styling — restore the gold rule, or accept the chart-panel look?** — The card moved to `panelClass: "chart-panel code-relationships"`. `.code-relationships` (specscribe.css:1826) and `.chart-panel` (:4021) have equal specificity (0,1,0), so the later rule wins every conflicting declaration; its `border` shorthand resets `border-left` and `background: var(--warm-white)` replaces `var(--parchment)`. All five declarations are overridden and the retained comment asserts the opposite. Precedent for the fix exists at :3841 (`.chart-panel.review-findings { border-left: 3px solid var(--rust); }`). Owner call because it is a visible design choice.
- [x] [Review][Decision] **AC #2's "designed empty state" is implemented as omission** — `RelationshipGraph.Render` returns `string.Empty` for a zero-node model and `BuildRelationshipsPanel` drops the tab entirely; `Render_EmptyModel_EmitsNothingAtAll` pins that as intended per NFR8. "Never a broken or misleading empty graph" is met; "shows a designed empty state" is met by omission, not by a state. Ratify or require a state.
- [x] [Review][Decision] **AC #1's encoding wording diverges from the shipped channels** — AC #1 says "edges weighted/colored by the Story 24.1 confidence"; the code bands edge width by **support** (`WidthBandFor(e.Support, maxSupport)`) and moves confidence to node **radius** + tooltip + twin, because Task 3 explicitly forbade confidence-in-stroke-width. Separately "nodes sized by change frequency" is shared-commit count, with every citing artifact at a fixed base size. Both are documented in-code and internally consistent; the AC text is what is stale. Amend the AC for the record, or accept as-is.
- [x] [Review][Patch] **A failed mount leaves the control bar and legend revealed over a dead chart** [src/SpecScribe/assets/specscribe.js:unwindRelGraph] — `controls.hidden = false` runs before `Plotly.newPlot` and the legend is revealed synchronously after; `unwindRelGraph` restores only the host and panel markers. On the async-rejection path the reader gets a filter bar and an eight-row legend over a `display:none` host — the exact JS-off defect this story closed, re-entering through the failure path. The checkboxes also stay live, so a click calls `Plotly.restyle` on a purged div and throws uncaught.
- [x] [Review][Patch] **`flushRelGraphReveals` swallows a post-`newPlot` throw without unwinding** [src/SpecScribe/assets/specscribe.js:flushRelGraphReveals] — bare `catch (err) {}` where the eager path deliberately calls `unwindRelGraph(root)` and documents why. Leaks a plotted-but-unregistered instance plus a permanent window `resize` listener closing over the payload, and leaves `data-relgraph-ready` unset so a later pass mounts a second plot over the same div. With an Insights panel present this deferred path is the **default** path for every rich `--deep-git` page, not a corner case.
- [x] [Review][Patch] **`unwindRelGraph` never splices the host out of `relGraphMounts`** [src/SpecScribe/assets/specscribe.js:unwindRelGraph] — the root is pushed synchronously, then an async rejection unwinds without removing it. The next init re-mounts and pushes a **second** entry for the same node, so `Plotly.Plots.resize` and `__ssRelGraphCleanup` run twice per host thereafter.
- [x] [Review][Patch] **A declined deferred mount never clears the boot placeholder** [src/SpecScribe/assets/specscribe.js:flushRelGraphReveals] — no `else` branch, so `data-relgraph-failed` is never set when `initRelationshipGraph` returns `false` (e.g. the Plotly bundle is blocked). The reader sees "Initializing graph…" over an empty box for the rest of the 5 s `BootTimeoutMs` window instead of the immediate hand-back the eager path gives; the root is also dropped from `relGraphPending` with no retry.
- [x] [Review][Patch] **Filter state is seeded from restored checkbox state *after* the traces are built** [src/SpecScribe/assets/specscribe.js:initRelationshipGraph] — `filters[key] = !!input.checked` runs post-geometry and nothing re-filters before `newPlot`. After a Firefox reload or a back/forward navigation the browser restores "Group by epic" as checked while no hub is drawn, the ready announcement reports the unfiltered count, and the reader must toggle **twice** to recover.
- [x] [Review][Patch] **`edgeText` uses `String.prototype.replace` with string patterns** [src/SpecScribe/assets/specscribe.js:edgeText] — `k.phrase.replace("{a}", a.p).replace("{b}", b.p)`. The replacement argument is still scanned for `$&`, `` $` ``, `$'`, `$$`, so a path containing `$&` renders `src/a{a}b.cs`; and a path containing a literal `{b}` is hit by the second `.replace` before the real placeholder, corrupting both endpoints. Corrupts the edge tooltip and the accessible name. Fix: `.split("{a}").join(a.p)` or a function replacer.
- [x] [Review][Patch] **`RelGraphDomSlug` collapses `-`, `_`, `.` and case into one token** [src/SpecScribe/CodeFileTemplater.cs:RelGraphDomSlug] — the `x2f` escape guards slashes only, so `src/a-b.cs` and `src/a_b.cs` both yield `relgraph-code-x2f-src-x2f-a-b-cs-html`. The client resolves its payload document-wide by id, so under SPA/webview consolidation the second graph mounts the first file's nodes and coordinates. The method's own doc comment states the stakes ("a collision is now a mis-rendered chart") without closing this case.
- [x] [Review][Patch] **`ContainsHost` is a bare substring test that matches the project's own source text** [src/SpecScribe/RelationshipGraph.cs:ContainsHost] — `HostMarker = "data-relgraph"` is a prefix of seven sibling attributes and appears verbatim in files the code page embeds via `BuildSource`. `RelationshipGraph.cs`'s and `specscribe.js`'s own code pages would set `GraphEngineNeeded`/`GraphBootInline` and pull the 1.2 MB bundle for a chart they do not have, against `AssetManifest.cs`'s stated byte-identical contract. Masked today because `SiteGenerator` only generates pages for cited files (which always have a real host); live at the public `RenderPage` API.
- [x] [Review][Patch] **The "collision is IMPOSSIBLE BY CONSTRUCTION" guarantee is angular, not geometric** [src/SpecScribe/CouplingLayout.cs:MaxDriftFraction] — the bound is on **angle**; stacking is a function of arc length x radius in pixels. At this story's own caps (40 ring markers, `RingOuter = 0.46`, `Size = 520`) the guaranteed worst-case separation is ~10 px against markers the client sizes at 9-24 px. `CouplingLayoutTests.Solve_AngularDriftIsBoundedSoMarkersCannotStack` asserts only the angular inequality, so it proves the arithmetic clause and not the claim in its own name. The layout is fine in practice (the live pass measured 4 grazing pairs, worst ratio 0.82) — the overstated guarantee is what needs correcting.
- [x] [Review][Patch] **`GoldenReplacement_Payload_CarriesEveryNodeAndEdgeTheChartWillDraw` asserts nothing about edges** [tests/SpecScribe.Tests/SiteGeneratorCodeInsightsTests.cs] — no `"edges":[`, no `"a":`/`"b":`, no style-key, no count. `"w":` and `"t":` both match node fields, so a payload emitting **zero edges would pass** a test whose name claims edge coverage. This is one third of AC #3's fingerprint replacement.
- [x] [Review][Patch] **The twin replacement test checks "non-colour" with a link, not a word** [tests/SpecScribe.Tests/SiteGeneratorCodeInsightsTests.cs:GoldenReplacement_TextTwin_*] — the comment claims every dash/shape/width distinction is also a word; the assertion beneath it is `Assert.Contains("code/src/Lib/Sibling.cs.html", twin)`, a navigation check. Neither `cross-boundary` nor `process coupling` — this story's own twin-completeness fix — is asserted anywhere in the replacement set.
- [x] [Review][Patch] **Stale a11y attributes survive on a marker whose payload node cannot be resolved** [src/SpecScribe/assets/specscribe.js:applyA11yLayer] — `if (!n) continue;` skips the whole stamp, so a previously-labelled marker keeps the prior node's `aria-label`, `data-tip-html` and `data-relgraph-href`: an accessible name on the wrong marker. Reachable on the same `pts.length > drawnNodes.length` skew the one-call-restyle comment says was observed live.
- [x] [Review][Patch] **Edge midpoint labels are transiently stamped against a stale DOM** [src/SpecScribe/assets/specscribe.js:applyFilters] — `drawnEdges = mg.members` is assigned before both restyles, and the node-trace restyle fires `plotly_afterplot` before the mid-trace restyle runs, so the first `applyA11yLayer` maps new `drawnEdges` onto the old midpoint DOM. Self-corrects on the second afterplot — the same "wrong in the window" shape the node trace was already fixed for, left in place for edges.
- [x] [Review][Patch] **`Escape` does not move focus off the marker** [src/SpecScribe/assets/specscribe.js:onNodeKeydown] — `root.blur()` where the focused element is the `path.point`; `root` has no `tabindex` and is never `document.activeElement`, so the call is a no-op and Escape only hides the tooltip. `ev.currentTarget.blur()` is what the handler intends.
- [x] [Review][Patch] **Dead local left behind by the note deletion** [src/SpecScribe/CodeFileTemplater.cs:425] — `var hasRelated = related.Count > 0;` in `BuildRelationshipsCard`; its only consumer was the deleted `note` ternary. Assigned, never read (the twin at :644 has its own copy). Reads as though the card still branches on the coupled population.
- [x] [Review][Patch] **File List omits the regenerated IR content layer** [24-2-per-file-ego-coupling-graph.md] — the `.ref-*` deletion and `.ss-relgraph*` addition required `web/assets/ir-content.css` + `ir-content.manifest.json` to be re-derived, which happened in a **separate later commit** (`5a78ee7`) contrary to CLAUDE.md's same-change rule. No live defect: `5a78ee7` is an ancestor of HEAD and the current file carries 23 `ss-relgraph` references with zero retired-family references, so `check:ir-content` is green. Add both files to the File List and note the ordering.
- [x] [Review][Defer] **Lift reaches the twin only through a `title` attribute** [src/SpecScribe/CodeFileTemplater.cs:BuildRelationshipsTwin] — deferred, pre-existing. `title` on a non-interactive `<li>` is unreliably announced by screen readers, so a fact available to a sighted mouse user from both the chart tooltip and the twin hover may reach an AT user from neither (ADR 0013 §2 completeness). The in-code comment shows this is a deliberate Story 24.1 choice — spending sr-only reading time on lift would bury the confidence the row is about — so it predates this story and warrants its own decision rather than a drive-by change.

**Dismissed as noise (2):** the live-browser audit's numbers are unverifiable from a diff (that is the nature of diff review, not a defect — the code-side preconditions it depends on are all present); and the roving-order deviation's justification substantially holds (twin is citers→coupled, graph is focal→artifacts→hubs→coupled, so the two populations agree in relative order — the graph merely also includes focal and hub nodes the listing does not enumerate).
...
- `web/assets/ir-content.css`, `web/assets/ir-content.manifest.json` — the scoped IR-content layer re-derived for the `.ref-*` deletion and the `.ss-relgraph*` addition. **Added at code review 2026-08-07**: this was missing from the File List, and the regeneration landed in a *separate later* commit (`5a78ee7`) rather than riding this change, contrary to CLAUDE.md's same-change rule for a `specscribe.css` edit. No live defect — `5a78ee7` is an ancestor of HEAD and `check:ir-content` is green — but the ordering is recorded so the next stylesheet change does not repeat it.
...
| Date | Change |
| --- | --- |
| 2026-08-07 | Code review (`/bmad-code-review 24.2`, worktree `code-review-24-2` cut from `c73ebcb`): review → **done**. Scoped by File List and hunk over `6df8e0d` + `5a78ee7`, not the commit range. Four adversarial layers; 3 owner decisions, 16 patches applied, 1 deferred, 2 dismissed. D1/D2/D3, ADR 0030 §2–§5 and ADR 0012 §3/§6 all MET; ADR 0013 §2 PARTIAL (lift is `title`-only, deferred). The headline is that the component's **failure** path re-opened the JS-off defect its own live pass had closed — a six-item mount-lifecycle cluster both hunters found independently. Also: two vacuous assertions inside ADR 0013 §6's own fingerprint replacement, a DOM-id slug collision that would have drawn the wrong file's relationships under SPA/webview consolidation, an `AssetManifest` flag that matched the project's own embedded source text, an overstated "collision is impossible by construction" guarantee that is angular rather than geometric, and a `.code-relationships` rule every declaration of which was silently overridden while its comment claimed otherwise. AC #1 and AC #2 amended by owner decision to match what shipped. Suite 2,978 passed / 0 failed / 3 skipped after a `--no-incremental` rebuild. `check:ir-content` is environmentally red in the worktree (a full extract over-prunes by 1,571 lines on a 503-page IR); extraction reverted for a surgical two-file edit, and the gate needs re-verifying on `main`. |
