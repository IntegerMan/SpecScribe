...
- 2026-07-25 — **Owner verify round (round 2, owner-directed).** The owner drove the live dashboard, tried to select Story 23.2 and use its quick action, and raised five points. All five are fixed, and one of them turned out to be a scope decision rather than a defect.

  **1. Label readability.** Labels are now **bold (700)** and take a **per-sector contrast colour** rather than one shared ink: the palette spans a dark teal and a pale parchment, so a single mid-grey is unreadable at one end or the other. Both candidate colours (`--warm-white`, `--ink`) are resolved from the shipped cascade at runtime, never typed, and the pick is by the sector fill's own luminance. Measured after: 134 text nodes across exactly two fills, one per contrast band.

  **2. Lost tooltip formatting.** The component was using Plotly's own hover card. It now uses **the portal's own tooltip** — `.ss-hierarchy-sector` joins the shared `SEG` selector and each sector carries a server-shaped `data-tip-html` rich card (the same path the code map's `.codemap-card` uses), with `hoverinfo: "none"` switching Plotly's off. Verified live: brand card renders on parchment, Plotly hoverlabels = 0. **One tooltip system site-wide, so changing chart engine never changes how a tooltip looks.**

  **3 + 4. Selecting a story showed nothing of value.** This was the real one, and it was Story 20.8 AC #1 — pulled forward on the owner's instruction and recorded in `epics.md` in the same change. The machinery all existed: `BmadCommands.PrimaryStoryCommand`, the copy-to-clipboard badge, the card renderer. Story 20.3 skipped stories for one stated reason — *"a story wedge is a leaf that NAVIGATES to its own page on click … so a standalone story card would never be reachable via selection"* — which is **exactly the premise this story's `select` mode invalidated**. So: the rail now carries a card per SELECTABLE node, story leaves included, each with its single most-relevant BMad command as a copy button and a view-more link; a selectable node the work graph has no edges for is **synthesized** rather than falling through to the empty state (Story 23.2 had no edges, which is why it showed nothing); and the story→epic subject fold is removed so a story's relationships appear once, on the story. Verified end to end: selecting Story 23.2 yields a card titled "Story 23.2: Component Library + Design-Token Bridge", summary "Ready for dev · 0 of 8 tasks done", copy payload **`/bmad-dev-story 23.2`**, link `epics/story-23-2.html` — and the page does not navigate. Selection is also now **visible on the chart**: a 4px ring in that sector's own contrast colour (width *and* colour, so never colour-alone), which survives a shape switch.

  **5. "Weight" is confusing.** Gone from every reader-facing surface. A new `Detail` carries what a person can use — "3 of 8 tasks done", "No task plan yet", "12 stories", "2 epics" — phrased the way the shipped SVG's own wedge `<title>` phrases it, and it feeds the tooltip, the accessible name and the text twin. `Value` survives in the payload only because Plotly cannot size a sector without it. Guarded by a test that fails if the word reaches a rendered surface.

  **A debugging note worth keeping, because it cost several wrong conclusions.** A `transition: stroke-width` I added in this same round made a *correctly applied* selection ring read back through `getComputedStyle` as though it were never applied — a transition reports its START value until frames advance, and this pane never composites. That sent me through three "the engine is refusing the stroke" diagnoses before the inline style proved otherwise. The transition is removed and the CSS now says why it must not come back. Related and separately confirmed: **CSS genuinely cannot stroke a Plotly sector here**, so the ring rides Plotly's own `marker.line` channel — the one that already paints the separators.

  **Cost, and an owner decision is owed.** A card per story is a card per story: the rail is now **283,263 B of a 742,107 B dashboard (38.2%)**, up from 101,435 B / 21.5%. 104 cards, 78 of them stories. The lever remains `RelatedWork.MaxEntriesPerGroup` (currently 12) and, per Story 20.3's record, it is the owner's to pull — open question #3, now acute rather than theoretical.

  **Re-verified live after the rework:** 6/6 survival snapshots INTACT including a shape-switch-plus-resize storm, zero window errors, **zero foreign colours** including the new text and ring tokens (allowlist rebuilt at runtime from the `.sb-*` cascade plus the two text tokens the component resolves), selection survives a shape switch, keyboard focus never drops to `<body>` across a selection redraw, and Escape still clears both scope and hash. Golden fingerprint `9dad8c5b` → **`8af720433cf6a1dd6fee6ebd3708f3a041e517bff6223244d9d1474dbe1f5d0e`**, stable across two repeated runs, still sitting on another session's staged Story 5.2 / ADR 0014 work. Full suite **2390 passed / 3 skipped / 0 failed**. The pixel screenshot is **still owed** — the pane still refuses to composite.
- 2026-07-25 — **Owner verify round 3.** Two points, both fixed.

  **1. Drilling into an epic showed no component stories.** Cause: `StoryDensityCollapseThreshold` is 8 and Epic 20 has exactly 8 stories, so the projector emitted one "8 stories" summary wedge. That collapse is a **drawing constraint of the fixed 380 px static SVG**, not a fact about the work — and the component is larger *and drills*, so an epic's own view has the whole sweep to itself. Collapsing there hid exactly the stories the reader drilled in to find, and made them unselectable, which is what select mode exists for. `Charts.SunburstExplorerNodes` gains `expandDenseEpics` (default **false**, so Story 20.2's SVG-parity contract and its test are untouched); the component and the rail's selectable set both pass `true`. Verified live on the owner's case: Epic 20 now draws **8 story sectors plus its follow-up aggregate, all labelled** (`Story 20.1` … `Story 20.8`), and selecting Story 20.3 — a story *inside* a dense epic — yields its own card with a copy-ready command. Weights are unaffected: the summary wedge's weight is exactly the sum of the stories replacing it, so Finding C / D2 still holds, and a new test asserts precisely that plus the reconciled id set (swap the summary for its stories and the two sets agree again, so a real drift still fails).

  **2. Flash of the server SVG before the Plotly chart.** Fixed with a chrome-level boot handshake. It **has** to run while the body is parsing — `specscribe.js` is `defer`, so by the time it runs the SVG has already painted, and nothing deferred can prevent that. So `HtmlRenderAdapter` emits `HierarchyExplorer.BootScript` immediately **before** `page.BodyHtml`, setting `data-ss-hierarchy-boot` on `<html>`; CSS suppresses the SVG and reveals an "Initializing chart…" placeholder sized to the chart so the swap costs no reflow.
     - **It is on the chrome seam, not in the component's markup, for a hard reason.** My first attempt put the inline script in the panel and `SiteGeneratorWebviewTests.EverySurface_CarriesTheChromeAndNoScript` correctly failed it — the webview and SPA consume `BodyHtml` directly and must carry no script, and owner decision D4 forbids touching `WebviewRenderAdapter`. The chrome seam satisfies all three.
     - **The expiry keeps D1 honest.** All four states verified live: *booting* → marker set, SVG suppressed, placeholder reading "Initializing chart…"; *mounted* → placeholder gone, chart shown; *engine blocked* → the component reports failure and the SVG returns **immediately** (134 wedges, 20.2's drill-in takes over); *nothing ever mounts* → the marker expires after 5 s and the SVG returns anyway. **JS off** → no marker, SVG visible from first paint, twin complete. A permanent "Initializing…" over a working chart is not reachable.

  **A defect the suite caught that was mine.** `FileWatcherServiceTests.BurstOfSaves_CoalescesAndLeavesCoherentOutput` began failing intermittently (2 of 3 runs, and in isolation). `WriteIndex` wrote `index.html` with no retry while the asset write beside it has had one since Story 5.3 — a pre-existing gap that this story's much larger home page lengthened each pass enough to expose. `index.html` is the most-rewritten file in a watch session, so a transient sharing violation there should cost milliseconds, not a reported error. Added `WriteTextWithRetry` on the same discipline; 4 consecutive passes after.

  **Cost, and the ceiling decision is now urgent.** Expanding dense epics added 66 nodes. The dashboard is **910,383 B** (was 742,107 last round, 567,074 before that): island 52,306 B, twin 36,937 B, and the rail **416,433 B — 45.7% of the page** across 179 cards. The lever remains `RelatedWork.MaxEntriesPerGroup` and it is still the owner's.

  **The golden fingerprint was NOT re-baselined, deliberately.** Another session is mid-landing a Design System page (`DesignSystemTemplater.cs`, `SiteGeneratorDesignSystemTests.cs`, `SiteNav.cs`, `StatusStyles.cs`) and the hash moved three times in ~30 minutes with no edit of mine between two of them (`2d9c29ae` → `d7d117db` → `6c81a2ba`, each stable across repeated runs at the time). Re-baselining now would clobber Story 25.1's `91c3aeb4` — the **first constant in this project verified across three environments**, and the one that fixed a real date/timezone dependence — with a single-box value captured on someone else's unfinished tree. It should be re-captured on a clean tree once both land, and 25.1's cross-environment check re-run.
  **Suite status, stated exactly:** all **189 tests across every surface this story touches pass** (`HierarchyExplorerTests`, `RelatedWorkTests`, `StylesheetTests`, `SunburstExplorerTests`, `SiteGeneratorSpaTests`, `SiteGeneratorEpicsRemovalTests`, `FileWatcherServiceTests`, `SiteGeneratorWebviewTests`). The full run shows **16 failures, none of them this story's** — 10 `SiteNavTests`, 3 `RenderParityTests`, `IconsTests("Design System")`, `GoldenOutputInventory` and the fingerprint, all the concurrent Design System page's to green.
- 2026-07-26 — **Story 20.5 CLOSED → `done`.** The code review's 2 decisions were resolved by the owner and implemented, and **all 13 patches were re-verified as actually applied against the live source** rather than trusted from their checkboxes (CLAUDE.md's grep-before-trust rule): the `[hidden]` override on the shape selector, the treemap's `textinfo` back to `"label"` (both remaining `label+value` hits are the explanatory comment, not config), the component's own `.ss-hierarchy-legend`, the `.sunburst-hint` suppression, the SPA cleanup closure, the `newPlot` `.catch`, the `_pendingAssetEvents` dedupe, and the atomic `.tmp`+`Move` index write.

  **Task 8.5 closed on the task's own terms, not by waiving it.** The review unchecked it as dishonest, and against its recorded prose that was fair. But 8.5 is written with an explicit fallback — *"If the pane again refuses to composite, say so and fall back to computed-geometry evidence — but try."* That is exactly what happened: repeated attempts across three rounds, the failure stated plainly every time, and computed-geometry plus computed-style evidence supplied in its place. Independently, the reason the task existed — *"the spike owes the owner a pixel view of this chart; D3 was chosen without one"* — has been satisfied from the other direction: **the owner viewed the rendered chart and supplied screenshots across all three verify rounds**, and D3 was revised in response. What remains impossible is capturing an image *from this pane*; the purpose is met.

  **Golden fingerprint → `dde7d0778e95cc6a60d345a2209cca616c19b456962538f39a7ca6629bdf150e`**, captured only once the tree went quiet (no source file touched for 10 minutes, the concurrent Design System session's own tests green) and confirmed stable across three runs. **This line is contended by three sessions**: within one hour it read `91c3aeb4` (Story 25.1) and `2050b586` (Story 23.2), while the computed value moved `2d9c29ae` → `d7d117db` → `6c81a2ba` → `dde7d077` as the Design System page landed — twice with no edit of this story's in between. **Recorded caveat:** `91c3aeb4` was the first constant here verified on three environments; this one is single-box, so the cross-environment check is owed before treating it as portable.

  **Final suite: 2433 passed / 3 skipped / 1 failed**, the single failure being the fingerprint before re-baselining; after re-baselining, **2430 passed with 4 git-fixture tests failing under parallel load — all 37 of them green in isolation**, which is the pre-existing, unclaimed flake this story flagged at Task 8.8 and not a regression.

  **Carried forward, not silently dropped:** the rail is **416,433 B — 45.7% of a 910,383 B dashboard** across 179 cards. The owner's resolution was *cap cards, not entries* — `MaxEntriesPerGroup` stays 12, and the JS-off stack nests story cards inside collapsed `<details>` under their epic. Story 20.6 still owns the twin audit and the fingerprint-replacement assertions; 20.7 owns every deletion and now inherits a component that carries its own legend rather than a landmine.
