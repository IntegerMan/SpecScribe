...
- [x] [Review][Patch] Coincident points fully occlude each other with no way to reach the covered ones — When two or more files share the exact same `(Lines, Changes)` pair, their SVG circles render at the identical `(cx, cy)`; only the last-drawn (alphabetically-last) circle is reachable by mouse hover/click. **Owner decision: add small deterministic jitter** (keyed on path, not random) to separate exactly-coincident points so every circle stays independently hoverable/clickable. [`Charts.cs:2667-2706`] — **Applied**: `coincidentGroups` + `jitterRadius` in `Charts.RiskQuadrant`; regression test `RiskQuadrant_CoincidentPoints_GetDeterministicJitterSoEveryCircleStaysReachable`.
- [x] [Review][Patch] `RiskQuadrant` has no detail-cap on rich tooltip cards, unlike its sibling `CodeTreemap` [`Charts.cs:2692`, cf. `Charts.cs:2210-2235`] — **Applied**: reused `SelectDetailedCodeMapFiles`/`MaxDetailedCodeMapFiles`; long-tail points fold metrics into `aria-label` via `CompactMetricsTail` instead of the card. Regression test `RiskQuadrant_AboveTheDetailCap_LongTailPointsLoseTheCard_ButKeepAccessibleName`.
- [x] [Review][Patch] `WriteRiskQuadrant` redundantly rebuilds the whole `CodeMap` instead of reusing `WriteCodeMap`'s already-built "full" variant [`SiteGenerator.cs:3099-3113`, cf. `SiteGenerator.cs:3066-3090`] — **Applied**: `WriteCodeMap` now returns the built `CodeMap?`, threaded into `WriteRiskQuadrant(nav, fullCodeMap)`; no second tree walk.
- [x] [Review][Patch] Inconsistent path comparer between the plot sort order (`Ordinal`) and the ranked-list tie-break (`OrdinalIgnoreCase`) on the same field [`Charts.cs:2509`, `Charts.cs:2552`] — **Applied**: `RiskQuadrantElevatedFiles`'s tie-break now uses `StringComparer.Ordinal`, matching `BuildRiskPoints`.
...
### Second bug-fix / clarity pass (owner-reported)

Three more issues, all cosmetic/UX rather than functional:

1. **"The Y axis feels unclear... not sure about the cutoff being a straight horizontal line."** Root cause: Y (churn frequency) was plotted on a LINEAR scale while X (size) was log-scaled — on this repo's own real data, churn turned out to be just as heavy-tailed as size, so nearly every point bunched against the bottom edge and the median cutoff line landed almost on the baseline, reading as arbitrary. Fixed by log-scaling Y to match X (`BuildRiskPoints` now computes `LogChanges = Math.Log(Math.Max(Changes, 1))`, and the elevated-flag/median-split both operate in that same log space) — this spreads points evenly instead of crushing them, which is also what makes the horizontal cutoff line visually meaningful again. Additionally added real-unit tick labels at both axes' extremes (`.risk-tick-label`) AND a label directly on each median cutoff line itself (`.risk-median-tick-label`, e.g. "median 156" / "median 3") — a log-scaled axis and an unlabeled boundary line otherwise give no sense of actual magnitude. Considered and deliberately did NOT invent a curved/percentile-contour boundary — that's a materially different (and more complex) statistical design than a median split and felt like over-engineering a cosmetic-clarity ask; flagged here in case the owner actually wants that instead.
2. **Pager controls should be at the bottom, not above the list.** Reordered `RiskQuadrantTemplater.AppendElevatedGrid` to emit the `<ol class="risk-grid">` first and the `<div class="risk-pager" hidden>` after it; updated `specscribe.js`'s `initRiskGridPager` to look up the pager via `grid.nextElementSibling` instead of `previousElementSibling` (document order is load-bearing for that lookup, not just cosmetic). New `RiskQuadrantTemplaterTests.RenderPage_PagerFollowsTheGridInDocumentOrder` regression guard.
3. **Per-item rank numbers aren't valuable.** Dropped the `<span class="risk-grid-rank">` entirely from each grid item (and its now-unused CSS rule) — the list is still visually ranked by document order (busiest-first), the number itself added nothing. `RenderPage_GridItemsCarryRankAndMetaText` renamed to `RenderPage_GridItemsCarryMetaTextButNoRankNumber` and asserts the class is gone.

New Charts-level regression coverage: `RiskQuadrant_BothAxesCarryRealUnitTickLabelsAndTheMedianCutoffLinesAreLabeled` and `RiskQuadrant_YAxisIsLogScaledLikeXSoAHeavyTailedChurnDistributionDoesNotCrushEveryPointToTheBaseline` (the latter asserts a doubling-changes series lands at roughly even `cy` spacing — the actual signature of log scaling — rather than the increasingly-compressed spacing linear scaling would produce).

Golden fingerprint regenerated multiple times during this pass as several genuinely unrelated concurrent sessions (Story 7.11's ADR-0010 correct-course, Story 7.12's per-variant freshness-sunburst rework) landed on this shared working tree between each lock-in attempt — each regeneration was verified stable across repeats before committing, and the final constant reflects both this story's changes and whatever the tree looked like at that moment (documented inline). 1961/1964 tests green in the final full-suite run; the 2 persistent failures are the same pre-existing `HtmlTemplaterTests` coverage-card regression (unrelated, confirmed via `git diff`/`grep` this story never touches `HtmlTemplater.cs`); a 4th failure seen in one intermediate run (`SiteGeneratorGroupedNavTests`/`SiteGeneratorTimelineTests`, alternating across runs) passed cleanly in isolation both times it appeared — a transient flake from concurrent file writes during the full-suite run, not a real regression. This story's own 190 tests (RiskQuadrant/RiskQuadrantTemplater/SiteNav/Stylesheet/SiteGeneratorCodeMap/Icons) are green in every run. Manually verified all three fixes (log-scaled Y + tick/median labels, pager-after-grid, no rank numbers) against a fresh `--deep-git` generation of this repo.
...
- 2026-07-21: Second bug-fix/clarity pass (owner feedback) — log-scaled the Y axis to match X (fixes the "Y axis feels unclear"/arbitrary-cutoff feedback: churn is just as heavy-tailed as size on real data), added real-unit tick labels to both axes plus labels directly on the median cutoff lines, moved the pager below the elevated-risk grid, and dropped the per-item rank number. 1961/1964 tests green (2 pre-existing unrelated failures; this story's own 190 tests all green); golden fingerprint regenerated; manually verified against this repo's own `--deep-git` history.
- 2026-07-22: Code review (parallel adversarial + edge-case + acceptance-audit) — 4 patches applied: `RiskQuadrant` now applies the same `MaxDetailedCodeMapFiles` rich-tooltip-card cap `CodeTreemap` uses (guards against the class of bug that once bloated `code-map.html` to ~82.5MB); exactly-coincident points (identical Lines and Changes) get a small deterministic jitter so every circle stays independently hoverable/clickable (owner-decided fix for a decision-needed finding); `WriteRiskQuadrant` now reuses `WriteCodeMap`'s already-built `CodeMap` instead of re-walking `_codeFiles` a second time; the `RiskQuadrantElevatedFiles` tie-break comparer now matches `BuildRiskPoints`'s (`Ordinal`, was `OrdinalIgnoreCase`). 2 items deferred (degenerate-axis duplicate tick labels; non-code framing copy on non-source tracked artifacts — both pre-existing/cosmetic). 2008/2008 tests green; golden fingerprint regenerated (verified via stash/rerun that the shift is 100% attributable to a concurrent `4a107ad "7.11"` commit on this shared tree, not these patches).
