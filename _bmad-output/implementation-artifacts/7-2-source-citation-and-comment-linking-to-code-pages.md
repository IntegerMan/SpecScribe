...
- [ ] [Review][Patch] `IsRelativeCodeHref` is unscoped to actual `[Source: …]` citations — it rewrites/strips ANY relative `<a href>` on the page (any file with an extension, not `.html`/`.md`), so a legitimate prose link to a non-code asset (e.g. an image or download under `_bmad-output`) that doesn't resolve gets silently de-linked to plain text. [`src/SpecScribe/CodeReferenceLinkifier.cs:77-96,165-183`]
- [ ] [Review][Patch] "Referenced by" back-links are never validated against a page that was actually written: `BuildReferencedBy`'s fallback (`PathUtil.ToOutputRelative`) is used whenever a citing artifact isn't in `_referenceMap`, and there's no cleanup of `_codeReverseMap` when a citing artifact's own page generation fails/is skipped — both can render a link to a 404. Symmetrically, `_codePages` entries removed in `GenerateCodePagesInternal` (on write failure/path escape) are pruned only AFTER story/doc/ADR pages were already linkified against the optimistic pre-prune map, so an already-written citing page can carry a dead `#L{n}` link to a code page that never got generated. [`src/SpecScribe/SiteGenerator.cs:711-777,829-877,885-905`]
- [ ] [Review][Patch] `DiscoverCodeReferences(files)` only scans the `_bmad-output` source corpus, never `EnumerateAdrFiles()` — but `GenerateAdrsInternal` still runs every ADR page through `CodeReferenceLinkifier.Linkify`. A code citation inside an ADR for a file not *also* cited by a `_bmad-output` doc silently never resolves (degrades to plain text, not broken, but inconsistent with the story's "runs on every generated page — story, doc, ADR, retro, sprint, index" design intent). [`src/SpecScribe/SiteGenerator.cs:172,530-596`]
- [ ] [Review][Patch] `CodeFileTemplater.RenderPlaceholder` (oversized/binary cited files) never receives the computed `referencedBy` list — only the normal `RenderPage` branch does — so a cited file that happens to be too large or binary silently loses the "Referenced by" block AC #2 otherwise guarantees. [`src/SpecScribe/SiteGenerator.cs:840-860`]
- [ ] [Review][Patch] `ArtifactTitlePattern` matches the first `# …`-shaped line anywhere in the raw markdown, including inside a fenced code block or YAML frontmatter comment — can produce a wrong/misleading title in a "Referenced by" entry. [`src/SpecScribe/SiteGenerator.cs:778-785`]
