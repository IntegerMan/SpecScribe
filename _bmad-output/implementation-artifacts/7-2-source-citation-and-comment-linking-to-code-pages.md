...
- [x] [Review][Defer] `IsRelativeCodeHref` is unscoped to actual `[Source: …]` citations — it rewrites/strips ANY relative `<a href>` on the page (any file with an extension, not `.html`/`.md`), so a legitimate prose link to a non-code asset (e.g. an image or download under `_bmad-output`) that doesn't resolve gets silently de-linked to plain text. Deferred — reason: reviewed two fixes with the story owner and both are unsafe. (1) Requiring a leading `../` climb breaks the bare `[label](src/Foo.cs)` citation shape, which is genuinely used in the live corpus (e.g. `10-1-insights-navigation-and-structure-page-retirement.md:17`) as well as `CodeReferenceLinkifierTests.EscapesEmittedHref`. (2) Gating strip-on-failure by a real filesystem existence check breaks in-portal mode's deliberate disk-free design (`CodeReferenceLinkifierTests` docstring: "In-portal cases are disk-free") and the `UnresolvedHref_DegradesToPlainText` test. A real fix needs citation context threaded from the markdown-parse stage through to HTML, not a rewrite of the rendered-HTML matcher — out of scope for this pass. [`src/SpecScribe/CodeReferenceLinkifier.cs:77-96,165-183`]
- [x] [Review][Defer] `BuildReferencedBy`'s fallback (`PathUtil.ToOutputRelative`) guesses a URL whenever a citing artifact isn't in `_referenceMap`, rather than omitting the entry. Attempted a "never guess, omit instead" patch, but reverted: it regressed `SiteGeneratorCodeCitationTests.CodePage_HasReferencedByBackToCitingArtifacts` — in a project with no `epics.md`, `_referenceMap` is never populated at all, and in that case the naive fallback formula is exactly correct for ordinary docs (matches `GenerateOneInternal`'s own output-path computation), so "omit" throws away legitimate back-links. The guess is only unreliable for entities with a non-naive output path (drafted stories, which are always correctly overridden in `_referenceMap` when present, so unaffected) — deferred as-is, no unambiguous safe fix within this pass. [`src/SpecScribe/SiteGenerator.cs:1523-1539`]
- [x] [Review][Patch] `DiscoverCodeReferences(files)` only scans the `_bmad-output` source corpus, never `EnumerateAdrFiles()` — but `GenerateAdrsInternal` still runs every ADR page through `CodeReferenceLinkifier.Linkify`. A code citation inside an ADR for a file not *also* cited by a `_bmad-output` doc silently never resolved. Fixed: `DiscoverCodeReferences` now shares one scan body (`ScanArtifact`) across both the `_bmad-output` corpus and `EnumerateAdrFiles()`, so ADR-only code citations resolve too. [`src/SpecScribe/SiteGenerator.cs:1233-1283`]
- [x] [Review][Patch] `CodeFileTemplater.RenderPlaceholder` (oversized/binary cited files) never receives the computed `referencedBy` list — only the normal `RenderPage` branch does — so a cited file that happens to be too large or binary silently loses the "Referenced by" block AC #2 otherwise guarantees. Already fixed by a later story landed on `main` before this review ran — both `RenderPlaceholder` call sites now pass `referencedBy`; verified in the current tree. [`src/SpecScribe/SiteGenerator.cs:1451,1464`]
- [x] [Review][Patch] `ArtifactTitlePattern` matches the first `# …`-shaped line anywhere in the raw markdown, including inside a fenced code block or YAML frontmatter comment — can produce a wrong/misleading title in a "Referenced by" entry. Fixed: `ExtractArtifactTitle` now scans line-by-line, skipping a leading `---`/`---` frontmatter block and any `` ``` ``/`~~~` fenced regions, before matching the first real ATX heading. [`src/SpecScribe/SiteGenerator.cs:1346-1378`]
- [x] [Review][Defer] Href/inline citation resolution doesn't percent-decode paths and requires an extension on the last path segment, so citations to extensionless files (`Dockerfile`, `Makefile`) or filenames with encoded spaces never resolve via the href form — deferred, pre-existing completeness gap, not a regression and not required by AC #1/#2. [`src/SpecScribe/CodeReferenceLinkifier.cs:165-183`]
