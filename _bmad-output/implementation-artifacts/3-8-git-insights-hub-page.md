...
**Follow-up polish (owner feedback #2):**
- **Whole-row click selects.** The drill-down trigger moved off the file name onto the whole row via the stretched-link pattern: an invisible `.gi-row-link` anchor stretches its `::after` over the row (the row is `position: relative`), so a click anywhere on the row reveals that file's contributors — still pure-CSS `:target`, no JS. The file name is now reserved for eventual navigation to the file's own page (7.1/7.4): when a resolver gives it a target it renders as an `a.gi-file-name` that rides above the row overlay (`z-index`) and navigates; until then it is a plain `<span>` the overlay's click selects.
- **Stable column widths.** The master-detail grid changed from `minmax(0, 3fr) minmax(0, 2fr)` to `minmax(0, 1fr) 20rem` — a fixed-width detail column. The fractional split let the revealed panel's changing content nudge both tracks on every selection (the jarring width shift); a fixed rem width is content-independent, so selecting rows now changes only the panel's contents, not the layout. Verified in-browser: master 579px / detail 320px stay identical across the default state and every selection at 1280px.
...
- 2026-07-09: Follow-up polish — whole-row click now selects (stretched `.gi-row-link` overlay; file name reserved for future file-page navigation), and the detail column is now a fixed `20rem` width so revealing/swapping panels no longer reflows the columns. 559 tests green; verified stable widths + row-click on desktop and mobile.
