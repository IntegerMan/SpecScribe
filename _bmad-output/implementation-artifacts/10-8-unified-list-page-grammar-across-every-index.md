...
Code review 2026-07-20 (parallel adversarial: Blind Hunter + Edge Case Hunter + Acceptance Auditor). Scope: story-10.8 code isolated to its originating commit `f0f30bd` (baseline→HEAD was polluted by 10.9/10.10/10.11 + deferred-debt bundles); all findings re-verified live against HEAD. 4 dismissed as noise (incl. the multi-word slug bug, already fixed at HEAD to first-word slugging).

- [x] [Review][Patch] AC #1/#2 — rewire the timeline literally onto `ListRow.Render`; keep the rich cards as conforming (owner decisions 2026-07-20, revised) — first ruling was "rewire all families," but on reading the code the epic/story/requirement families are rich multi-block **cards** (`AppendEpicCard` = h2+goal+meta+pending-note+link; `AppendStoryCard` = header+title+status/task pair; `AppendRequirementCard`/`AppendCoverageRow` = head+text+"Delivered by" epic list), not single rows — a literal `ListRow.Render` rewrite would destroy their layout (same reason Design Direction #5 exempts the tables). **Revised owner ruling: rewire the timeline only** (a genuine row family) and keep the cards conforming via their own grammar. Timeline day-rows now call `ListRow.Render` (date → summary slot, commit/artifact summary → right-side metadata), sharing the card shell with a NEUTRAL accent (see accent patch below). Subsumes the timeline-summary right-shift patch. Touches `TimelineTemplater.cs` + its pinned tests + golden regen.
- [x] [Review][Patch] Drive the ADR (and shared list-row) left accent bar from real status (owner decision 2026-07-20) [src/SpecScribe/SiteGenerator.cs:871; specscribe.css:5946] — `.list-row` currently inherits `.followup-row`'s fixed `border-left: var(--status-review)` and the ADR call never sets `resolved`, so every record shows a review-stage bar. **Owner ruled: drive the accent from `adr.Status`** — map the ADR status vocabulary (Accepted/Superseded/Deprecated/…) onto the status tokens so the accent reflects each record (needs a status→token accent path that doesn't disturb the follow-up family's own `--status-review` default).
- [x] [Review][Patch] Timeline commit summary is pushed to the far-right edge of its row [src/SpecScribe/TimelineTemplater.cs:86-91] — nesting `timeline-summary` inside `.list-row-meta` (`margin-left:auto`) with `.list-row-scan { width:100% }` detaches the summary from its date; previously they were adjacent flex siblings. Visual regression on `timeline.html`; byte-golden + `Assert.Contains` class checks don't catch layout. Fix: keep timeline summary adjacent to the date (don't route it through the `margin-left:auto` meta cluster).
- [x] [Review][Patch] Orphaned/duplicated `<summary>` doc comment in StatusStyles [src/SpecScribe/StatusStyles.cs:198-227] — `FreeTextBadge`'s doc block was inserted after `ForSprint`'s original `<summary>` but before the `ForSprint` method, so `FreeTextBadge` now carries two stacked `<summary>` elements and `ForSprint` (line 227) has lost its documentation entirely (CS1571 risk if XML docs are enabled). Fix: move the `ForSprint` summary block back to directly above `public static string ForSprint`.
- [x] [Review][Patch] Trailing punctuation on an ADR status defeats canonical routing [src/SpecScribe/StatusStyles.cs:214,375] — `Normalize` trims/lowercases only (unlike `ForStatus`, which strips `.!?:,`), so an ADR `**Status:** Done.` misses the `ForSprint` switch → grey "pending/Unknown" pill instead of green, and the fallback slug becomes `status-done.` (literal dot, matches no CSS). Newly reachable via the ADR surface 10.8 added. Low likelihood but a small hardening; pre-existing root in `Normalize`. Fix: strip trailing punctuation before matching, mirroring `ForStatus`.
- [x] [Review][Patch] `ListRow.Render` always emits an empty `.list-row-meta` wrapper [src/SpecScribe/ListRow.cs:47-60] — when badge, chips, and primary link are all absent, the `<div class="list-row-meta">…</div>` is still opened/closed unconditionally (empty-but-present, mild NFR8 smell). Latent — no current caller hits it, but the primitive is built for future reuse. Fix: guard the meta div on `badge || chips.Any() || primaryLink`.

### Resolution (2026-07-20)

All 6 patches applied on `main` (commit `efe6e34`, "fix: Story 10.8 code-review remediation"). Foundational change: decoupled `.list-row`'s left accent from `.followup-row`'s fixed `--status-review` to a neutral `--list-row-accent` default, with opt-in `.list-row-accent-{done,pending,deferred}` modifiers — this both fixes the ADR accent finding (driven off `StatusStyles.AdrAccentToken`) and lets the timeline share the card shell without a false review-stage bar. Timeline day-rows now render literally through `ListRow.Render` (date → summary slot, tally → right metadata), the bespoke `.timeline-row` CSS was retired, and the right-shift is now the intentional, consistent list grammar. `FreeTextBadge` gained trailing-punctuation tolerance, the orphaned `ForSprint` doc comment was restored, and the empty `.list-row-meta` wrapper is now guarded. Tests updated across `StatusStylesTests`/`StylesheetTests`/`TimelineTemplaterTests`/`SiteGeneratorAdrToleranceTests`; **full suite 1819 green** (golden fingerprint unchanged — the golden fixture exercises neither a git-history timeline nor the synthesized-ADR-landing branch). 4 findings dismissed as noise (incl. the multi-word slug bug already fixed at HEAD).

**Caveats:** (1) the commit was made on `main` per owner direction and also swept in a concurrent session's unrelated in-flight working-tree changes (About-SDD, nav, Story 10.10) present at commit time — an earlier concurrent `git reset --hard` on the shared tree had transiently wiped uncommitted work this session. (2) The timeline is now a subtle-card layout (visual redesign); its markup is pinned by automated tests but a browser eyeball of `timeline.html` is a recommended soft follow-up, as the in-app preview tooling was unavailable this session.
