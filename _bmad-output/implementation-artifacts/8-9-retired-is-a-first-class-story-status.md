...
Code review (2026-07-28), scoped to this story's own File List, diffed from baseline `b696485` to `HEAD`. Three parallel layers (Blind Hunter, Edge Case Hunter, Acceptance Auditor) produced 23 raw findings; after dedup and reading the actual current source at each location, 16 survive triage: 1 decision-needed, 5 patch, 5 defer, 5 dismissed as noise/false-positive.

- [ ] [Review][Decision] Requirement rollup misreports a requirement covered solely by an all-retired epic as "Planned" — `RequirementsParser.DeriveStatus` (src/SpecScribe/RequirementsParser.cs:341-359) has no branch for `classes.All(c => c == "retired")` and falls through to `Planned`, implying not-yet-started rather than abandoned. This state is newly reachable only because AC #4 lets `StatusStyles.ForEpic` return `"retired"` for the first time. Needs an owner call: is "Planned" an acceptable simplification (matching the existing "coarse epic-level signal, not a finer per-requirement claim" precedent), or does `RequirementStatus` need a distinct member for a requirement whose only covering epic(s) are all retired?
- [ ] [Review][Patch] Epic "Up Next" card falsely reads "Every story in this epic is done." for an all-retired epic [src/SpecScribe/EpicsViewBuilder.cs:521-524]
- [ ] [Review][Patch] `IconsTests.ForStatus_EveryKnownCssClassReturnsAGlyph` missing the `retired` row named in the story's own ranked Test Gates list (#6) [tests/SpecScribe.Tests/IconsTests.cs:8-17]
- [ ] [Review][Patch] `SiteGeneratorOutlineTests.EpicStages_UseRetroGatedClassifier` not extended with an all-retired case, though the story's own Dev Notes name it as where AC #4's retro-gate carve-out gets pinned at the integration level [tests/SpecScribe.Tests/SiteGeneratorOutlineTests.cs:217]
- [ ] [Review][Patch] No test pins the retired+ready mixed-state epic combination for `StatusStyles.ForEpic` [tests/SpecScribe.Tests/StatusStylesTests.cs]
- [ ] [Review][Patch] No negative test proves `EpicsParser.RetirementKeyword` does NOT match the spaced "won't fix" form — the asymmetry both the doc comment and ADR 0025 describe in prose has no pinning test [tests/SpecScribe.Tests/EpicsParserTests.cs]
