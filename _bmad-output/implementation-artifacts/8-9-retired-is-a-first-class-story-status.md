...
Code review (2026-07-29), scoped to this story's own File List, diffed from baseline `b696485` to `HEAD`. Three parallel layers (Blind Hunter, Edge Case Hunter, Acceptance Auditor) produced 23 raw findings; after dedup and reading the actual current source at each location, 16 survive triage: 1 decision-needed, 5 patch, 5 defer, 5 dismissed as noise/false-positive.

- [x] [Review][Decision] **RESOLVED 2026-07-29** (owner: fix the parser, not BMad's outputs). Requirement rollup misreported a requirement covered solely by an all-retired epic as "Planned". Fixed by adding `RequirementStatus.Retired` as a 7th canonical tier — `RequirementsParser.DeriveStatus` now returns it when `classes.All(c => c == "retired")` (mixed retired+live coverage still reads Active/Ready/Planned as before; only a uniformly-retired covering set gets the new tier). Threaded through every consumer (no silent drops, Trap-1 discipline): `StatusStyles.ForRequirement`/`RequirementLabel` (new explicit arms, shares `--status-deferred` — no 7th CSS token), `ProjectCounts.RequirementSatisfaction` (new `Retired` field, `Total`, `Tiers`, `Readings`), the satisfaction bar/chips (new bracket + 5th chip), the requirements-flow Sankey (`FlowStates`/`FlowStateKey` gain a `retired` bucket, mirroring how `Unmapped` already splits from `pending`; new `.req-flow-state.retired`/`.req-flow-ribbon.retired`/`.seg.retired` CSS rules), the dashboard sub-line, and the traceability page's ranking text — the latter two (`Charts.TraceabilityChips`, `TraceabilityTemplater`) deliberately fold Retired into "Covered" rather than adding a 4th chip, preserving Story 21.1's owner-locked 3-state matrix shape (a retired-covered requirement still names a real delivering epic). New tests: `RequirementsAndProgressTests.DeriveStatus_AllCoveringEpicsRetired_IsRetired_NotPlanned`, `StatusStylesTests` (3 new facts), `ProjectCountsTests.Build_RequirementSatisfaction_IncludesRetiredInTotalAndTiers`, `ChartsTests.RequirementFlowConservation_RetiredIsItsOwnBucket_SeparateFromDeferred`, `ChartsTraceabilityTests.TraceabilityLegend_RetiredFoldsIntoCovered_NotDeferred`. `dotnet build`/targeted `dotnet test` (318 passed) both clean at the time of this fix. ⚠ A concurrent session's mid-write `HierarchyExplorer.Projectors.cs` (missing `VariantTitle`) left the FULL solution non-compiling at hand-off — unrelated to this change (not in this fix's file list, confirmed via `git diff --stat`), per CLAUDE.md § Concurrent work. The golden fingerprint was not re-measured here since the tree can't currently build end-to-end; whoever next regenerates it must account for this fix's rendered changes (new Retired chip/bracket/Sankey bucket on `requirements.html`, the dashboard, and `traceability.html`) as an additional mover.
- [x] [Review][Patch] **FIXED.** Epic "Up Next" card falsely read "Every story in this epic is done." for an all-retired epic. `AppendUpNextCard` now branches on `StatusStyles.ForEpic(epic) == "retired"` and prints "Every story in this epic was retired." instead [src/SpecScribe/EpicsViewBuilder.cs:521-532]
- [x] [Review][Patch] **FIXED.** `IconsTests.ForStatus_EveryKnownCssClassReturnsAGlyph` gained the `retired` row named in the story's own ranked Test Gates list (#6) [tests/SpecScribe.Tests/IconsTests.cs:16]
- [x] [Review][Patch] **FIXED.** `SiteGeneratorOutlineTests` gained a 4th epic (Epic 4, sole story `Status: retired`) so `EpicStages_UseRetroGatedClassifier` now pins the all-retired retro-gate carve-out at the integration level, not just the `StatusStylesTests` unit level. Two pre-existing fixture-wide assertions (`Outline_MapsEveryEpicAndStory_OneToOne_InCoreOrder`'s epic-number array, `Summary_CountsStoriesByStage_CoreSide`'s `Total`) updated additively to include the new epic/story — no existing epic 0-2 assertion touched [tests/SpecScribe.Tests/SiteGeneratorOutlineTests.cs]
- [x] [Review][Patch] **NO CHANGE NEEDED** — re-checked against the actual test file before adding a duplicate: `StatusStylesTests.ForEpic_DoneOrRetiredReadsDone_AllRetiredReadsRetired` (line 482) already pins `retired` + `ready-for-dev` → `"ready"`. The original finding was itself a false positive.
- [x] [Review][Patch] **FIXED.** Added `EpicsParserTests.Parse_RetirementKeyword_DoesNotMatchTheSpacedWontFixForm_UnlikeForStatus` — a comment containing spaced `won't fix` now has a pinning test proving it does NOT divert (0 `RetiredNoticesHtml`), guarding the deliberate asymmetry a future "alignment" pass could otherwise silently regress [tests/SpecScribe.Tests/EpicsParserTests.cs]
- [x] [Review][Defer] `BmadCommands.ForStory`/`ForEpic` next-step pickers suggest re-drafting a retired story or re-planning an all-retired epic on their own "Next Steps" panels — pre-existing (file untouched by this diff, matches on raw status string not `StatusStyles`' classification), directly contradicts this story's stated goal, recommend a fast-follow story [src/SpecScribe/BmadCommands.cs:478-533, 535-610] — deferred, pre-existing
- [x] [Review][Defer] `StatusStyles.IsRetirementWord` strips hyphens across the WHOLE normalized string before matching, so a hyphen-mangled variant like `de-precated` misclassifies as retired — low real-world likelihood [src/SpecScribe/StatusStyles.cs:88-89] — deferred, pre-existing pattern / low likelihood
- [x] [Review][Defer] Completion Notes' `check:ir-content` claim ("4 new rules, 865 total") doesn't match the actual diff (+11 rules, 873 total) — 7 of the 11 are unrelated to `retired` and belong to a concurrent change; recommend correcting the Debug Log Reference for accuracy [web/assets/ir-content.manifest.json] — deferred, record-accuracy only
- [x] [Review][Defer] ADR 0025 left in `Proposed` with no ratification step — consistent with the existing pattern for ADRs 0021/0022, a repo-wide process gap not specific to this story [docs/adrs/0025-retired-is-a-terminal-story-stage-in-both-classifiers.md] — deferred, pre-existing
- [x] [Review][Defer] Trap 7's quick-dev-doc corpus check (frontmatter words newly diverting under the widened vocabulary) isn't recorded as done in Completion Notes — likely a non-issue but unverified; recommend a quick grep before closing [_bmad-output/implementation-artifacts/8-9-retired-is-a-first-class-story-status.md] — deferred, self-verification gap only

**Dismissed as noise/false-positive (5):** the golden-fingerprint "contradiction" (actually a fully documented, correctly-executed 3-stage regeneration audit trail — CLAUDE.md's process working as intended); `epic-8-retrospective: done` beside `epic-8: in-progress` (exactly what AC #9 required — an explicit stated reason via inline comment); the `EpicsTemplater.cs:173/280` blast-radius entry (confirmed stale — that file no longer emits `.story-card` markup, only a `StatusStage` view-model value consumed by the one shared renderer that IS patched); an unrelated Story 22.5 sequencing note bundled into the `epics.md` diff (expected shared-file noise from concurrent sessions per CLAUDE.md); AC #7's JS-on narrative not explicitly naming the sunburst wedge (a Completion Notes completeness nit, not a code issue).

**Review closed 2026-07-29, status → done.** After the concurrent `CodeMapTemplater.cs`/`HierarchyExplorer.Projectors.cs` sessions settled, re-verified clean: `dotnet build` — 0 errors; full serial suite (`xUnit.MaxParallelThreads=1`) — **2811 passed / 0 failed / 3 skipped** (the pre-existing symlink-privilege skip family); `npm run check:ir-content` — OK, 876 rules + 4 keyframes in sync (re-extracted the 3 new `retired` selectors this review's fix added); `npm run measure:parity` — **golden=IR 191/191** (the Nuxt-column mismatches in that report are pre-existing migration gaps the tool itself attributes to a different story, not this one). Generated output inspected directly: `requirements.html` carries the new 5th "Retired" satisfaction chip (count 0 today — no epic in the live repo is yet all-retired), `traceability.html`'s strip correctly stays 3-chip (Retired folds into Covered), the Sankey correctly omits the zero-count `retired` node, and all three new CSS rules resolve to `--status-deferred` as designed. ⚠ **Not done this session:** the pixel-level live-browser look ADR 0013 §Decision 3 / CLAUDE.md § Verification calls for — the browser tool's safety classifier was intermittently unavailable for the remainder of this session. Owner decision: close the story on the strength of the suite + generated-artifact verification above rather than block on tool availability; a follow-up live-browser pass on `requirements.html`/`traceability.html`/the dashboard is owed before the NEXT visual change to this area, not deferred indefinitely.
...
| Date | Note |
|---|---|
| 2026-07-29 | **Code review complete → `done`.** Three adversarial layers (Blind Hunter · Edge Case Hunter · Acceptance Auditor) over this story's own File List (baseline `b696485`→`HEAD`, sibling stories excluded per CLAUDE.md § Scoping a code review): 23 raw findings → 1 decision-needed, 5 patch, 5 defer, 5 dismissed. **The decision-needed one was a real defect this story created:** AC #4 made `StatusStyles.ForEpic` able to return `retired` for the first time, and `RequirementsParser.DeriveStatus` had no arm for it — a requirement covered solely by an all-retired epic fell through to `Planned`, reporting an abandoned plan as merely not-yet-started. Owner chose to fix the parser rather than work around it, so `RequirementStatus` gained a **7th tier, `Retired`**, threaded through every consumer with explicit arms (no `_ =>` silent-default, the Trap 1 discipline this story itself established): `ForRequirement`/`RequirementLabel`, `ProjectCounts.RequirementSatisfaction` (`Retired` field + `Total`/`Tiers`/`Readings`), the satisfaction bar + a 5th chip, the requirements-flow Sankey (`FlowStates`/`FlowStateKey` gain a `retired` bucket exactly as `Unmapped` already splits from `pending`), the dashboard sub-line, and the traceability ranking — with `TraceabilityChips`/`TraceabilityTemplater` deliberately folding Retired into **Covered** to hold Story 21.1's owner-locked 3-state matrix. Reuses `--status-deferred`; **still no 7th `--status-*` token** (D2/Story 9.3 hold). 4 of 5 patches applied (`EpicsViewBuilder` no longer tells an all-retired epic "Every story in this epic is done"; `IconsTests` + `SiteGeneratorOutlineTests` + `EpicsParserTests` gained the missing `retired`/all-retired/negative-`won't fix` gates); the 5th was **re-checked against the file and found already covered** at `StatusStylesTests:482` — a false positive, not patched. Verified after the concurrent `CodeMapTemplater`/`HierarchyExplorer.Projectors` sessions settled: build 0 errors, **serial suite 2811 passed / 0 failed / 3 skipped**, `check:ir-content` OK (876 rules, re-extracted for 3 new selectors), `measure:parity` **golden=IR 191/191**. ⚠ Pixel-level live-browser verification NOT performed — the browser tool was unavailable all session; owner closed on suite + generated-artifact evidence, with a live pass owed before the next visual change to these surfaces. 5 deferred items in `deferred-work.md`, the load-bearing one being that **`BmadCommands` still offers a retired story as next work** (pre-existing, `BmadCommands.cs` untouched by this story, but it contradicts this story's goal — worth a fast-follow). |
