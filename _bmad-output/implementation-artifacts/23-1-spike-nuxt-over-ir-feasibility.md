...
- **CSP evidence** was gathered live in a real browser against `scripts/csp-probe.mjs`, which replays `WebviewRenderAdapter.cs:102`'s policy string verbatim and substitutes `__NONCE__` per request. Verdicts came from network-request outcomes + `__vue_app__`/SVG counts, not from reasoning about the spec.
...
- `spike/nuxt-ir/package.json`, `spike/nuxt-ir/package-lock.json` (gitignored)
