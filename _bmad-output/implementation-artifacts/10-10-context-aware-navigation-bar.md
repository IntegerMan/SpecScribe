...
- `tests/SpecScribe.Tests/HtmlRenderAdapterTests.cs` (UPDATE — More-disclosure overflow tests, added post-review)

## Post-Review Follow-up: Overflow "More" Disclosure

Owner feedback after review (real-repo screenshot of Epic 10, 11 stories): the local-context band's pills wrapped across multiple lines for epics with many stories, eating vertical header space. Fixed by capping inline pills at `HtmlRenderAdapter.LocalContextInlineLimit` (8) and collapsing the remainder into a "More (N)" disclosure — reusing the SAME `.key-view-group`/`.key-view-trigger`/`.key-view-panel` markup, CSS (hover/focus-within), and `specscribe.js` click handler the generic quick-links band already uses (that handler is class-selector-generic, so no new JS was needed). The active item is guaranteed visible without opening the panel: if it falls in the overflow window it's pinned into the inline row instead of being buried (verified live on `story-10-10.html`, item #10 of 11). Added 3 new `HtmlRenderAdapterTests` (at-limit no-disclosure, beyond-limit collapses correctly, active-item-pinned). Golden fingerprint required a third regeneration — this repo's background auto-committer was actively landing unrelated concurrent commits on `main` during verification (the same shared-main gotcha already noted for Story 10.9), causing the hash to be transiently unstable; three consecutive back-to-back runs against the settled state agreed and were locked in. Full suite: 1704 tests green (13 new total this story).

## Post-Review Follow-up: Long-Label Truncation + Dark-Bar Auto-Open

Second round of owner feedback (real-repo screenshot of an Epic 9 follow-up detail page):

1. **Excessively long pill labels.** Follow-up items carry full-sentence summaries, so even at the 8-pill cap the band grew tall (each pill wrapped or stretched most of the bar's width). Fixed with `HtmlRenderAdapter.TruncateNavLabel`: inline pills ellipsise past `LocalContextPillLabelMax` (28 chars), overflow-panel rows past `LocalContextPanelLabelMax` (44), and the full untruncated text always rides a native `title` tooltip (chosen over the codebase's `data-tooltip` CSS-`::after` system, which clips inside the nav's overflow — see [tooltip-clipping-use-ss-tooltip-node]; a plain `title` never clips and needs no JS). Added `white-space: nowrap` to `.local-context-pill` so a pill never wraps mid-label. An untruncated short label gets no redundant tooltip.
2. **Dark-bar journey dropdowns rendered `open` on load.** Story 10.1 forced the active group's `<details>` open (`openAttr = hasActive ? " open"`), so navigating to e.g. an Insights page showed the Insights menu sprung open — and because the `open` attribute is baked into the static HTML, it stayed open through a refresh, reading as a stuck menu covering the page. Removed the auto-open: the active group keeps its `has-active` class (a summary-color highlight, so the reader still sees which section they're in) but the disclosure stays collapsed, opening only on hover/focus/click like every other group.

Added 2 new tests (long-label truncation with tooltip + short-label untouched; dark-bar group marked-active-but-not-open) and updated the former `RenderNavBar_OpensActiveGroupAndMarksActiveLeaf` assertion to the new no-auto-open contract. Golden fingerprint regenerated (the removed ` open` attrs shift every fixture page whose active leaf lives in a multi-child group; label truncation is a no-op on this fixture's short Story-N.M/FR/NFR labels). The background auto-committer landed several more unrelated commits on `main` mid-verification, so the hash moved a few times before settling — the value locked in was confirmed against the final tree. Full suite: 1735 tests green (15 new total this story). Note: the transient full-run failures observed during verification were an environmental git-CLI-availability flake under parallel load (each failing test passed in isolation with git present), not a regression.

Not changed (confirmed by-design): pages at the top of their own hierarchy — the Requirements/Epics indexes, `action-items.html`/`deferred-work.html` roots, and the `follow-ups/group-*.html` pages — still show the generic Delivery/Insights/Follow-ups/Project quick-links band, because there is no meaningful "local family" above them. That generic band already uses the same `.key-view-group` More/dropdown pattern, so it is not the tall-bar case.
...
- 2026-07-19: Owner-reported overflow bug fixed — epics with many stories wrapped the local-context band across several lines. Pills now cap at 8 inline (`HtmlRenderAdapter.LocalContextInlineLimit`), with the remainder collapsing into a "More (N)" disclosure that reuses the existing `.key-view-group` pattern verbatim (no new CSS/JS). Active item pinned into view when it would otherwise land in the overflow window. Verified live on `epics/epic-10.html` (11 stories → 8 pills + "More (3)") and `epics/story-10-10.html` (active item #10 pinned, "More (2)" for the other two). 3 new tests; golden fingerprint regenerated (required 3 attempts to outlast concurrent `main` writes from this session's background auto-committer). Full suite green (1704 tests).
- 2026-07-19: Two more owner-reported issues fixed — (1) long pill labels ellipsised to 28 chars inline / 44 in the overflow panel with the full text on a native `title` tooltip, plus `white-space: nowrap`, so verbose follow-up/ADR labels no longer stretch or wrap the band; (2) dark-bar journey dropdowns (Insights, etc.) no longer render `open` on page load — the active group keeps its `has-active` summary highlight but stays collapsed (it previously sprang open and stayed open through a refresh). Verified live: the Epic 3 follow-up detail band shows 8 truncated pills + "More (21)" with tooltips, and `git-insights.html` emits `has-active` with no `open`. 2 new tests + 1 updated. Golden fingerprint regenerated. Full suite green (1735 tests).
