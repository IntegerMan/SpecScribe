...
- [x] [Review][Decision] Machine summary line prints unconditionally, even in a fully interactive human terminal — `PrintMachineSummary(counts, total)` (ConsoleUi.cs:216) runs outside any `IsInteractive` gate, so every interactive run now also emits the raw `SpecScribe: generated=... elapsed_ms=...` grep line into the polished colored table output. AC #3's Given/When text scopes the machine-line requirement to the non-interactive case; the story's own Open Questions addressed suppressing the *pretty table* in CI (Q2) but never asked whether the machine line should be interactive-gated too. **RESOLVED (owner, 2026-07-25): keep current behavior** — the line stays always-on, no code change.
- [x] [Review][Decision] The "no color" half of AC #3 / DESIGN "CLI Output" ("plain text, no spinner, no color") is not independently enforced — `PrintLogo` (colored `FigletText` + link markup), the per-error `[red]x[/]` lines, the `[grey]Initial build...[/]` prose line, and `PrintOutputLink`'s `[link=...]` markup all still route through `AnsiConsole` with color tags unconditionally in both modes; only the rounded table and live progress are gated on `Capabilities.Interactive`. Color suppression relies entirely on Spectre's own `Capabilities.Ansi` auto-detection, which is a different capability than the one this diff branches on everywhere else — and several real CI platforms (e.g. GitHub Actions, GitLab CI) report `Ansi=true`/`Interactive=false`, so colored escape codes could still land in piped CI output. The story explicitly chose NOT to rely on Spectre's implicit degradation for the progress bar ("do not rely on that implicitly — make the branch explicit") but did rely on it here, undisclosed. **RESOLVED (owner, 2026-07-25): keep relying on Spectre's Ansi auto-detection** — no code change.
- [x] [Review][Patch] `generate`/`watch`/`Program.cs`'s fatal-error handlers return bare literal exit codes instead of the `ExitCodes` constants this story introduced [src/SpecScribe/Program.cs:57,62,67; src/SpecScribe/Commands.cs:461] — fixed: all four now return `ExitCodes.Failure`/`ExitCodes.Success`.
- [x] [Review][Patch] `ConsoleUi.IsInteractive`'s doc comment claims it's reused by `Program.cs` and `InteractiveCommand`, but both still call `AnsiConsole.Profile.Capabilities.Interactive` directly — the TTY-detection check is duplicated three times with a comment claiming otherwise [src/SpecScribe/ConsoleUi.cs:152-155; src/SpecScribe/Program.cs:52; src/SpecScribe/Commands.cs:482] — fixed: `IsInteractive` made `internal` and reused from `Program.cs` and `InteractiveCommand`; doc comment corrected.
- [x] [Review][Patch] `GenerationSummary.Count`'s tallying switch has no `default` arm — a future `GenerationOutcome` value would be silently excluded from every count, the machine summary line, and the exit-code decision with no compiler warning [src/SpecScribe/GenerationSummary.cs:42-58] — fixed: `default` arm now throws `ArgumentOutOfRangeException`.
- [x] [Review][Patch] `PrintMachineSummary`'s raw `Console.Out.WriteLine` can throw `IOException` when the downstream pipe closes early (e.g. `specscribe generate | head`); the top-level `Program.cs` catch turns that into a spurious "Fatal error" + exit 1 even though generation itself succeeded, undermining the CI exit-code contract this story introduces. `Program.cs:6-13` already has the identical try/catch-IOException pattern for the same class of problem [src/SpecScribe/ConsoleUi.cs:231-232] — fixed: wrapped in try/catch `IOException`.
- [x] [Review][Patch] `CliFeedbackTests.GenerationRun_SurfacesTheExitCodeGenerateCommandReturns`'s name promises integration coverage of `GenerateCommand.Execute` that it doesn't provide — it only re-exercises `GenerationRun`'s own trivial computed properties, already covered by the `GenerationSummary.ExitCode` tests immediately above it [tests/SpecScribe.Tests/CliFeedbackTests.cs] — fixed: renamed to `GenerationRun_ComputesExitCodeAndHadErrorsFromCounts`, dropped the unused `SiteGenerator`/`ForgeOptions.Resolve` ceremony in favor of `null!` for the unasserted `Generator` field.
- [x] [Review][Patch] `GenerationSummary.FormatLine(IEnumerable<GenerationEvent>, TimeSpan)` and `ExitCode(IEnumerable<GenerationEvent>)` are public API surface with zero production callers — only the `GenerationCounts`-based overloads are wired into `ConsoleUi`/`Commands.cs`; the events-based overloads exist solely to be exercised by tests [src/SpecScribe/GenerationSummary.cs:72,82] — fixed: overloads deleted; tests now compose `GenerationSummary.Count(events)` with the `GenerationCounts`-based overloads via local test helpers.
- [x] [Review][Defer] AC #3's interactive/non-interactive branching (`RunWithProgress`'s `if (!IsInteractive)`, `PrintInitialSummary`'s `IsInteractive` gates) has zero automated coverage — no `Spectre.Console.Testing`/`TestConsole` harness exists in the test project. Deferred, pre-existing test-infra gap, and already honestly disclosed by the story's own "Honest limits on verification" section.
- [x] [Review][Defer] Nothing drives `GenerateCommand.Execute`/`RunGeneration`'s actual return-wiring end-to-end — every exit-code test calls `GenerationSummary.ExitCode` directly or hand-constructs a `GenerationRun`. Deferred, pre-existing: closing this needs a new `CommandContext`/Spectre.Console.Cli test harness, not a small unambiguous patch.
