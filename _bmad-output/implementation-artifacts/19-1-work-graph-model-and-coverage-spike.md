...
**Surface reach for 19.2 (confirming the story's table):** **HTML + SPA only.** HTML is the primary host; SPA parity comes free via the shared `<main id="main-content">` body seam + `RenderParity` (same as every other standalone page). ~~**Webview and CLI are non-goals** — no reason found to promote the graph into the webview's dashboard/epics-only surface; if a true cross-surface webview gate is ever wanted it is a *separate* decision (Epic 6 Action #3), not an expansion of 19.1 or 19.2's MVP.~~ **SUPERSEDED 2026-07-23** (19.2 code review): the epic/story "Overview | Work Graph" tab rides Story 6.4's shared body-render seam (`page.BodyHtml`, reused verbatim by every surface for parity), so it reached webview automatically once added to the epic/story body renderers — a true webview-specific bypass would mean building a second, separate page per surface, breaking that seam's single-body invariant. Given that cost, the owner promoted graph-in-webview to intentional rather than adding a bypass. CLI remains a non-goal (unaffected — CLI never rendered epic/story bodies at all).
