...
_Code review 2026-07-11 (parallel adversarial review: Blind Hunter + Edge Case Hunter + Acceptance Auditor; 701/701 tests independently re-confirmed green in Debug). Original triage: 2 decision-needed, 8 patch, 4 deferred, 5 dismissed. Both decisions resolved by the owner (1 → deferred to 6.5; 1 → dismissed). Net: 8 patch, 5 deferred, 6 dismissed._

_**Resolution 2026-07-11:** all 8 patches applied and verified — 704/704 tests green (Debug; +3 new: story-page + placeholder chrome parity, and a source-untouched read-only test at the writing seam), TypeScript typecheck clean, golden byte-identity gate still holds (webview-only CSP change). Patch summary: `toolPath` → `machine-overridable` scope; stdout `setEncoding('utf8')`; disposed-panel guards on first-paint + refresh; error-path clears the singleton so reopen retries; CSP gains `base-uri 'none'; form-action 'none'`; entry-doc placeholder substitution now splices the content region out so page content can't be mangled or forge the nonce. The 5 deferred items are logged in [deferred-work.md](deferred-work.md)._

**Decision needed** — both resolved:

- Nav-toggle keyboard/focus affordances → **deferred to Story 6.5** (owner call, see Deferred below).
- CSP `img-src https:` → **dismissed, keep as-is** (owner call: remote markdown images should keep rendering; the beacon risk is accepted).

**Patch** (fix unambiguous):

- [x] [Review][Patch] `specscribe.toolPath` is declared with no `scope` (defaults to workspace-overridable) and the manifest declares no `untrustedWorkspaces` capability — a checked-in `.vscode/settings.json` can point it at an arbitrary binary that `spawn`s on "Open Status" → RCE from opening an untrusted repo. Add `"scope": "machine-overridable"` (the `git.path` pattern). [extension/package.json:28]
- [x] [Review][Patch] Renderer stdout is accumulated via per-chunk `d.toString()` with no `setEncoding` — a multibyte UTF-8 char (em-dash, pervasive in the templates) split across a ~64 KB chunk boundary decodes to `�`, corrupting visible content on the multi-MB payload. Add `proc.stdout.setEncoding('utf8')` (or collect Buffers + `Buffer.concat`). [extension/src/extension.ts:175]
- [x] [Review][Patch] First-paint and in-flight refresh touch `p.webview.html`/`p.title` with no disposed-guard — closing the panel during the ~3.5 s cold spawn throws an unhandled rejection (first paint) or a spurious "refresh failed" warning (refresh). Re-check panel/disposed before touching `p`. [extension/src/extension.ts:104-118]
- [x] [Review][Patch] On first-render failure the `panel` singleton is left set (catch sets `errorHtml` and returns) — re-running "Open Status" hits `if (panel) { panel.reveal(); return; }` and just re-reveals the dead error panel with no retry until the user manually closes it. Clear/dispose `panel` on the error path. [extension/src/extension.ts:96-100]
- [x] [Review][Patch] AC #4 parity runs `FindDivergences` on only 3 of 5 surfaces — story page and story placeholder get no fact-level parity assertion (only a weak `Contains`/`DoesNotContain` string check). Add chrome-parity coverage for a story page and a placeholder. [tests/SpecScribe.Tests/WebviewRenderAdapterTests.cs]
- [x] [Review][Patch] AC #6 "writes nothing" is tested only against the pure `RenderWebviewSurfaces` projection; the shipped `specscribe webview` command runs a full `GenerateAll()` (writes ~202 files to temp scratch) and is untested for read-only-ness. Add a test asserting the project tree and configured output are untouched after running the command. [tests/SpecScribe.Tests/SiteGeneratorWebviewTests.cs]
- [x] [Review][Patch] Webview CSP omits `base-uri` and `form-action` (neither inherits from `default-src`) — an injected `<base>` or `<form>` in swapped content can retarget relative loads / POST outward on a click. Add `base-uri 'none'; form-action 'none';`. [src/SpecScribe/WebviewRenderAdapter.cs:81]
- [x] [Review][Patch] `__NONCE__`/`__CSP_SOURCE__` substitution is a global `split/join` over the whole entry document including the content region — dashboard content containing the (publicly documented) tokens is mangled, and a forged `<script nonce="__NONCE__">` there would receive the live nonce and defeat the CSP. Use an unguessable per-render placeholder emitted in the bundle. _(Low reachability: only the dashboard document is substituted; navigation surfaces go via postMessage untouched.)_ [extension/src/extension.ts:147-150]

**Deferred** (real, not now):

- [x] [Review][Defer] Webview nav-toggle bridge drops the HTML surface's keyboard/focus affordances (Escape-to-close with focus return, focus-first-link-on-open, close-on-navigate) — bridge does only class-toggle + `aria-expanded`; no parity fact catches the NFR6/AC#2 gap. [src/SpecScribe/WebviewRenderAdapter.cs DocumentTemplate] — deferred to Story 6.5 (owner call: 6.5's host-aware theming + explicit helper actions owns the webview interaction chrome; porting keyboard affordances belongs with that pass)
- [x] [Review][Defer] Concurrent `specscribe webview` spawns for the same repo collide on the single per-repo scratch dir (two VS Code windows, or CLI + extension) → one render fails with an IOException. [src/SpecScribe/Commands.cs:78-93] — deferred, uncommon and retriable
- [x] [Review][Defer] A story `.md` deleted between `GenerateAll()` and `RenderWebviewSurfaces()` throws `FileNotFoundException` and aborts the whole bundle — no per-story placeholder fallback like the HTML render path has. [src/SpecScribe/SiteGenerator.cs:702-734] — deferred, sub-second same-process window, next refresh recovers
- [x] [Review][Defer] Renderer timeout uses default SIGTERM (may not reliably kill `dotnet` on Windows) and stdout buffers unbounded (no cap). [extension/src/extension.ts:168-171] — deferred, not fatal
- [x] [Review][Defer] Scratch key hashes `RepoRoot.ToUpperInvariant()` → two repos differing only in path case collide on a case-sensitive filesystem (Linux). [src/SpecScribe/Commands.cs:80-81] — deferred, negligible in practice
...
- 2026-07-11 — **Code review passed; Status → done.** Parallel adversarial review (Blind Hunter + Edge Case Hunter + Acceptance Auditor) found no rendering-correctness defect — golden byte-identity and AC #1–#3 held. 8 patches applied (see Review Findings), 2 owner decisions resolved (nav-toggle keyboard/focus fidelity → deferred to 6.5; `img-src https:` → kept), 5 items deferred to [deferred-work.md](deferred-work.md), 6 dismissed. Most significant fix: `specscribe.toolPath` given `machine-overridable` scope (a workspace `.vscode/settings.json` could otherwise point it at an arbitrary binary spawned on "Open Status"). Also: UTF-8 stream decoding of the renderer's stdout, disposed-panel guards, error-path retry, `base-uri`/`form-action` CSP tightening, and content-safe placeholder substitution. Suite 704/704 green (Debug), TypeScript typecheck clean. The one real remaining gap is unchanged: the manual F5 smoke in a real VS Code host (ADR 0005's documented Electron-paint/watcher-loop gap; checklist in `extension/README.md`).
