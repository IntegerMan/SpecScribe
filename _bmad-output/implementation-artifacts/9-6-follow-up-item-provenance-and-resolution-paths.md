---
baseline_commit: a39752ce2348534ac2229eca0b2a4d1f4fa855a8
---
...
- [x] **Task 2 — Action Items: group by source retro, age within (AC: #2, owner decision #3)**
  - [x] Replace the flat `foreach (item in openItems)` loop with **grouped rendering**: group `OpenActionItems` by `EpicNumber`, order groups by epic number ascending, preserve file order within each group (the age proxy), and place `EpicNumber == null` items in a trailing "Unattributed" group. Do the grouping as a **pure, deterministic** transform (stable ordering — no hash-set iteration, no wall-clock) so regeneration is byte-identical (NFR8).
  - [x] Each group gets a heading that doubles as provenance: e.g. `From the Epic N retrospective` linked to the retro page via `EpicRetroMap` (fall back to a plain, unlinked heading when no retro page exists — same gate the per-item "From Epic N retrospective →" link uses today). This can **replace** the now-redundant per-item "From Epic N retrospective →" link (the group header carries it) — decide and note which, keeping exactly one provenance affordance per item so the page doesn't double up.
  - [x] Keep the per-item status badge, the debt→deferred-work link, and the "Resolve with AI" command unchanged inside each grouped card.

- [x] **Task 3 — Action Items: cross-link near-duplicate items across retros (AC: #2, owner decision #2)**
  - [x] Add a **conservative, pure** near-duplicate detector over `OpenActionItems`: two items from **different** epics whose normalized action text is highly similar (e.g. normalize case/whitespace/punctuation, then require a high token-overlap ratio or a long shared normalized substring — pick a threshold that matches the Epic 1↔Epic 2 heatmap-debt pair without matching merely same-topic items). Keep it deterministic and side-effect-free.
  ...
  - [x] Guard against runaway matches: if the heuristic is too eager it will produce misleading cross-links, which is worse than none. Prefer **false negatives over false positives**; a unit test pins the canonical pair and pins that unrelated items do **not** cross-link.

- [x] **Task 4 — Deferred Work: parse the note into a structured model (AC: #1, #3, owner decision #1)**
  - [x] Add a new **pure parser** `src/SpecScribe/DeferredWorkParser.cs` (static, never-throws, no I/O) that turns the deferred-work note into a model. Suggested shape:
...
- Live generate to temp output confirmed: action-items grouped by epic-retro; visible Story/Epic/spec links; Resolve payload uncorrupted; deferred-work structured cards with Resolved badge+✓; home callout count intact.
- Epic 1↔Epic 2 heatmap-debt pair is `status: done` in live sprint-status — cross-link covered by FollowUpSurfacesTests fixtures instead.
- Owner sunburst visibility goal tracked as Story 9.7 (explicitly not absorbed); 9.6 groups by epic on the follow-ups pages as the preferred association shape.

### Completion Notes List

- Surface 1: ActionItemsTemplater groups by epic ascending (Unattributed trails), group-header provenance replaces per-item retro link, FollowUpRefs linkifies visible text only, conservative Jaccard near-dupe cross-links.
- Surface 2: DeferredWorkParser (pure, never-throws) + DeferredWorkTemplater structured cards; WriteDeferredWork post-pages overwrite at existing path with ApplyReferenceLinks; unstructured notes fall back to plain body.
- CSS: `.action-items-group` / `.action-item-cross` / `.deferred-*` with non-color-only resolved treatment (done badge + ✓ + opacity).
- Tests: DeferredWorkParserTests, FollowUpSurfacesTests, RetroTests payload guard, StylesheetTests; 1233 tests green. Golden fingerprint constant documented for 9.6 CSS delta (fixture has no action_items/deferred-work).

### File List

- src/SpecScribe/ActionItemsTemplater.cs
- src/SpecScribe/FollowUpRefs.cs (new)
- src/SpecScribe/DeferredWorkParser.cs (new)
- src/SpecScribe/DeferredWorkTemplater.cs (new)
- src/SpecScribe/SiteGenerator.cs
- src/SpecScribe/assets/specscribe.css
- tests/SpecScribe.Tests/DeferredWorkParserTests.cs (new)
- tests/SpecScribe.Tests/FollowUpSurfacesTests.cs (new)
...
- _bmad-output/implementation-artifacts/9-6-follow-up-item-provenance-and-resolution-paths.md
