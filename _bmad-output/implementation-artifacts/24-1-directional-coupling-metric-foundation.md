...
#### Review Resolution (2026-08-07)

All three decisions were taken by the owner and all nine patches applied in the same pass. Full suite green:
**2977 passed / 0 failed / 3 skipped**. Verified additionally against a real `--deep-git` render of this
repository (796 pages), not only against unit fixtures.

**Owner decisions**

- **D1 — ranking:** require `support > minSupport` for the visible top-N, and collapse mirror rows. Implemented in
  `BuildDirectedCoupling`: the strict set degrades to the full admitted set when nothing clears it, so a young
  repository gets a weak panel rather than an empty one, and a new `IsEcho` drops a reverse row only when support
  AND confidence tie (lift is mathematically symmetric, so those rows were identical in every rendered value).
  Genuine asymmetry still yields two rows. **Measured effect on this repo:** the visible Ranked Pairs rows moved
  from bare-floor support-2 pairs ordered by path to support **9, 9, 9, 8, 8, 8, 8, 7**.
- **D2 — confidence:** keep the formula, correct the docs. `CoupledFile`'s doc now states plainly that numerator
  and denominator come from different commit populations and why that is deliberate (a 200-file sweep is not
  evidence two files belong together, but it *is* evidence the file churns). The Dev Notes claim that the metric
  "inherits that filtering for free" was the inaccurate part and is superseded by this note.
- **D3 — AC #2:** closed in 24.1. `DeepGitPulse.Coupling` is now `IReadOnlyList<CoupledPair>`; the pair carries
  `CrossBoundary` and `Kind`, classified ONCE in `ParseNumstatLog` beside the identical computation feeding
  `DirectedCouple`. `Charts.CouplingGraph` consumes that record and reads both flags off it instead of calling
  `ClassifyCoupling` again, as does the hub's process-pair check. The graph's population and shared-commit ranking
  are unchanged, honouring Q1 — only the type widened. Cross-boundary rides the edge `<title>` as words; the ranked
  table remains the surface that marks it in visible text.

**Notable follow-on effects**

- `GitMetrics.Lift`'s base rate now divides by commits that actually contributed files, not `commits.Count`.
  `DeepGitPulse.AnalyzedCommits` deliberately still reports the full window — the two answer different questions,
  and that distinction is now documented at both sites rather than asserted backwards.
- `DeepGitPulse.MinSupport` records the floor actually used, so `SiteGenerator.BuildRelatedRelatedEdges` filters
  the (deliberately unfiltered) `CoChangePairs` map with the real value instead of a bare literal.
- `Charts.LiftNumber` is now the one place lift's rounding is decided; `LiftLabel` adds the HTML entity while the
  relationship graph's JSON `Detail` keeps a literal `×`. Confirmed in rendered output: lifts read `1.72`, `3.5`,
  `16.88`, `47.83` — values the old `"0.0"` format flattened.
- Test coverage added for all of Task 3, which had none: population, both directions, echo collapse, the
  evidence preference and its fallback, the floor reaching the hub view, the recorded floor, the lift base-rate
  exclusion, and the shared flags. The tautological floor assertion is joined by real behavioural ones.

**Still open:** the 8 deferred items below, logged in `deferred-work.md`. One of them (the boundary badge's
1.66:1 border and sub-10px column type) is a **medium** accessibility finding awaiting an owner design call — it
is the only medium-severity item not fixed in this pass.
...
| Date | Change |
| --- | --- |
| 2026-08-07 | Code review (Blind Hunter + Edge Case Hunter + Acceptance Auditor): 39 raw findings deduped to 25 — 3 decision-needed, 9 patch, 8 deferred, 5 dismissed. All three decisions taken by the owner and all nine patches applied in one pass. Ranking now prefers evidence over the bare floor and collapses echo rows (visible top-N moved from support-2 to support 7–9 on this repo); lift's base rate stopped counting file-less merge commits; the wrong-file lift caption, the stray `>= 2` literal, the two lift formatters, `Percent`'s false 100%/0% ends, and `Basename`'s missing normalization all fixed; AC #2 closed by widening `DeepGitPulse.Coupling` to `CoupledPair` so the graph reads shared `CrossBoundary`/`Kind` instead of re-deriving. Task 3 gained the test coverage it never had. Suite green 2977/0/3, verified against a real `--deep-git` render. Status → done; 8 deferrals logged, one medium (badge contrast / sub-10px type) awaiting an owner design call. |
