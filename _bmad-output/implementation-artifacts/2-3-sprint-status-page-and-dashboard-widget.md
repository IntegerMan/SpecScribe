...
- [x] [Review][Patch] Dashboard sprint widget (AC #2) had been lost in a later "Dashboard adjustments" regression — the home dashboard showed only a nav link. Fixed by restoring the intended design (from good-state commit `7aac29c`): `HtmlTemplater.AppendNowAndNext(sb, epicsModel, sprint)` — when sprint data is present, the **Now & Next panel BECOMES the sprint board** (`SprintTemplater.RenderBoard` capped at 3 cards/column with a "+N more" link to sprint.html, a progress wheel, and a "View sprint board" CTA, labeled "from sprint-status.yaml"); with no sprint it falls back to the derived in-dev/review/up-next cards. There is deliberately NO separate "Sprint Status" donut panel — the board is the widget. [src/SpecScribe/HtmlTemplater.cs, src/SpecScribe/SprintTemplater.cs]
- [x] [Review][Patch] "Now & Next" panel had been deleted from the home dashboard. Fixed by the same restoration above — Now & Next is back (as the sprint board when tracked, derived cards otherwise), not as a duplicate second panel. [src/SpecScribe/HtmlTemplater.cs]
