...
- **Wire the code link strategy setting through.** Add the setting as an **optional/nullable, non-`required`** `ForgeOptions` property so existing `new ForgeOptions { … }` constructions (tests included) keep compiling, and plumb it: `SiteSettings` CLI option → `Resolve()`, plus `SavedSettings`/`SettingsStore.ApplyTo`/`TrySave` for persistence. [Source: `src/SpecScribe/SiteSettings.cs:29-31`, `src/SpecScribe/SettingsStore.cs:56-88`, `src/SpecScribe/ForgeOptions.cs:53-99`] **[Amended 2026-07-13, code-review]:** originally this DO item also said "when set, skip `GenerateCodePagesInternal` entirely (external mode)." Story 7.7 superseded that with an additive model — `CodeSourceBaseUrl` now adds a "view source online" link to each in-portal page rather than replacing in-portal generation. This story still owns the setting + its plumbing; the generation gate described here no longer applies.
...
- ~~**DON'T pull in a syntax-highlighting dependency** (highlight.js, Prism, ColorCode, TextMateSharp, etc.) or any client JS.~~ **[Superseded by AC #4, 2026-07-12]:** AC #4 later required exactly this — a vendored, offline Prism.js bundle — as a progressive enhancement. The original rationale ("syntax-readable" ≠ tokenized coloring) held for the *initial* cut of this story; it no longer applies once AC #4 was added. "Syntax-readable" here means *legible as code* (monospace, line numbers, preserved whitespace, horizontal scroll for long lines) as a **no-JS floor**, not a ceiling. [Ref: memory — charts/rendering are pure, self-contained; the site ships one tiny sanctioned JS file only — this exception is now Prism.]
...
- **External mode [Amended 2026-07-13, code-review — superseded by Story 7.7]:** with `CodeSourceBaseUrl` set on `ForgeOptions`, `GenerateAll` still produces `code/` pages for every referenced file (additive model), each carrying an extra "view source online" link to the external base; it no longer suppresses in-portal generation.
...
- [x] [Review][Decision] AC #1's external-mode gate no longer matched the code — resolved 2026-07-13 (owner choice: amend spec, not code). Story 7.7 deliberately made `--code-url` additive (in-portal pages always generate; each gains a "view source online" link) instead of exclusive. Amended AC #1's DO item, the Testing Requirements "External mode" bullet, and the Dev Notes "Code link strategy" summary in this file to describe the additive model instead of the original exclusive-gate text.

- [x] [Review][Patch] `CopyEmbeddedAsset` for the Prism bundle is unguarded, unlike every other write path in this feature [src/SpecScribe/SiteGenerator.cs:1204-1205] — **fixed:** wrapped both calls in try/catch → `GenerationEvent(Error, ...)`, matching the "never throw out of the phase" discipline (NFR2) used everywhere else in this story.
- [x] [Review][Patch] `.swift` maps to `language-swift` but the vendored Prism bundle has zero Swift grammar support [src/SpecScribe/CodeFileTemplater.cs:606] — **fixed:** dropped the mapping (with an explanatory comment) so `.swift` files fall through to plain monospace instead of a `language-swift` class that Prism silently no-ops on. No test coverage existed for Swift, so nothing needed updating.
- [x] [Review][Patch] Empty (0-byte) source files render one phantom anchored blank line `id="L1"` and report "1 line" instead of 0 [src/SpecScribe/SiteGenerator.cs:1352-1361] — **fixed:** `SplitCodeLines` now returns an empty array for zero-length input instead of falling through to `[""]`.
- [x] [Review][Patch] Path-traversal containment check compares lexical paths only, not resolved real paths [src/SpecScribe/CodeReferenceScanner.cs:161-188] — **fixed:** `TryResolvePath` now resolves the candidate's final symlink target (if any) via `FileInfo.ResolveLinkTarget` and re-checks containment against it.
- [x] [Review][Patch] `--code-url` accepts malformed values with no validation (missing scheme, whitespace-only) [src/SpecScribe/ForgeOptions.cs] — **fixed:** added `TryValidateCodeUrl` (requires an absolute http/https URL, rejects whitespace-only) in `ForgeOptions.Resolve`; a malformed explicit value now falls back to auto-detection/null instead of being accepted verbatim.
- [x] [Review][Patch] ~~External-link host detection uses a naive substring match~~ — **retracted on closer inspection:** `ExtractHost` already isolates just the authority component (scheme-to-next-`/`) before the `Contains("github")` check, so the `mycompany.com/mirror/github-clone` example doesn't actually reach that branch (`ExtractHost` would return only `mycompany.com`). No code change made; this was a false positive from the reviewing subagent.
- [x] [Review][Patch] Dead code: `BinarySniffBytes` constant is declared and never referenced anywhere in the diff [src/SpecScribe/SiteGenerator.cs] — **fixed:** removed the unused constant and its comment.
- [x] [Review][Patch] Story doc gap: `tools/prism-vendor/package-lock.json` is a new file in the diff but isn't listed in the story's File Structure Requirements/File List — **fixed:** added to the File Structure Requirements list.
- [x] [Review][Patch] Story doc gap: the DON'T list still says "DON'T pull in a syntax-highlighting dependency (... Prism ...)" even though AC #4 (added 2026-07-12) explicitly requires the vendored Prism bundle — **fixed:** struck the bullet and annotated it as superseded by AC #4.
- [x] [Review][Defer] `StringComparer.OrdinalIgnoreCase` on repo-relative path dictionaries (`_codePages`, `_codeReverseMap`, discovery's `SortedSet`) risks silently colliding two case-differing files on a case-sensitive (Linux) checkout [src/SpecScribe/SiteGenerator.cs:78,87,1109-1115] — deferred, pre-existing: this mirrors the codebase-wide convention already used for `_docs` and the markdown reference map since before this story; not novel to 7.1.
- [x] [Review][Defer] `CommitHref`'s abbreviated-hash fallback does a linear `Dictionary` scan with no ambiguity check, so two same-prefix short hashes resolve non-deterministically [src/SpecScribe/SiteGenerator.cs] — deferred, pre-existing: `CommitHref`/`_commitPages` are Story 7.5's seam, reused here for the code page's History tab, not introduced by 7.1.
- [x] [Review][Defer] `EnumerateCodeFiles`/`MaxCodeFileBytes` reuse for the code-map's per-file line count (Story 7.6) reads every tracked file fully into memory and silently omits >1MB files from the size visualization — deferred, out of 7.1's scope (Story 7.6 code, bundled into this diff only because it shares `SiteGenerator.cs`).
- [x] [Review][Defer] Placeholder pages (binary/oversized/unreadable files) never receive `FileInsight`/History-tab data even when `--deep-git` has it, because `RenderPlaceholder` doesn't accept those parameters — deferred, out of 7.1's scope (the Insights/History tabs are Story 7.4/7.8/10.x work bundled into `CodeFileTemplater.cs`).
- [x] [Review][Defer] `TabGroupName` can theoretically collide across two paths that differ only in `/` vs. a literal `-` in a directory name — deferred, low likelihood, cosmetic (radio-group cross-wiring only affects the SPA/webview whole-document capture mode).
...
- **Code link strategy (`CodeSourceBaseUrl` / `--code-url`):** default (unset) → in-portal pages only. Plumbed through `ForgeOptions.Resolve`, `SiteSettings`, and `SavedSettings`/`SettingsStore` (CLI wins over saved). This story owns the setting + its plumbing; citation resolution (in-portal page vs. `{base}/<path>#L{n}`) is deliberately left to Story 7.2. The `_codePages` map (repo-relative → output-relative) is cached for 7.2 but not wired into any linkifier here. **[Amended 2026-07-13, code-review]:** the original "set → the in-portal phase is skipped entirely (external mode)" behavior was superseded by Story 7.7, which made the setting additive (in-portal pages always generate; each gains a "view source online" link to the external base) instead of exclusive.
...
- `tools/prism-vendor/{build.js,driver.js,package.json,package-lock.json,README.md}` — reproducible bundle recipe (node_modules gitignored).
