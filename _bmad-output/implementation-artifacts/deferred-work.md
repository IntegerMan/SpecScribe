...
## Deferred from: code review of spec-epic-10-deferred-work-cleanup (2026-07-20)

- source_spec: `spec-epic-10-deferred-work-cleanup.md`
  summary: `AbbreviationExpander`'s numbered-reference lookahead still doesn't treat a backslash as a separator, so a Windows-path-style numbered reference (e.g. `ADR\0005`) still wraps the acronym mid-reference.
  evidence: Edge Case Hunter, on the code-review patch tightening the dot/colon/slash lookahead. Speculative real-world occurrence — this portal's rendered prose is Markdown-derived HTML, where a bare backslash-separated citation form is unlikely to appear; not added to the separator set to avoid unbounded scope creep on a narrow acronym-tooltip nicety. Revisit if a real backslash-separated citation style is observed in authored content. [`AbbreviationExpander.cs`]
...
- source_spec: `10-11-sticky-section-nav-and-breadcrumb-coherence.md`
  summary: ~~`AppendLocalContextBand`'s pinned-active-item removal (`overflow.Where(i => i != pinnedActive)`) uses `NavLocalItem` record value-equality — if two overflow items share identical Label+Href+IsActive, all matching items are removed instead of just the pinned one, silently dropping a legitimate sibling from the "More" panel.~~ **RESOLVED 2026-07-20** (commit `efe6e34`, Story 10.8 code-review remediation): already fixed — `overflow.FindIndex(i => i.IsActive)` + `overflow.RemoveAt(pinnedIndex)` replaced the value-equality `Where` removal before this cleanup pass started; verified against current `HtmlRenderAdapter.cs`, no further change needed.
...
- source_spec: `10-11-sticky-section-nav-and-breadcrumb-coherence.md`
  summary: ~~`KeyViewGroupOrder.Contains(q.Group) ? q.Group : "Project"` fallback has no test coverage for an unrecognized/misspelled `Group` string silently misfiling a nav link into "Project".~~ **RESOLVED 2026-07-20** (`spec-epic-10-deferred-work-cleanup`): pinned by `HtmlRenderAdapterTests.RenderNav_OffHome_UnrecognizedQuickLinkGroup_FallsBackToProjectGroup`; fallback behavior was already correct, just untested.
...
- source_spec: `10-7-sunburst-navigability-at-project-scale.md`
  summary: ~~Dense-collapse (`Charts.Sunburst`) still computes `hasNoPlan` over all stories, but a collapsed 8+-story epic's zero-task stories fold into one `sb-story-summary` wedge with no `.sb-noplan` class — if every no-plan story lives inside a collapsed epic, the legend advertises a "no plan" swatch that matches no wedge and legend-hover dims all segments.~~ **RESOLVED 2026-07-20** (`spec-epic-10-deferred-work-cleanup`): `Charts.Sunburst` now tracks `hasVisibleNoPlan`, set only when an un-collapsed no-plan story wedge is actually drawn (dense-collapsed epics never set it); the legend swatch is suppressed when nothing matches. Pinned by `ChartsTests.Sunburst_DenseEpic_AllNoPlanStoriesCollapsed_SuppressesOrphanedNoPlanLegendSwatch`. `EpicSunburst` (per-epic detail chart, no dense-collapse) was unaffected — every story already gets its own wedge there.
...
- source_spec: `10-5-document-rendering-legibility.md`
  summary: ~~`ReferenceChipRenderer.ProtectedSplit` does not shield inner text of `<kbd>`/`<samp>` (open-tag-only catch-all); chip patterns inside those elements can be rewritten.~~ **RESOLVED 2026-07-20** (`spec-epic-10-deferred-work-cleanup`): `ProtectedSplit` gained `<kbd\b[^>]*>.*?</kbd>|<samp\b[^>]*>.*?</samp>` alternatives (same open-close capture shape as `<code>`/`<pre>`). Pinned by `ReferenceChipRendererTests.BareFileLine_InsideKbdSpan_IsUntouched` / `...InsideSampSpan_IsUntouched`.
...
- source_spec: `10-5-document-rendering-legibility.md`
  summary: ~~Retirement HTML comments in an epic preamble (before the first `### Story` heading) are never scanned by `HoistBetweenStoryRetiredComments` and stay in goal/meta text.~~ **RESOLVED 2026-07-20** (`spec-epic-10-deferred-work-cleanup`): `EpicsParser.ParseEpicSections` now starts the hoist scan right after the epic heading (`idx + 1`) instead of at the first story, so a preamble comment is hoisted too; the existing preceding-non-blank-is-a-story-heading guard already prevents misclassifying a story's own leading comment. Pinned by `EpicsParserTests.Parse_RetirementComment_InEpicPreamble_IsHoistedNotSweptIntoGoalText`.
...
- source_spec: `spec-abbreviation-expander-skip-numbered-refs.md`
  summary: ~~`AbbreviationExpander` still wraps bare acronyms mid-reference when the separator is punctuation other than space/hyphen/en/em-dash (e.g. `ADR.0005`, `ADR:0005`, `ADR/0005`).~~ **RESOLVED 2026-07-20** (`spec-epic-10-deferred-work-cleanup`): trailing lookahead widened to two shapes: space/hyphen/en/em-dash (optionally repeated) directly before 2+ digits, OR a single dot/colon/slash directly abutting 2+ digits with NO intervening space — deliberately narrower than a blanket "any of `.:/ ` then digits" so ordinary prose like "the ADR. 42 people voted" or "Total FR: 12" (space between the punctuation and the number) still expands normally. Pinned by `AbbreviationExpanderTests.Expand_SkipsNumberedReferences_DotColonOrSlashThenDigits` and `Expand_DotColonSlashFollowedBySpaceThenDigits_StillExpands`. Code-review patch (Blind Hunter caught the initial blanket-widening's false-negative risk on ordinary sentence punctuation).
...
- ~~source_spec: `spec-3-7-deferred-debt-cleanup.md`
  ...
  evidence: Blind Hunter, on the diff adding `RequirementFlowTextEquivalent`. Cosmetic duplication, not a behavior bug today; both copies currently agree exactly. Revisit if `RequirementFlow`'s coverage-partitioning is next touched.~~ **RESOLVED 2026-07-20** (`spec-3-7-deferred-debt-cleanup-2`): `NoCoverageKey`, `NoCoverage`, `CoverageKeys` (ordered epic keys + sentinel), `ForCoverageKey` (per-key membership), and `EpicTitlesByNumber` are now class-level statics shared by both `RequirementFlow` and `RequirementFlowTextEquivalent` — a future coverage-semantics change is a one-place edit. Only the epic-label string formatting (intentionally different: compact node label vs. title-inclusive sr-only label) remains distinct. [`Charts.cs`]
...
- source_spec: `6-4-read-only-vs-code-webview-runtime-for-dashboard-and-epics.md` — surfaced during the 6.4 review; not a correctness defect, left for a follow-up. (Section-header line, not a distinct item — no action of its own; see the entry below it.)
- ~~**Scoped re-render not implemented — every live-push does a full site regeneration.** `WebviewCommand` runs `generator.GenerateAll()` into a temp scratch dir on **every** debounced source change, then `RenderWebviewSurfaces()`; ADR 0005 §3 explicitly said *"Story 6.4 **must** add scoped re-render to feel live on large repos"* (~1.8–2.0 s warm full pass; coalesced by the shim's in-flight guard so never concurrent, but still full each time). Correctness is fine and small/medium repos feel fine; on large repos live refresh lags. Follow-up: scope the re-render to the changed artifact family (mirror `SiteGenerator.RegenerateEpics`) so refresh trends sub-second. Track as a 6.4 polish item or a dedicated story.~~ **RESOLVED 2026-07-20** (`spec-6-4-deferred-work-cleanup`, hardened after adversarial + edge-case review): added a persistent `specscribe webview --serve` mode (`WebviewCommand`/`Commands.cs`) that stays resident and reuses `FileWatcherService`'s existing debounced `RegenerateEpics()` incremental path instead of a fresh `GenerateAll()` per push, streaming one NDJSON `SerializePayload` line per regen — serialized under a lock so two files changed in the same debounce window can't interleave their writes into the stdout stream, and skipping the re-render/push entirely for `Skipped` (no-op) regens, not just `Error`. The extension's `SpecScribeStore` (`extension.ts`) spawns this ONE persistent connection (`PersistentRenderer`) per panel instead of respawning `runRenderer` on every save: a connection that never produces a payload (older core) falls back permanently to the one-shot path, while one that dies AFTER already streaming payloads triggers an automatic recovery reload (retrying `--serve`, not a permanent fallback) instead of leaving the panel silently stale forever. `PersistentRenderer`'s line buffer carries the same size cap as the one-shot path's stdout accumulator. [`Commands.cs`, `SiteSettings.cs`, `extension.ts`]

## Deferred from: code review of 6-4-...-webview-runtime (2026-07-11, parallel adversarial review)

- source_spec: `6-4-read-only-vs-code-webview-runtime-for-dashboard-and-epics.md` — surfaced by the parallel adversarial review; real but not blocking, left for a follow-up. (Section-header line, not a distinct item — no action of its own; see the entries below it.)
- ~~**Webview nav-toggle bridge drops keyboard/focus affordances — deferred to Story 6.5 (owner decision 2026-07-11).** The HTML surface's `NavToggleScript` (`HtmlRenderAdapter.cs`) implements Escape-to-close-with-focus-return, focus-first-link-on-open, close-on-navigate at mobile widths, and resize-reset; the webview bridge (`WebviewRenderAdapter.cs` DocumentTemplate) reimplements only class-toggle + `aria-expanded`. No `RenderParity` fact covers these affordances, so the harness cannot catch the NFR6/AC#2 fidelity loss. Owner routed this to Story 6.5, whose host-aware theming + explicit-helper-actions scope already owns the webview interaction chrome — fold the keyboard/focus affordances into the bridge there.~~ **RESOLVED 2026-07-20** (`spec-6-4-deferred-work-cleanup`): the bridge's nav-toggle click handler now focuses the first nav link on open, and a document-level `keydown` listener closes whichever `.site-nav` is open on Escape and returns focus to its toggle button (close-on-navigate/resize-reset were not carried — narrower parity than the HTML script, judged sufficient for the reported NFR6/AC#2 gap). Pinned by `WebviewRenderAdapterTests.Render_NavToggleBridge_HasKeyboardAndFocusParityWithTheHtmlScript`. [`WebviewRenderAdapter.cs`]
- ~~**Concurrent same-repo `specscribe webview` spawns collide on the scratch dir.** `Commands.cs` `RedirectOutputToScratch` keys the temp output only by a hash of the repo root, so two spawns for the same project (two VS Code windows, or a manual CLI run alongside the extension) `File.WriteAllText` the same files concurrently → IOException fails one render. The in-panel `loading` coalescer only prevents overlap within a single panel. Uncommon and retriable; give each spawn a unique subdir (or a lock) when this is next touched.~~ **RESOLVED 2026-07-20** (`spec-6-4-deferred-work-cleanup`): `RedirectOutputToScratch` takes an exclusive lock (`FileOptions.DeleteOnClose`, rooted in a static field so the GC can't finalize/release it early) on the stable per-repo scratch dir; a same-repo spawn that finds it already locked by a still-running concurrent spawn falls back to a process-id-suffixed sibling dir instead of racing its writes. [`Commands.cs`]
- ~~**Story artifact deleted mid-render aborts the whole webview bundle.** `SiteGenerator.BuildStoryPageFragments` → `MarkdownConverter.ReadAllTextShared` (reached from `RenderWebviewSurfaces`) reads a path cached in `_storyArtifactsById`; if the `.md` is deleted between `GenerateAll()` and `RenderWebviewSurfaces()` the read throws `FileNotFoundException` and fails the entire bundle rather than degrading that one story to a placeholder (as the HTML path's per-epic try/catch does). Sub-second same-process window; next refresh recovers. Mirror the placeholder fallback when this seam is next touched.~~ **RESOLVED 2026-07-20** (`spec-6-4-deferred-work-cleanup`): `RenderWebviewSurfaces`'s per-story fragment build is now wrapped in a try/catch (`IOException` or `UnauthorizedAccessException`, covering both deletion and ACL-denial) that degrades just that story to its placeholder page — the rest of the bundle still completes. Pinned by `SiteGeneratorWebviewTests.RenderWebviewSurfaces_StoryArtifactDeletedMidRender_DegradesThatStoryOnly`. [`SiteGenerator.cs`]
- ~~**Renderer timeout SIGTERM + unbounded stdout.** `extension.ts` `runRenderer` uses `proc.kill()` (default SIGTERM, may not reliably terminate the `dotnet` host on Windows → orphaned process) and accumulates stdout with no size cap. Not fatal; harden the kill (SIGKILL escalation) and cap the buffer when the shim is next touched.~~ **RESOLVED 2026-07-20** (`spec-6-4-deferred-work-cleanup`): a shared `killWithEscalation` helper sends SIGTERM then escalates to SIGKILL after a 5s grace period if the process hasn't exited, used by both the timeout path and a new stdout size cap (256 MB, well above the ~8 MB observed whole-site payload) that force-kills a runaway/looping renderer instead of accumulating unbounded memory — both abort paths still reject the promise immediately rather than waiting on the (possibly slow) kill to complete, preserving the original error-latency. [`extension.ts`]
- ~~**Scratch key case-folds the repo root.** `RedirectOutputToScratch` hashes `RepoRoot.ToUpperInvariant()`, so two distinct repos differing only in path case map to the same scratch dir on a case-sensitive filesystem (Linux). Negligible in practice; drop the case-fold (or use the OS path comparer) when this is next touched.~~ **RESOLVED 2026-07-20** (`spec-6-4-deferred-work-cleanup`, corrected after adversarial review): the scratch key now folds case only on a case-INSENSITIVE filesystem (`OperatingSystem.IsWindows()` — this project's primary target OS), so the same physical repo referenced with two different path casings still maps to the same stable dir there, while two genuinely distinct repo paths differing only by case stay distinct on a case-sensitive filesystem (Linux). An initial blanket "never fold" pass would have reintroduced the "successive spawns accumulate instead of overwrite" bug on Windows; the review caught this before it shipped. Pinned by `WebviewCommandTests.ScratchKey_CaseDifferingRepoRoots_MatchTheOsFilesystemsOwnCaseSensitivity`. [`Commands.cs`]

## Deferred from: code review of spec-6-4-deferred-work-cleanup (2026-07-20, adversarial + edge-case review)

- source_spec: `spec-6-4-deferred-work-cleanup.md`
  summary: `PersistentRenderer`'s stderr-to-payload diagnostic pairing is best-effort (whatever accumulated in `errText` is attributed to the next completed NDJSON line); if the `--serve` process crashes with trailing stderr notices immediately before exit, that final diagnostic batch is never flushed to the Problems panel.
  evidence: Edge Case Hunter. Narrow window (only the very last notices before a crash), cosmetic (diagnostics are advisory, never load-bearing for correctness) — revisit if trailing-notice loss is ever reported in practice.
- source_spec: `spec-6-4-deferred-work-cleanup.md`
  summary: The `IOException`/`UnauthorizedAccessException` catch around `RenderWebviewSurfaces`'s per-story fragment build covers deletion and ACL-denial but not a story file that exists yet is mid-write/corrupt during the same race window (which `ReadAllTextShared`'s `FileShare.ReadWrite` may read as truncated/garbled content rather than throwing at all).
  evidence: Blind Hunter, flagged as worth confirming rather than assumed. Would need to trace how a torn read propagates through the markdown pipeline before this is provably a bug (Markdig is generally lenient); left as a real-but-unconfirmed edge for a future pass.
...
- ~~**Client render/interaction perf at scale unmeasured (AC #1) → carry into Story 6.7.** The 6.6 spike measured client render only on this small repo (fetch 35 ms / render ~7–8 ms) and extrapolated the Epic-7 blow-up for file count alone (+863 → ~1,060 files). Owner decision 2026-07-10: not load-bearing for the re-affirm-C# outcome, so the large-dataset render/interaction measurement belongs on **Story 6.7 (JSON+SPA delivery adapter)** where the adapter is built — make it an explicit AC/task there.~~ **RESOLVED 2026-07-20** (`bmad-quick-dev` Story 6.6 deferred-work pass — measured directly, not re-homed): Story 6.7 shipped the `--spa` adapter (done) but never added the explicit at-scale AC, so this pass measured it against the shipped adapter on this repo at **`--deep-git` scale = 1,461 pages / 1,597 static HTML files (~7× the 219-page 6.7 dataset)**, served over HTTP and driven in a real browser. Results: (1) **file-count win holds and improves at scale** — 1,597 static files → static site + **34 SPA files** (~98% fewer navigable files); (2) **first paint is scale-invariant** — `app.html` 127 KB, DOMContentLoaded **~355 ms / load ~366 ms**, 93 dashboard SVGs painted, zero chunk fetch (dashboard inlined); (3) **normal in-view navigation stays snappy** — a moderate page ≈ fetch 40 ms + parse 17 ms + DOM-inject 115 ms ≈ **~170 ms** (epics chunk 4.3 MB / requirements 1.7 MB, both <60 ms fetch+parse). **The at-scale run also surfaced two real defects the 219-page 6.7 verification could never see** (byte-blind chunker + 82.5 MB code-map page → a 112.9 MB `pages-root.json` whose fetch+parse alone is ~567 ms on localhost) — logged as their own deferred items below. Per ADR 0006's owner ruling these remain non-load-bearing for the re-affirm-C# decision; the SPA is an optional additive adapter. [`SpaDelivery.cs`, `SiteGenerator.cs`]
- ~~**Out-of-scope planning edits rode along in the spike's commit range.** AC #6 constrains what lands on `main` to the decision's own artifacts (ADR 0006, README index, 0005 amend-note, story record, sprint-status update for 6.6). The `1c9270b`..`HEAD` range also carries an `8-4-state-aware-next-step-command-surface` flip (backlog→ready-for-dev in `sprint-status.yaml`), `epics.md` edits, the 6-4/6-5 story-file edits, and a dependabot `spike/vscode` esbuild bump (0.24.2→0.28.1). These are already committed on `main` and are most likely intentional planning bundling, not spike leakage — none touches `src/**` or `tests/**`. Left as-is; if a future review wants a clean per-story boundary, split planning-status edits from spike close-out commits.~~ **CLOSED 2026-07-20** (`bmad-quick-dev` Story 6.6 deferred-work pass) as accepted-as-is: re-confirmed intentional planning/dependency housekeeping, not spike leakage — the ride-along edits are all planning-status/dependency churn, **none touches `src/**` or `tests/**`**, and all are long since committed on `main`. Unwinding committed history for a retroactive per-story commit boundary is not worth the churn; the AC #6 quarantine that actually matters — no production pivot code on `main` (`src/SpecScribe/**` untouched) — held. No action. _(The `- source_spec:` line at the top of this section is section metadata / attribution for the two items above, not a defect — no action; any follow-up detail page generated for that bare line is a known parser artifact, not an open item.)_

## Deferred from: at-scale SPA perf measurement (Story 6.6 deferred-work pass, 2026-07-20)

- source_spec: `6-7-json-and-spa-delivery-adapter.md`
- **SPA chunker caps by page COUNT, not byte size — one mega-page creates a pathological chunk that penalizes every co-located page.** `SpaDelivery.BuildDataFiles` groups content regions by top-level output segment, then splits each group at `MaxPagesPerChunk` (75) via `count / MaxPagesPerChunk + 1` — a pure *count* cap with no byte ceiling (the doc-comment even claims it "bounds the largest at Epic-7 scale", but count-only splitting cannot). Measured at `--deep-git` scale (1,461 pages): the `root` group's **18 pages produced `pages-root.json` = 112.9 MB**, because `code-map.html` alone is 82.5 MB (next item). Since `about` / `sprint` / `requirements` / `epics` / `code-map` all share that chunk, navigating to ANY root page forces a **112.9 MB fetch (277 ms on localhost; ~10 s+ on real broadband) + 289 ms `JSON.parse` = ~567 ms before DOM injection**. Contrast: the epics (4.3 MB) and requirements (1.7 MB) chunks fetch+parse in <60 ms and inject in ~115 ms. Fix direction: add a per-chunk *byte* budget to the split, and/or isolate any over-threshold page into its own lazily-fetched chunk, so one giant page can't tax its neighbors' first navigation. Non-load-bearing per ADR 0006 (SPA is an optional additive adapter); revisit on the next SPA / Epic-7-scale perf pass. [`SpaDelivery.cs` (`MaxPagesPerChunk`/`BuildDataFiles`), `SiteGenerator.cs` (`BuildSpaBundle`)]
- **`code-map.html` balloons to ~82.5 MB at large-repo / `--deep-git` scale.** The source treemap renders a rect per tracked file; against the full repo tree (this run: 1,461 pages, deep-git on) the single static `code-map.html` is **82.5 MB** — a *static-site* problem the SPA merely inherits (it ships the same pre-rendered region, so injecting it into the DOM on navigation — scaling from a measured 115 ms / 36 KB baseline — is multi-second and main-thread-blocking). Adjacent to the existing per-file byte-ceiling deferral on `spec-7-1-deferred-debt-cleanup.md` (`TryCountCodeLines` lost its 1 MB skip). Fix direction: bound the treemap (cap depth / leaf count with an honest "+N more" truncation, or a total-size budget) so the page stays legible and loadable on large repos. Non-load-bearing; revisit with a Code Map / NFR1 pass. [`CodeMap.cs` (source treemap)]
...
- ~~**Commit body containing a literal `0x1F` control char could truncate numstat rows.**~~ **RESOLVED 2026-07-19** (`spec-3-8-deferred-debt-cleanup`). `ParseNumstatRecords` now rejoins everything between the subject and the last split segment as the body (re-inserting the sentinel), and always takes the LAST field as the numstat block — an embedded 0x1F in message text can no longer shift the numstat rows off a fixed index. Covered by `GitMetricsTests.ParseNumstatRecords_BodyContainingRawFieldSentinelDoesNotTruncateNumstatRows`.
- ~~**Commits with unparseable dates diverge `CommitCount` from summed `Activity`.**~~ **RESOLVED 2026-07-19** (`spec-3-8-deferred-debt-cleanup`). `BuildInsights` now derives `GitInsightsData.CommitCount` from `Activity`'s own summed counts instead of raw `commits.Count`, so an undated commit is excluded from both totals rather than inflating one without the other — they can never diverge by construction. Covered by `GitMetricsTests.BuildInsights_CommitCountNeverDivergesFromSummedActivity`.
- ~~**Churn sums every numstat row per commit while `Changes` dedups per commit-path.**~~ **RESOLVED 2026-07-19** (`spec-3-8-deferred-debt-cleanup`) — closed as a coverage gap, not a behavior change: the tradeoff was already intentional and documented in the code's own comment. Pinned by `GitMetricsTests.BuildInsights_ChurnSumsEveryNumstatRowWhileChangesCountsOncePerCommit` so it can't silently change either way.
- ~~**`:target`-revealed contributor panel has no explicit focus management for keyboard/AT users.**~~ **RESOLVED 2026-07-19** (`spec-3-8-deferred-debt-cleanup`). `.gi-contributors-panel` now carries `tabindex="-1"`, and `specscribe.js` moves focus into it on `hashchange`/initial load when JS is available (no-JS behavior is unchanged — the panel still reveals via CSS). The sibling commit-heatmap/coupling-graph `:target` panels were explicitly left alone (out of scope for this story).
- ~~**No test asserts a stale `git-insights.html` is removed after `--deep-git` is later disabled on the same output dir.**~~ **RESOLVED 2026-07-19** (`spec-3-8-deferred-debt-cleanup`). Added `SiteGeneratorGitInsightsTests.GenerateAll_DeepGitLaterDisabled_RemovesStaleHubFromSameOutputDir`, pinning the existing `GenerateAll` full-output-wipe guarantee for this page specifically.

## Deferred from: code review of story-3-7 (2026-07-09)

- source_spec: `3-7-requirements-flow-and-status-blocks.md`
- ~~**Sankey layout doesn't scale height for very large requirement counts.** `Charts.RequirementFlow`'s `unitH = Math.Max(2.0, (usableH - gap * (maxNodes - 1)) / n)` floors at 2px, but the SVG `height`/`viewBox` is a fixed constant (`topPad + usableH + 26`) that never grows to compensate. Once `n` (or the node/gap count) is large enough to hit the floor — roughly 150+ requirements at current column widths — a column's total layout height can exceed `usableH`, overlapping the header/footer. At today's project scale (26 requirements) this is nowhere close to triggering. Pre-existing pattern shared with `CouplingGraph`/`RefinementFunnel`, not unique to this story. Revisit if a project's requirement count grows into that range, or when the shared custom-SVG-layout helpers are next touched.~~ **RESOLVED 2026-07-19** (`spec-3-7-deferred-debt-cleanup`): the SVG canvas height now grows to the tallest actual column (`ColumnHeight` per column, `Max()`'d against `usableH`) instead of a fixed constant, so hitting the `unitH` floor no longer overflows the header/footer. [`Charts.cs`]
- ~~**Dashboard requirements panel lacks the per-epic/per-status text-equivalent that requirements.html has.** `HtmlTemplater.AppendRequirementsPanel` renders the tile grid + the Sankey on the home dashboard, but (unlike `requirements.html`) has no requirement cards below it. Screen-reader users on the dashboard get the whole-diagram `aria-label` (overall totals) and per-tile tooltips (per-requirement id/status), but not the per-epic breakdown a sighted user gets by hovering individual ribbons/nodes. Low severity — the totals and per-requirement status are still accessible as text, just not the epic-level detail. Revisit if the dashboard panel is next touched.~~ **RESOLVED 2026-07-19** (`spec-3-7-deferred-debt-cleanup`): new `Charts.RequirementFlowTextEquivalent` renders an `sr-only` per-epic/per-status breakdown list on the dashboard requirements panel (`HtmlRenderAdapter.Dashboard.cs`), matching what `requirements.html`'s requirement cards already give sighted users. [`Charts.cs`, `HtmlRenderAdapter.Dashboard.cs`]
...
- **RESOLVED 2026-07-20** (audit, no code change — `spike/vscode/` stays quarantined and untouched, ADR 0005 intent). All 8 items below describe defects in the throwaway spike shim; each has a confirmed hardened equivalent in the production `extension/src/extension.ts` runtime (Stories 6.4–6.12), verified line-by-line against current HEAD:
  - ~~Default renderer spawn path is never populated + dead `exe` fallback.~~ → `resolveTool()` (`extension/src/extension.ts`): real 3-tier resolution (`specscribe.toolPath` setting → bundled binary → `specscribe` on PATH), shared by the panel spawn and the terminal handoff.
  - ~~Panel-reuse re-runs the whole `openStatus` handler.~~ → `openStatus()` early-returns `active.reveal(request)` when a controller exists; watcher/message-handler registration happens once, inside `createController`.
  - ~~Live-push watcher glob decoupled from the renderer's resolved source root.~~ → `SpecScribeStore.rebuildWatchersFromRoots()` anchors watchers to the core-emitted `sourceRoot`/`adrRoot`/`repoRoot` (Story 6.11), correct even when VS Code is opened on a subdirectory.
  - ~~Overlapping debounced re-renders race with no generation guard.~~ → `SpecScribeStore.load()` coalesces concurrent spawns via an in-flight promise (`if (this.loading) return this.loading`).
  - ~~Surface-switch `await load()` has no error handling.~~ → `refreshCommand`/`createController`'s cold-start load are both wrapped in try/catch with a user-facing toast (`showActionableError`).
  - ~~`withRuntime` does a whole-document token `split/join`.~~ → `composeEntryHtml()` extracts the rendered content behind a random per-call sentinel before substituting `__NONCE__`/`__CSP_SOURCE__`, then splices it back — content containing those literal tokens can no longer corrupt the shell or forge a nonce.
  - ~~Live-push scoped to `*.md` only.~~ → `SpecScribeStore.startWatching()` globs include `.yaml`/`.yml`/`.toml` (`_bmad-output/**/*.{md,yaml,yml}`, `_bmad/config.toml`) alongside `.md`.
  - ~~No spawn timeout / kill.~~ → `runRenderer()` sets a 60s timeout that calls `proc.kill()` and rejects.
  - ~~Renderer arg/enumeration edges.~~ → production's `WebviewCommand` (`src/SpecScribe/Commands.cs`) binds arguments through Spectre.Console.Cli's typed `SiteSettings`, not string-matched argv — the ambiguous-positional-arg bug class the spike's hand-rolled `Program.cs` had doesn't exist in this design.

## Deferred from: code review of story-6-5 (2026-07-11)

- source_spec: `6-5-host-aware-theming-and-explicit-helper-actions.md` — **CLOSED 2026-07-20** (`spec-6-5-deferred-work-cleanup`): header/pointer line, no distinct defect of its own; folded into the five closures below.
- ~~**`.vscode-light` has no dedicated contrast-tuning block.** `src/SpecScribe/assets/specscribe-webview-theme.css` — the story's own design decision sanctions reusing today's warm-light accent values for the light webview theme, but its "must still be verified against the actual host light background" clause has no recorded check or test. Revisit as a manual/QA verification pass, not a code patch.~~ **RESOLVED 2026-07-20** (`spec-6-5-deferred-work-cleanup`). Pinned an explicit `.vscode-light { ... }` block (byte-identical to `:root`'s shipped values — no rendering change) mirroring the other three theme scopes. The verification is COMPUTED, not eyeballed: `StylesheetTests.VscodeLightBlock_MatchesRootValues_AndRealTextTokensClearWcagAA` asserts (a) zero drift from `:root` and (b) the tokens actually used as body text color (teal/teal-deep/moss/rust, confirmed by grep) clear WCAG AA on VS Code's white Light+ canvas. Correction during review: an earlier draft of this closure claimed ALL accent tokens "clear AA with a wider margin," which is false for `--gold-light` (~2.4:1 vs white) — that token is border/fill-only, always paired with its own literal pastel background, so "vs the page canvas" isn't its relevant adjacent surface; the test deliberately does not assert a number for it. [`WebviewThemingTests.cs`, `StylesheetTests.cs`]
- ~~**`SiteGeneratorWebviewTests.FullGenerateThenWebviewPass_LeavesSourceArtifactsUntouched` only compares file set + `LastWriteTimeUtc`, not content.** A write that preserves mtime would pass this "leaves source artifacts untouched" guard undetected. Strengthen with a content hash comparison when this test is next touched.~~ **RESOLVED 2026-07-20** (`spec-6-5-deferred-work-cleanup`). Added a SHA-256 content-hash comparison alongside the existing mtime comparison in `SiteGeneratorWebviewTests.cs`, both snapshotted from a single file listing per phase (avoids a key-set-divergence risk between two separately-enumerated dictionaries); a content mutation that preserves `LastWriteTimeUtc` now fails the test.
- ~~**Live-refresh debounce spawns the renderer even after the panel is disposed.** `extension/src/extension.ts` — the `disposed` guard in the debounced refresh callback is checked only after `await load()` resolves, not before calling `load()`, so a refresh timer firing post-disposal still spawns a wasted `specscribe webview` child process (discarded result, no user-visible effect).~~ **RESOLVED as superseded 2026-07-20** (`spec-6-5-deferred-work-cleanup`, audit — no code change). This described `extension.ts` as it existed at Story 6.5; the Story 6.9/6.11 `SpecScribeStore` refactor replaced that logic entirely. Verified against current HEAD: `SpecScribeStore.onWatchEvent` (`extension.ts`) now gates every reload on `anyConsumerVisible()` *before* calling `load()` — a disposed panel with no other visible consumer (tree) marks the reload `dirty` instead of spawning — and the panel's own `dataChanged` subscriber checks its `disposed` flag before pushing, not after an unguarded async gap. The exact ordering this item asked for already exists structurally. No regression test protects this ordering going forward (no TS test harness exists in this repo) — tracked as a new deferred item below.
- ~~**Test assertions in `WebviewHelpersTests`/`WebviewThemingTests` pin exact prompt wording with ordinal string matching** (e.g. `"Do NOT modify any files"`), coupling test stability to copy-editing rather than to the read-only behavioral contract they're meant to protect.~~ **RESOLVED 2026-07-20** (`spec-6-5-deferred-work-cleanup`). Extracted the directive into `WebviewHelpers.ReadOnlyDirective` (a named `const`); both test classes assert the prompt/doc carries the constant (structural — catches "forgot to append it"). A review patch added a second, independent test asserting the constant's OWN text loosely contains read-only keywords ("not"/"modify"/"text only", case-insensitive) — the constant-vs-constant check alone was tautological (any wording, even gibberish, would have passed) and couldn't have caught the directive being emptied or gutted.
- ~~**No test exercises `<`/`>` in `siteTitle` reaching the `data-ss-prompt` attribute** — only a double-quote case is covered, even though the same `PathUtil.Html` path is relied on to prevent markup injection there.~~ **RESOLVED 2026-07-20** (`spec-6-5-deferred-work-cleanup`). Added `Render_EscapesTheHelperPrompt_WhenTheSiteTitleContainsAngleBrackets` (`WebviewThemingTests.cs`), pinning that a `<script>`-bearing site title reaches `data-ss-prompt` HTML-encoded, never as a raw tag.

## Deferred from: code review of spec-6-5-deferred-work-cleanup (2026-07-20)

- source_spec: `spec-6-5-deferred-work-cleanup.md`
  summary: No regression test protects the "gate every reload on `anyConsumerVisible()` before calling `load()`" ordering in `extension/src/extension.ts` (`SpecScribeStore.onWatchEvent`) — a future refactor could silently reintroduce a post-disposal wasted spawn with nothing to catch it.
  evidence: Confirmed correct at current HEAD by manual code-path audit while closing the Story 6.5 "live-refresh debounce spawns the renderer even after the panel is disposed" item, but this repo has no TypeScript test harness at all (`extension/package.json` has no test script/framework configured) — adding one is out of scope for a narrow deferred-work cleanup. Revisit if/when the extension gains a TS test runner.
...
- **RESOLVED — see `spec-7-2-deferred-work-cleanup.md` (2026-07-20).** ~~Code-reference resolution requires an extension on the last path segment and doesn't percent-decode paths.~~ `src/SpecScribe/CodeReferenceLinkifier.cs` (`IsRelativeCodeHref`) now accepts a documented allow-list of conventional extensionless filenames (`Dockerfile`, `Makefile`, `LICENSE`, etc.) and both citation matchers percent-decode the candidate path before resolving, so a citation with `%20` (or similar) now matches its real codePages/repo-file key.
  evidence: Edge Case Hunter finding (low), reviewed and reasoned about directly against the merged code.
- **`_codePages` forward-map entries are pruned (on a code-page write failure or output-escape) AFTER story/doc/ADR pages were already linkified against the optimistic pre-prune map.** `src/SpecScribe/SiteGenerator.cs:711-777,829-877` — re-verified 2026-07-20 (`spec-7-2-deferred-work-cleanup.md`): the "output escapes root" `_codePages.Remove(...)` at `SiteGenerator.cs:1657-1665` is still present in the code (not removed) but is now confirmed provably unreachable in practice — `repoRel` is always derived via `Path.GetRelativePath` after an inside-repo check, so it can never contain a `..` segment that would let `code/{repoRel}.html` climb above `OutputRoot`. The other prune site is a broad `catch (Exception ex)` around the whole per-file render/write (`SiteGenerator.cs:1737-1741`) — it's not scoped to I/O faults specifically, so it would also mask a latent bug anywhere in that block (`BuildReferencedBy`, `CodeFileTemplater.RenderPage`/`RenderPlaceholder`, `WriteOutput`), not only a genuine TOCTOU race between discovery's `File.Exists` check and the actual read/write. The realistic likelihood is still low (same accepted risk class as `spec-entity-prev-next-navigation`'s deferred sibling-pager finding), but the confirmed-unreachable branch is now the *only* part of this finding that's fully closed. A proportionate fix (a post-write second linkification pass, or reordering code-page generation before all other pages) remains out of scope.
  evidence: Acceptance Auditor + Blind Hunter agreed; verified against the actual `GenerateAll` call order (`DiscoverCodeReferences` → epics/story/doc pages → `GenerateAdrsInternal` → `GenerateCodePagesInternal`); re-traced 2026-07-20 against `CodeReferenceScanner.TryResolvePath`.
- **`IsRelativeCodeHref` is unscoped to genuine `[Source: …]` citations — it matches ANY relative `<a href>` with a non-`.html`/`.md` extension.** `src/SpecScribe/CodeReferenceLinkifier.cs:77-96,165-183` — a legitimate prose link to an unrelated asset (image, download) that doesn't resolve gets silently stripped to plain text by `RewriteHrefs`. Two candidate fixes were evaluated and rejected: requiring a leading `../` climb breaks the bare `[label](src/Foo.cs)` citation shape genuinely used in the live corpus (`10-1-insights-navigation-and-structure-page-retirement.md:17`) and in `CodeReferenceLinkifierTests.EscapesEmittedHref`; gating on real filesystem existence breaks in-portal mode's deliberate disk-free design and the `UnresolvedHref_DegradesToPlainText` test. A safe fix needs citation context threaded from the markdown-parse stage through to rendered HTML, not a rewrite of the HTML-stage matcher. Reconfirmed 2026-07-20 (`spec-7-2-deferred-work-cleanup.md`): still requires that larger threading change; not attempted again.
...
- **RESOLVED — see `spec-7-2-deferred-work-cleanup.md` (2026-07-20).** ~~`BuildReferencedBy`'s fallback (`PathUtil.ToOutputRelative`) guesses a URL for a citing artifact missing from `_referenceMap`, rather than confirming it.~~ The guessing fallback is intentionally kept (a prior "omit" patch was tried and reverted — see history below); the doc comment on `src/SpecScribe/SiteGenerator.cs`'s `BuildReferencedBy` (~line 1750) was misdescribing the reverted "omit" behavior instead of the shipped guess-fallback, and now accurately documents what actually ships.
  evidence: Acceptance Auditor + Blind Hunter agreed; verified directly against the real test suite (one attempted fix reverted after a confirmed regression). Comment/behavior mismatch caught and fixed 2026-07-20.
...
## Deferred from: code review of 10-10-context-aware-navigation-bar (2026-07-20)

- ~~`groupByHref` in `SiteGenerator.WriteFollowUpDetails` is built with a plain dictionary indexer (last-write-wins on key collision), with no guard confirming `FollowUpGroupPages.Enumerate`'s three membership sources (orphans / unplanned / per-epic) stay mutually exclusive. Currently unreachable — the three sources partition follow-up items disjointly by construction — but nothing enforces that invariant at the assignment site, so a future change to `Enumerate` could silently make a follow-up detail page's local-context band show the wrong group with no test or runtime signal. [`SiteGenerator.cs`]~~ **RESOLVED 2026-07-20** (`spec-epic-10-deferred-work-cleanup`): the `groupByHref` build loop now carries a `Debug.Assert` making the mutual-exclusivity invariant observable in Debug builds instead of only silently letting the last-write-wins indexer hide a collision — note this assert compiles out entirely in Release, so it is a dev-time signal only, not a production guard. Code-review patch verified: no test exercises the `Debug.Assert` line itself. `FollowUpGroupPagesTests.Enumerate_MixedInventory_MembershipSourcesAreMutuallyExclusiveByDetailHref` pins the underlying invariant at the `FollowUpGroupPages.Enumerate` data level (the assert's actual precondition), not the assert statement in `SiteGenerator.cs`.
- ~~`counts`/`geometry`/`FollowUpGroupPages.Enumerate` now run unconditionally at the top of `WriteFollowUpDetails`, even when both `actionItems` and `deferredPairs` are empty — the prior code only built this inside the `deferredPairs.Count > 0` guard. Pure wasted computation on a project with zero follow-up work; no correctness impact. [`SiteGenerator.cs`]~~ **RESOLVED 2026-07-20** (`spec-epic-10-deferred-work-cleanup`): already non-reproducible — investigated and found no code change needed. The existing `if (actionItems.Count == 0 && deferredPairs.Count == 0) return;` guard at the top of `WriteFollowUpDetails` already prevents the computation whenever there is genuinely zero follow-up work; when `actionItems.Count > 0`, the computation is no longer wasted since Story 10.10 wired `actionLocalContext` through the same `groupByHref` map, so both loops legitimately need it.
