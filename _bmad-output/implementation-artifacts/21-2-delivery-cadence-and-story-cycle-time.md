...
- 2026-07-22: Code review (3-layer adversarial) — applied 3 patches (heatmap aggregates bounded to `<= today` + duplicate-day-safe; dead `storyHref`/`FindStory`/`epics` param removed), deferred 2 (per-story git shell-out gating; dashboard-strip clock), dismissed 2. 2144 tests green, golden unchanged. Status review → done.
...
_Code review 2026-07-22 (bmad-code-review, 3-layer adversarial: Blind Hunter + Edge Case Hunter + Acceptance Auditor). Diff scoped to the story File List against `baseline_commit` eaa2348._

- [x] [Review][Patch] Future-dated completion overstates heatmap aria-label, window label, and counts vs the rendered grid [src/SpecScribe/Charts.cs:1406] — `firstDay`/`lastDay` (`series.Min`/`Max`, L1406-1407), `totalCompletions`/`activeDays` (L1431-1432), and the window text (L1545) are computed over the FULL series with no `<= todayValue` bound, while the cells (L1481), `maxCount` (L1421), and the completion log (L1582) all exclude future days. A done story whose `LastUpdatedDate` is after the run's `today` (a manually-entered/typo'd future Change-Log date, or git author-clock skew) makes the SVG announce more completions / a later end-date than the grid actually draws — violates the project's truthfulness invariant. `CommitHeatmap` never hits this because its series is git-bounded; this heatmap's `LastUpdatedDate` source is not. Fix: bound the display aggregates to `s.Day <= todayValue` (fall to the empty state if none remain).
- [x] [Review][Patch] Dead `storyHref` param + `FindStory` helper on the cycle-time histogram path [src/SpecScribe/Charts.cs:1626] — `CycleTimeHistogram(cycleTimes, storyHref)` never references `storyHref` (bars are per-bucket, no per-story link), yet `CadenceTemplater.RenderPage` (CadenceTemplater.cs:72) builds `id => storyHref(FindStory(epics, id))` to feed it, making the whole `FindStory` helper (CadenceTemplater.cs:83-92, incl. its synthetic-StoryInfo fallback) unreachable. Misleading public signature + ~15 dead lines. Fix: drop the param from `CycleTimeHistogram`, drop the resolver arg + `FindStory` from the templater.
- [x] [Review][Patch] `series.ToDictionary(s => s.Day, …)` throws `ArgumentException` on a duplicate-day series [src/SpecScribe/Charts.cs:1405] — `DeliveryCadenceHeatmap` is `public`; the production caller feeds a `SortedDictionary`-keyed (unique-day) series so it's not reachable today, but any future/test caller passing repeated days aborts the whole page render instead of degrading. Fix: aggregate duplicates (GroupBy/sum) instead of `ToDictionary`. Low — not reachable in current wiring.
- [x] [Review][Defer] First-touch git shell-out runs once per done story on every generation, not gated on `--deep-git` [src/SpecScribe/SiteGenerator.cs:418] — deferred, spec-accepted. `DeliveryCadence.Build(..., GitFirstTouchResolver(...))` spawns one `git log --follow` subprocess per done story with a resolvable done-date each run; on a mature repo (hundreds of done stories) that's hundreds of subprocesses even without `--deep-git`. The spec explicitly sanctions this as "bounded new git work" and `cadence.html` always renders the cycle-time histogram, so it's by design — flagged as a future perf-pass candidate (gate the cycle-time half behind a git/deep-git signal), not a defect.
- [x] [Review][Defer] Dashboard cadence strip reads its own wall clock instead of the run's single `today` [src/SpecScribe/DashboardViewBuilder.cs:86] — deferred, matches accepted baseline. `Charts.DeliveryCadenceStrip(c, CadenceOutputPath)` is called with no `today`, so the strip's 8-week window uses its own `DateOnly.FromDateTime(DateTime.Now)` — a second clock read from the one `WriteCadence` computes for the page grid. Byte-identical on a fixed clock (AC #2 holds), diverges only on a midnight-straddling run; matches the accepted `CommitHeatmap` per-call `DateTime.Now` precedent. Fix if desired: thread the run's `today` into the strip.

**Resolution (2026-07-22):** All 3 patches applied — heatmap display aggregates now bound to `<= today` (`visible` subset, falls to empty-state if nothing remains) + duplicate-day-safe `GroupBy`; dead `storyHref`/`FindStory` removed (and the now-unused `epics` param dropped from `CadenceTemplater.RenderPage`). Build clean, full suite 2144 passed / 3 skipped, `GoldenContentFingerprint` unchanged (byte-identical — this repo has no future-dated stories). Both defers logged to `deferred-work.md`. Status → done.

_Dismissed as noise (2):_
- Cycle-time endpoints measured from `LastUpdatedDate` (git-else-Change-Log) vs `TryGetFirstCommitDate` (git first-commit) can silently drop a story on a negative span — spec-sanctioned: the story explicitly skips (never clamps/fabricates) bad matches and carries the "approximate" caveat.
- Diff bundles sibling Stories 19.2 (WorkGraph) / 21.3 (ImpactMap) inside shared files (`Charts.cs`, `SiteGenerator.cs`, `specscribe.css`, golden fingerprint) — a review-scope hygiene note (already surfaced at the checkpoint), not a 21.2 code defect; those are real sibling stories landing in the same commits.
