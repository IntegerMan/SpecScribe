...
- [x] [Review][Patch] (owner-resolved 2026-07-13: wire it now — fixed) AC3 not met: `fileHref: null` withholds working code-page links from the Code Map despite the resolver existing — `WriteCodeMap` calls `CodeMapTemplater.RenderPage(map, layout, nav, fileHref: null)` ([SiteGenerator.cs:2102](../../src/SpecScribe/SiteGenerator.cs)), while the SAME `CodeItemHref` resolver is already wired into `DeepAnalyticsTemplater.RenderPage` ([SiteGenerator.cs:279](../../src/SpecScribe/SiteGenerator.cs)) and `GitInsightsTemplater.RenderPage` ([SiteGenerator.cs:1326](../../src/SpecScribe/SiteGenerator.cs)) in this same diff. Fix: pass `fileHref: CodeItemHref` in `WriteCodeMap`. **Bundled sub-fix:** `Charts.AppendTreemapFile` builds the SVG rect's `<a href>` from the bare `target` with no path prefix, while `CodeMapTemplater.AppendFileTable`'s text-table link uses `prefix + target` ([CodeMapTemplater.cs:151](../../src/SpecScribe/CodeMapTemplater.cs)) — align `AppendTreemapFile` to also apply `prefix` so both link sets agree once wired.
- [x] [Review][Patch] (fixed) Invalid nested ARIA on linked treemap cells — file rects get `role="link"` while also being wrapped in a real `<a href>` ([Charts.cs:1273-1280](../../src/SpecScribe/Charts.cs)), an interactive role nested inside an already-interactive link, likely to produce doubled/confusing screen-reader announcements. Drop the `role="link"` when the rect is already inside an `<a>`.
- [x] [Review][Patch] (fixed) AC4 gap on dimension switch — `recolor()` in `specscribe.js` re-buckets each cell's fill class but never updates the cell's `aria-label`, its `data-tip` tooltip text, or the always-on legend's dimension name ([specscribe.js:379-398](../../src/SpecScribe/assets/specscribe.js)). After switching to e.g. "Recently changed" or "Avg change size," color becomes the sole carrier of that dimension for assistive-tech and tooltip users — the exact thing AC #4 requires never happen. Refresh the text equivalents alongside the color classes.
- [x] [Review][Patch] (fixed) Degenerate single-point recolor mislabels the one file with data as "no activity" — in `recolor()`'s date branch, when exactly one cell has a value for the selected dimension, `min == max` so `range = 0`, and `bucket(0, 0)` short-circuits to level 0 via the `max <= 0` guard ([specscribe.js:360-364,390](../../src/SpecScribe/assets/specscribe.js)). The one file that DOES have git data for that dimension renders identically to files with none. Guard the degenerate range case (e.g. treat a single valid point as the top bucket).
- [x] [Review][Patch] (owner-resolved 2026-07-13: require a minimum-visible-slice guarantee — fixed) AC1 / Dev Notes contradiction on sub-pixel directories — `EmitRect`'s `MinTileable` check ([CodeMap.cs:391-398](../../src/SpecScribe/CodeMap.cs)) drops an ENTIRE directory subtree from the SVG (not just one small file) once its tiled inner region falls under 6px, measured at ~15% of files on the real-repo run per the Dev Agent Record ("705 file cells... 831-row text-equivalent table"). Fix: extend the existing `Math.Max(n.Lines, 1)` visibility floor through the tiling threshold so a directory that falls under `MinTileable` still emits a minimal representative slice for its subtree (or its files directly) instead of silently vanishing, accepting reduced legibility on very deep/wide trees.
- [x] [Review][Patch] (fixed) Zoom-tween duration bypasses the `--motion-*` token system — `setViewBox`'s `requestAnimationFrame` tween hardcodes `dur = 240` ([specscribe.js:437](../../src/SpecScribe/assets/specscribe.js)) instead of reading a `--motion-*` custom property, contrary to the explicit Subtask 5.4 instruction to "route the zoom transition through the `--motion-*` tokens... rather than a bare transition." The `prefers-reduced-motion` snap branch is correct; only the duration/easing source is off-convention.
- [x] [Review][Defer] Per-file exception guard in `EnumerateCodeFiles` is too narrow [SiteGenerator.cs:2307](../../src/SpecScribe/SiteGenerator.cs) — deferred, pre-existing. The per-file `catch (Exception ex) when (ex is IOException or UnauthorizedAccessException)` lets any other exception type from the shared `TryReadCodeText`/`SplitCodeLines` helpers (used since the Story 7.1 code pages) bubble to the outer catch and empty the WHOLE code-map result rather than skipping just that file. The shared helpers pre-date this diff; this story only enlarges the blast radius by also gating a nav item on the result.
- [x] [Review][Defer] `GitMetrics.TryListFiles` line-splits `git ls-files` output [GitMetrics.cs:850](../../src/SpecScribe/GitMetrics.cs) — deferred, pre-existing pattern. Splitting on `\n` and `.Trim()`-ing each line (instead of `-z`/NUL-delimited output) would corrupt paths for a tracked filename containing a literal newline or significant leading/trailing whitespace. Extremely rare in practice; matches the parsing discipline already used elsewhere in `GitMetrics`.

**Post-patch verification (2026-07-13):** Full suite green except two known-unrelated failures caused by a *different, concurrently-running session* actively editing `PathUtil.cs`'s footer/date rendering (separate Story 10.4 "PortalDates" work landing in this same shared `main` checkout while this review ran) — `PathUtilTests.RenderFooter_CarriesRepoAndDetailsLinks_FromRoot` and the golden content fingerprint (which hashes the footer). The fingerprint constant was updated once to `df8ad228cd513076d863a3eb7c279ef34b6f719031bd830ac05c869dee7f3f48` to reflect this story's own CSS/JS changes, but it kept drifting further on subsequent runs purely from the concurrent footer edits — **it will need a fresh, uncontested regeneration once the concurrent PathUtil.cs work lands**, this is not a Story 7.6 defect. All 971 other tests (including 3 new Story 7.6 regression tests added by this review: `CodeTreemap_AppliesThePrefixToLinkedFileHrefs`, `Layout_TinyDirectoryStillEmitsRectsForEveryDescendantFile`, `GenerateAll_WithCodeSourceBaseUrlConfigured_LinksTreemapCellsAndTableRowsToSource`) pass.
