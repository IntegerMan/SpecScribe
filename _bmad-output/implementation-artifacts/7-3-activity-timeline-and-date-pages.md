...
- [x] [Review][Patch] Artifact-mtime signal is silently gated behind `epics.md` existing — `_referenceMap` (the sole source for `BuildArtifactsByDay`) is only populated inside `RenderEpicsPages`, which only runs `if (epicsSourceFile is not null && ...)`. A project with real git history and real source artifacts but no `epics.md` gets zero timeline/date-page output, even though AC #1 frames "artifact timestamps" as an independent fallback to git. Confirmed independently by the Acceptance Auditor and Blind Hunter. **Owner decision (2026-07-13): decouple.** **Applied:** `BuildArtifactsByDay` now unions recognized artifacts from `_referenceMap` (epics/story family), `_docs` (standalone doc pages), and `_adrs` (ADR pages, resolved against `AdrSourceRoot`) instead of `_referenceMap` alone. 1078 tests green. [src/SpecScribe/SiteGenerator.cs — `BuildArtifactsByDay`]
- [x] [Review][Defer] Watch-mode (`GenerateOne`) never refreshes the timeline/date-page signal — editing an artifact's content (and hence its mtime) in watch mode only calls `WriteIndex(nav)`, reusing the stale cached `_timelinePath`/date pages; `BuildArtifactsByDay`/`GenerateDatePagesInternal`/`GenerateTimelineInternal` are only invoked from the full `GenerateAll()` path. The story's own DON'T list flagged this as a risk without mandating a fix. **Owner decision (2026-07-13): defer** — accept documented staleness until the next full generate; revisit alongside other watch-mode work. [src/SpecScribe/SiteGenerator.cs — `GenerateOne` ~L337-351]
- [x] [Review][Patch] Independent `DateTime.Now`/"today" reads across `BuildArtifactsByDay`/`GenerateDatePagesInternal`/`GenerateTimelineInternal`, plus a per-day write failure in `GenerateDatePagesInternal` not being excluded from `GenerateTimelineInternal`'s day set, could each in theory produce a timeline row linking to a `commits/{date}.html` page that was never written. **Applied:** `GenerateTimelineInternal` now sources its day set from `_commitDays` (the pages `GenerateDatePagesInternal` actually wrote) instead of independently recomputing the union, closing both the race and the write-failure gap. 1078 tests green. [src/SpecScribe/SiteGenerator.cs — `GenerateTimelineInternal`]
- [x] [Review][Patch] No `StylesheetTests` companion assertions for the new `.timeline-list`/`.artifacts-updated`/`.artifact-update-list` CSS classes, despite the spec's explicit File Structure Requirements note. **Applied:** added `Stylesheet_HasTimelineAndArtifactsUpdatedStyles`. [tests/SpecScribe.Tests/StylesheetTests.cs]
- [x] [Review][Patch] Inconsistent pluralization phrasing between the visible pill ("N artifact updated"/"N artifacts updated", via `Charts.Plural`) and the `<meta name="description">` text ("N artifact update"/"N artifact updates", hand-rolled) for the identical count on the identical page. **Applied:** `BuildMetaDescription`'s artifact branches now reuse `Charts.Plural` to match the pill exactly (commit-only wording left untouched — locked for byte-parity). [src/SpecScribe/CommitDayTemplater.cs — `BuildMetaDescription`]
- [x] [Review][Defer] `ArtifactLabel` silently swallows read failures (permission/I-O errors) into a bare filename stem with no `GenerationEvent`/diagnostic, unlike most other per-item failure paths in the same file — deferred, pre-existing degrade-silently pattern extended here, low impact. [src/SpecScribe/SiteGenerator.cs:853-864]
- [x] [Review][Defer] `ActivityModel.GroupArtifactsByDay` sorts/dedups by `(Label, Href)` text only; two distinct artifacts sharing a generic title (e.g. two docs both titled "Overview") render as adjacent, visually indistinguishable list entries with no path/type disambiguation — deferred, cosmetic edge case, low likelihood. [src/SpecScribe/ActivityModel.cs:9-36]
- [x] [Review][Defer] The unbounded `GitPulse.CommitsByDay` used for date pages/timeline (unlike the 300-commit-capped `_commitPages`/deep-git correlation from Story 7.5) means date pages beyond that window can never resolve their commit hashes to a detail page — safely degrades to plain `<code>`, but is a completeness gap inherited from 7.5's cap and extended (not introduced) by this story. [src/SpecScribe/SiteGenerator.cs — `GenerateDatePagesInternal`, `CommitHref` usage]
...
| 2026-07-13 | Code review (parallel adversarial + edge-case + acceptance-audit): 3 owner decisions resolved (decoupled the artifact-mtime signal from `epics.md`; accepted watch-mode staleness as deferred; left the heatmap-vs-list day-set discrepancy as-is by design) + 4 patches applied (`BuildArtifactsByDay` now unions `_referenceMap`/`_docs`/`_adrs`; `GenerateTimelineInternal` sources its day set from `_commitDays` instead of an independently recomputed union, closing a midnight-rollover race and a write-failure dead-link gap; `StylesheetTests` companion coverage added; meta-description artifact pluralization now matches the visible pill via `Charts.Plural`). 3 findings deferred, 8 dismissed. 1078 tests green. Status → done. |
