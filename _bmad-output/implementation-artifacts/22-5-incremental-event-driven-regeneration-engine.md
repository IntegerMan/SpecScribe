...
Code review 2026-07-29 (`/bmad-code-review 22.5`). Three layers ran: Blind Hunter
(`bmad-review-adversarial-general`), Edge Case Hunter (`bmad-review-edge-case-hunter`), Acceptance Auditor.
**Scoped by hunk, not commit range**, per CLAUDE.md § Scoping a code review: the story landed in `9421a8c`
(which bundles 8+ sibling stories). **Excluded**: `630ae25`'s hunks in `SiteGenerator.cs` (+144) and
`FileWatcherService.cs` (+20), which are **Story 22.6**'s delta sidecar (`EmitDeltaSidecar`,
`_previousManifestJson`, `EmitDelta`); `630ae25`'s `FileWatcherServiceTests.cs` (+63) hunk, which is 22.6's
`ConcurrentDebouncedPasses_LeaveTheDeltaSidecarCoherent`; `630ae25`'s `SiteGeneratorAdapterTests.cs` (+70)
hunk, which is **Story 20.10**'s fingerprint-regen provenance; and the concurrent session's working-tree
changes under `web/**` + `assets/specscribe.css`. All severities below were re-derived by reading the current
source, not taken from the review layers.

- [x] [Review][Decision] **`RefreshSourceInventory()` records DISK state, so an unrelated save silently cancels a pending escalation** — the parameterless overload the three narrow routes call walks `EnumerateSourceFiles()`, i.e. live disk, so after any narrow route the inventory *equals* the disk and `existsNow == wasRendered` degenerates to disk-vs-disk. This defeats the classifier's stated mechanism, which its own doc comment spells out: *"it always describes what the OUTPUT reflects rather than what the disk currently holds. That distinction is the whole mechanism."* Sequence (one debounce window; `git pull`, an agent writing two files, or a save ≥400 ms before a create all produce it): `architecture.md` edited **and** `new-doc.md` created → `architecture.md`'s pass wins `_gate`, `GenerateOne` → `RefreshSourceInventory()` walks the disk, sees `new-doc.md`, records it as *rendered* → `new-doc.md`'s pass classifies `existsNow=true, wasRendered=true` → **Narrow** → page written against cached `_nav`, appearing in no page's nav, with no whole-tree surface refreshed and no orphan cleanup. Found independently by two layers. `IncrementalOracleParityTests.RunClass` cannot see it — every case applies exactly one change class and dispatches sequentially. **Options: (a) narrow routes mutate the inventory for the path they handled only, never re-walking; (b) capture the disk snapshot at classify time and compare atomically; (c) make classify+dispatch atomic under one `_gate` hold. (a) is recommended — it restores the documented contract and is the cheapest.** **→ RESOLVED (owner, 2026-07-29): (a) per-path mutation. Now a Patch.** [`src/SpecScribe/SiteGenerator.cs` `RefreshSourceInventory` / `ClassifyRebuildScope`]
- [x] [Review][Decision] **Every file-level topology event logs `<directory change>`, a label whose own doc comment exists to mean the opposite** — `FileWatcherService.TopologyEventLabel`'s summary reads *"so the watch log names a directory change as such instead of attributing it to some arbitrary contained file."* `RunDebouncedPass` computes `relative` for the fired path then discards it on the escalation branch, and `RegenerateTopology`'s summary still describes itself as *"a **DIRECTORY-level** topology change"*. The story disclosed this honesty cost for the `epics.md` case only (Trap 4); the general mislabelling of every single-file add/rename/delete is undisclosed, and Note 7b's own live log shows `<directory change> full rebuild` for "added a doc". **Options: (a) accept it as the cost of one coherent event; (b) add a distinct file-topology label carrying the triggering path; (c) reuse the path as the label and keep the one-event shape. Trades directly against AC #7.** **→ RESOLVED (owner, 2026-07-29): (b) add a distinct file-topology label naming the triggering path, keeping `<directory change>` for genuine directory events, and correct `RegenerateTopology`'s stale "DIRECTORY-level" summary. One event per pass is preserved. Now a Patch.** [`src/SpecScribe/FileWatcherService.cs` `TopologyEventLabel` / `RunDebouncedPass`]
- [x] [Review][Decision] **`ClearEpicsFamilyOutputs` and `RemoveFor`'s `Removed` branch are now unreachable from the production watch dispatch** — Note 4's defence, *"still reachable through the public `RegenerateEpics` API (which is how they drive it)"*, conflates test reachability with production reachability. `RemoveFor` has exactly one production caller (`FileWatcherService`), reached only when `Narrow && !File.Exists`, which by the classifier's own rule implies `wasRendered == false`, so `_docs` cannot contain the path and only the `Skipped` branch can run. Same for the `ingest.SourceFullPath is null` → `ClearEpicsFamilyOutputs` path: an `epics.md` deletion escalates before reaching it. The 8 `SiteGeneratorEpicsRemovalTests` pass unmodified *because* they bypass the dispatch, and `FileWatcherServiceTests.DeletingEpicsFile_…` would now pass with `ClearEpicsFamilyOutputs` deleted outright (the absences it waits for come from `GenerateAll`'s output-root wipe) — so nothing in the suite asserts the dispatch ever reaches the teardown. This story also *added* work to `RemoveFor` (`_artifactHrefByRepoRel = null`, `RefreshCodeSurfaces`, `RefreshSourceInventory`) on a path its own classifier made unreachable. **Options: (a) accept the dead production path and say so in the ADR; (b) remove the now-unreachable teardown and its tests; (c) narrow the escalation so the teardown is reached again, accepting the 16-stale/3-missing oracle gap it was measured to leave.** **→ RESOLVED (owner, 2026-07-29): (a) accept the dead dispatch path — escalation is measured strictly more correct — and record in ADR 0027 that the watch dispatch no longer selects the teardown, so the 8 green `SiteGeneratorEpicsRemovalTests` are not misread as proof the dispatch covers it. Now a Patch (documentation).** [`src/SpecScribe/SiteGenerator.cs` `ClearEpicsFamilyOutputs` / `RemoveFor`]
- [x] [Review][Patch] AC #2 clause 3 is UNMET and Task 2's final subtask is falsely checked — no per-epic work-graph node/edge assertion exists anywhere; the whole-tree byte-diff was silently substituted, and the substitute is provably blind to the defect's own surface (the harness fixture renders `Work graph for` only in `work-graph.html`, never in `epics/epic-N.html`, so the 16/20-vs-13/12 class cannot be diffed there); the file also has no vacuity guard, unlike Story 22.4's equivalent [`tests/SpecScribe.Tests/IncrementalOracleParityTests.cs`]
- [x] [Review][Patch] The inventory is committed BEFORE the wipe and render it describes, so a failed `GenerateAll` leaves a session that can never escalate again — `RefreshSourceInventory(files)` sits above `Directory.Delete(OutputRoot, recursive: true)`, and `WatchCommand` deliberately does not fail-fast on an initial-build error; refresh on success at the end of the pass, as the narrow routes already do [`src/SpecScribe/SiteGenerator.cs` `GenerateAll`]
- [x] [Review][Patch] `WriteCodeMap` is NOT never-throw, contradicting `RefreshCodeSurfaces`' own doc claim, and it is sequenced ahead of `WriteIndex`/`RefreshSourceInventory`/`EmitSpaSite` — its `try` covers only `CodeMap.BuildVariants`; `CodeMapTemplater.RenderPage`, `ApplyReferenceLinks`, `EnsureHierarchyEngine` and `WriteOutput` (a bare `File.WriteAllText`, no retry) sit outside it, so one `IOException` on the 1.5 MB `code-map.html` turns an ordinary doc save into a pass with a stale index, an unadvanced inventory and an un-re-emitted IR. Pre-22.5 a save never touched that page [`src/SpecScribe/SiteGenerator.cs` `RefreshCodeSurfaces` / `WriteCodeMap`]
- [x] [Review][Patch] A transient `git-insights.html` write failure permanently disables the surface for the session, silently — `RefreshCodeSurfaces` passes `new List<GenerationEvent>()` and `reporter: null`, so the `Error` event is discarded, while the catch sets `_progress.DeepGit.Insights = null` and the method's first line then early-returns forever. On the narrow path the previous pass's page is still on disk and valid, so clearing the gate strips a nav entry for a page that exists — the inverse of the full path the catch was written for [`src/SpecScribe/SiteGenerator.cs` `RefreshCodeSurfaces` → `GenerateGitInsightsInternal`]
- [x] [Review][Patch] A nested ADR escalates to a full rebuild on every save, non-convergently — the ADR file watcher sets `IncludeSubdirectories = true`, but `EnumerateAdrFiles` is `TopDirectoryOnly` plus exactly one level, so `docs/adrs/decisions/2024/0007-x.md` is watched, can never enter `_sourceInventory`, and exists on disk → `existsNow != wasRendered` forever. Latent today (no such file), but the two-level scheme is the one `EnumerateAdrFiles`' own doc comment invites [`src/SpecScribe/SiteGenerator.cs` `ClassifyRebuildScope` / `EnumerateAdrFiles`]
- [x] [Review][Patch] Two classes Note 6's table marks "Narrow / ✅ byte-identical" have no test and no matrix row — the harness mutators are `ContentEditGenericDoc`, `ContentEditStory`, `AddGenericDoc`, `RenameGenericDoc`, `DeleteStory`, `DeleteAdr`, `DeleteEpics`; there is no content-edit-ADR and no content-edit-`epics.md` case, so AC #4's "each such class is **proven** … before it is allowed to stay narrow" is unmet for both and Note 6's "No class ended up unproven" is false [`tests/SpecScribe.Tests/IncrementalOracleParityTests.cs`]
- [x] [Review][Patch] The widened `BurstOfSaves_CoalescesAndLeavesCoherentOutput` settle condition can sample the count mid-burst — the added wait returns on the *first* matching event and `Assert.Equal(1, …)` runs immediately after, so the per-notification regression it guards (which produces a burst) can be sampled between the 1st and 2nd events and pass. Note 8b's "No assertion weakened" does not hold for this one; wait on a terminal marker or on count stability across two polls [`tests/SpecScribe.Tests/FileWatcherServiceTests.cs`]
- [x] [Review][Patch] AC #8's cross-reference obligation is 1-of-3 discharged — only `docs/adrs/README.md` references ADR 0027. Neither `docs/adrs/0008-json-ir-canonical-and-incremental-generation.md` §Decision 3 nor `ARCHITECTURE-SPINE.md` § AD-5 mentions it, and `ARCHITECTURE-SPINE.md` is absent from the File List. Note 7 substitutes the converse relation (the ADR cites them), which is not what the AC asked for [`docs/adrs/0008-…md`, `_bmad-output/specs/spec-specscribe/ARCHITECTURE-SPINE.md`]
- [x] [Review][Patch] ADR 0027 does not record the proven-narrow class list AC #8 names — it records the classifier, the escalation rule and the proof requirement, but its only per-class table is the pre-fix *divergence* table in §Context. The class→route→scope table lives only in this story file's Completion Note 6, which is exactly the placement CLAUDE.md § Decision records exists to prevent [`docs/adrs/0027-watch-rebuild-scope-is-one-classifier-and-topology-escalates.md`]
- [x] [Review][Patch] The permanent AC #5 gate runs with `emitSpa: false` and `deepGitAnalytics: false` — `Options()` takes `ForgeOptions.Resolve`'s defaults, so the fixture emits no `spa/`, no `git-insights.html`, no `deep-analytics.html`, no `impact-map.html`, no `commit/*.html` and no `code/**.html`. The `git-insights.html` refresh this story *added* therefore ships with no regression guard, resting only on the scratchpad matrix the record says was never committed — the same "the omission was unobservable" condition ADR 0027 §Context exists to end. Trap 3's *"fixing the recompute is fixing the IR"* is likewise unguarded [`tests/SpecScribe.Tests/IncrementalOracleParityTests.cs` `Options`]
- [x] [Review][Patch] `RefreshCodeSurfaces`' stated invariant is false and it mutates the shared `_codeFiles` cache from the hot path — the comment claims *"a change in the FILE SET is a topology change and escalated before reaching any narrow route, so by the time this runs only line counts can have moved."* `_codeFiles` is the git-tracked **code** set; no watcher observes `.cs`/`.ts`/`.css` and `ClassifyRebuildScope` returns `Narrow` for every non-`.md` path by construction, so that set changes freely with no escalation (branch switch, build, new source file). Pre-22.5 `_codeFiles` was computed once per `GenerateAll` [`src/SpecScribe/SiteGenerator.cs` `RefreshCodeSurfaces` / `EnumerateCodeFiles`]
- [x] [Review][Patch] Test-hardening bundle for the new gate — (a) `RunClass` never asserts the dispatched routes returned no `Error` event, so an errored watch route can pass the parity gate (and the two throwing paths above are exactly what that would catch); (b) `ClassifyRebuildScope_SeparatesContentChangesFromTopologyChanges` asserts `Narrow` for a `notes.txt` and a `.hidden.md` that are never created, so `existsNow == wasRendered == false` and both assertions pass even with the `.md` and `IsIgnored` guards deleted; (c) the diff enumerates files only, so an orphaned empty directory reports clean; (d) `NoChange()` is dead code; (e) rename is tested in one event order only [`tests/SpecScribe.Tests/IncrementalOracleParityTests.cs`]
- [x] [Review][Patch] Inventory robustness bundle — (a) `RefreshSourceInventory`'s catch filter is `IOException or UnauthorizedAccessException` while `Path.GetFullPath` can also throw `ArgumentException`/`PathTooLongException`/`NotSupportedException`, which `ClassifyRebuildScope` *does* catch: an escape aborts the route and leaves the inventory stale; (b) an `IOException` mid-walk silently installs a partial/empty inventory, which escalates every save for the rest of the session with nothing logged; (c) `ClassifyRebuildScope(null)` throws and a relative path resolves against the process CWD, on a `public` method; (d) `_sourceInventory` uses `OrdinalIgnoreCase` unconditionally, so a case-only sibling add on a case-sensitive filesystem (ADR 0006 ships npx) classifies `Narrow` [`src/SpecScribe/SiteGenerator.cs` `RefreshSourceInventory` / `ClassifyRebuildScope`]
- [x] [Review][Patch] `ResetDerivedStateForFullRebuild` misses `_commitPages`, `_commitDetails` and `_commitDays` — populated only inside `GenerateAll`'s deep-git gate and never cleared, so a second pass whose deep-git yield is empty resolves `CommitHref` against the first pass's map, pointing at `commit/*.html` the output-root wipe removed. Latent only because the same method's `_progress = null` gates the consumers off too; three lines of cheap insurance, and a realistic trigger exists in the documented 3,000 ms deep-git budget vs ~6,500 ms cold [`src/SpecScribe/SiteGenerator.cs` `ResetDerivedStateForFullRebuild`]
- [x] [Review][Patch] Two CLAUDE.md § Scoping record-keeping items in this story's primary files — (a) `GoldenContentFingerprint` **did** move inside this diff range (`ee00f947…` → `a649bd17…`, and it reads `501ee958…` at review time), self-attributed in its own added comment to the Epic 18 documentation pass and the Story 23.2 re-review; Note 8's "unchanged at `ee00f947…`" is true *of this story* but reads as though the constant in the file is still that value, and the handoff is unrecorded; (b) a sibling hunk `IsGeneratedLockFile`, self-attributed "[Story 23.2 re-review 2026-07-28]", was inserted **between** `EnumerateCodeFiles`' `</summary>` and the method itself, so `EnumerateCodeFiles` is now undocumented and `IsGeneratedLockFile` carries two `<summary>` elements — and it changes the output of the very walk `RefreshCodeSurfaces` re-runs [`tests/SpecScribe.Tests/SiteGeneratorAdapterTests.cs`, `src/SpecScribe/SiteGenerator.cs`]
- [x] [Review][Defer] Git insights and deep analytics are gated on epics presence, so an `epics.md`-less repo never renders them — cold or warm [`src/SpecScribe/SiteGenerator.cs` `GenerateAll`] — deferred, pre-existing. Raised by the Blind Hunter as a defect in `ResetDerivedStateForFullRebuild`'s `_progress = null`; **that reading is a false positive** and is recorded here so it is not re-litigated: `progress` is produced only by the epics-ingest callback, so a *cold* generate of an epics-less tree also leaves `_progress` null and renders neither page — the reset is oracle-faithful, and `GenerateAll`'s output-root wipe means no nav entry can dangle (which is why Story 5.3's split-not-null in `ClearEpicsFamilyOutputs` does not transfer to the escalation path). The residual, genuinely pre-existing issue is the coupling itself: `hasGitInsights = progress?.DeepGit?.Insights is not null` makes two git-derived surfaces depend on a planning artifact.
- [x] [Review][Defer] No parity case drives two routes concurrently, though `FileWatcherService` arms one debounce `Timer` per distinct path and the design's own doc comments rely on `_gate` serialization [`tests/SpecScribe.Tests/IncrementalOracleParityTests.cs`] — deferred, pre-existing gap in harness shape; the suite already carries a rotating file-write-contention flake (Story 23.2), so adding a concurrency case needs its own flake budget.

#### Review remediation — all 18 patches applied 2026-07-29

**What changed, in one line each:**

- **The classifier's ground truth is now what a pass RENDERED, not what the disk holds.** `RefreshSourceInventory`'s disk-walking overload is gone; `GenerateAll` installs the inventory from its own rendered `files` (via `TryBuildSourceInventory`) and the narrow routes mutate exactly the path they handled (`RecordSourceRendered` / `RecordSourceRemoved`). The family routes update nothing at all, with the reason stated: a narrow route only ever runs for a path whose existence already agrees with the inventory, so a walk there could only ever *add* a sibling's pending topology change — which is the whole defect.
- **The inventory installs AFTER the render, not before the output-root wipe.** A failed `GenerateAll` now keeps the previous known-good inventory, and reports an `Error` event if the inventory itself could not be rebuilt.
- **`RefreshCodeSurfaces` is contained and reports.** Its body is wrapped, its two false doc claims (`WriteCodeMap` "is never-throw"; the file-set "cannot flip") are corrected rather than restated, and the caller's real event list is threaded in where one exists.
- **`git-insights.html`'s catch no longer disables the surface for the session** — `Insights` is cleared only when the page is genuinely absent, which is correct on both the wiping full path and the non-wiping narrow one.
- **A nested ADR can no longer escalate forever** (`IsWithinAdrEnumerationDepth`): a path deeper than `EnumerateAdrFiles` reaches classifies `Narrow` instead of producing a non-convergent full-rebuild loop.
- **`ClassifyRebuildScope` guards its public surface** (null/empty → `Narrow`; relative paths resolve against `RepoRoot`, not the process CWD), `_sourceInventory`'s comparer is now platform-aware, and the walk's catch filter matches the classifier's.
- **The reset covers `_commitPages` / `_commitDetails` / `_commitDays`.**
- **A file-level escalation is labelled with the firing path**; `<directory change>` is reserved for genuine directory passes (`RegenerateTopology(string? label)`), and its stale "DIRECTORY-level" doc comment is corrected.
- **Tests:** AC #2's per-epic node/edge assertion now exists (`RegenerateEpics_ProducesTheSamePerEpicWorkGraphCountsAsAFullRebuild`, with its own vacuity guard); the two unproven narrow classes have cases (`ContentEditToAdr`, `ContentEditToEpicsFile`); the harness gained an oracle-file-count floor, a no-`Error`-event assertion, directory comparison, a reversed-order rename, real files behind the previously vacuous guard assertions, and the deep-ADR case; `NoChange()` is deleted; `BurstOfSaves` waits for the event stream to go quiet (`WaitForQuiet`) before counting.
- **Docs:** ADR 0027 gained the proven-narrow class table (Decision 6), the corrected log-honesty and cost consequences, and an explicit record that the epics teardown is unreachable from the production dispatch; ADR 0008 §Decision 3 and `ARCHITECTURE-SPINE.md` § AD-5 now both reference ADR 0027, closing AC #8's 1-of-3 cross-reference gap.

**⚠ The new `--spa` parity rows found a real thing on their first run, and it was the INSTRUMENT, not the code.** With the IR emitted, `spa/manifest.json` diverged on one page's `contentHash`/`bytes` (by exactly 5 bytes — the character-count difference between the two output directory names) while every HTML page compared equal. That is Trap 5 one level down: the manifest hashes each page's **raw, unfolded** content, so `diagnostics.html`'s output-root echo cannot be folded out of it the way it is folded out of page text. Handled by folding those two fields for that one page, **locally in the harness rather than in the shared `GoldenNormalization`** — the fingerprint gate compares one tree against a constant, never two trees in two directories, so it has no such artifact and widening the shared fold would have blunted it for nothing. With that handled, the narrow content classes are byte-identical **in the IR as well as the static pages**, which makes Trap 3's "the IR inherits every recompute defect verbatim" a measured statement for the first time.

**Verification.** Full suite on the shared tree: **2,817 passed / 4 failed / 3 skipped**. `GoldenContentFingerprint` **unmoved** (re-run in isolation, green). The 4 failures are `HierarchyRolloutTests` ×3 + `SiteGeneratorSpaTests.HierarchyExplorerIsland_SurvivesSpaContentRegionCapture`, and they are **not this review's** — proven, not assumed, per CLAUDE.md § Concurrent work: `git archive HEAD` into the scratchpad overlaid with **only** this review's four changed code files runs those same tests **19/19 green**. They are caused by a concurrent session's uncommitted edits to `src/SpecScribe/assets/specscribe.js`, `Charts.cs`, `CodeMapTemplater.cs` and `HierarchyExplorer*.cs` (Story 20.10 / 24.6 work in flight). The 47 tests in the directly affected classes — `IncrementalOracleParityTests` (now 22 cases), `FileWatcherServiceTests`, `FileWatcherServiceCrashGuardTests`, `SiteGeneratorEpicsRemovalTests` — are all green. `web/` gates were not re-run because nothing under `web/` was touched, and the same concurrent session still has `web/**` edits in flight.

**Dismissed as noise (3):** the ADR's "one additional source walk" understating hot-path cost — the numbers *are* recorded, in Note 0b (`content-doc` 3,189 ms post-fix vs 1,303 ms pre-fix, still 10.7× better than a full rebuild) and Note 6; folding all three roots to one `<root>` placeholder as a masking channel — accounted for per AC #5 and materially better than excluding `diagnostics.html`, with the three strings non-overlapping siblings; and the `_progress = null` reading recharacterized in the first Defer item above.

**Verified-as-claimed, recorded so they are not re-litigated:** the extracted `GoldenNormalization` fold set is identical to the original **token for token and in the same order** (CRLF→LF, `FoldToday`, root, `FooterClock`, `AssetCacheBust`, `SubtitleVersion`, `VersionRow`, `BuildRow` — including `BuildRow`'s Story 5.2 `.*?` form), so the golden gate was not widened; `ClassifyRebuildScope` takes `_gate`, calls no route, and cannot deadlock, and no new helper wraps `GenerateAll` in `_gate`; every `_sourceInventory` write happens under a caller's `_gate` and the field is replaced wholesale, so there is no memory race — the defects above are semantic; `IsDataSource` retains its precedence and the classifier structurally cannot claim a non-`.md` path; AC #5's "drive the shipped route" is genuinely met (`RunClass` calls `watcher.RunDebouncedPass`, no replicated predicate ternary); all six named change classes plus both no-op controls are present; every field the reset assigns matches its declared initializer, so a cold generate cannot move; `SchemaVersion` is 2, not bumped here, and Note 8's staleness reasoning about Task 7 holds; Tasks 9, 10 and 11 are genuinely done, with `epics.md` and `sprint-status.yaml` moved in the same change; and no scope guard was crossed.
...
- `tests/SpecScribe.Tests/SiteGeneratorAdapterTests.cs` — `NormalizeVolatile`/`FoldToday` now delegate to `GoldenNormalization` (fold set byte-for-byte unchanged; the golden constant did not move **for this story's changes**).
  ⚠ **Sibling-hunk handoff, recorded per CLAUDE.md § Scoping a code review (added by the 2026-07-29 code review).** `GoldenContentFingerprint` *did* move inside this story's commit range, in this file, and not because of this story: the added comment block self-attributes the hops to the **Epic 18 documentation pass** (`about-sdd-bmad.html` Modules section) and the **Story 23.2 code re-review** (`design-system.html`). Observed values `ee00f947…` → `a649bd17…` → `501ee958…` (the last read at review time, hours later, from a further concurrent session). Completion Note 8's "unchanged at `ee00f947…`" is true *of this story* but reads as though the constant in the file is still that value — it is not, and a future reader diffing against it will be chasing someone else's change. Those hunks are **excluded from this review** and belong to Epic 18 / Story 23.2.
  ⚠ A second sibling hunk in `src/SpecScribe/SiteGenerator.cs` — `IsGeneratedLockFile`, self-attributed "[Story 23.2 re-review 2026-07-28]" — is likewise **not this story's**, but its insertion point broke a doc comment on a symbol this story depends on: it landed between `EnumerateCodeFiles`' `</summary>` and the method, leaving `EnumerateCodeFiles` undocumented and `IsGeneratedLockFile` carrying two `<summary>` elements. Repaired here because `RefreshCodeSurfaces` re-runs that exact walk; the handoff is recorded rather than silently absorbed.
...
- `docs/adrs/0008-json-ir-canonical-and-incremental-generation.md` — §Decision 3 now names ADR 0027 as the operationalization of its "changed scope" half (AC #8 cross-reference; added by the code review).
- `_bmad-output/specs/spec-specscribe/ARCHITECTURE-SPINE.md` — § AD-5 now references ADR 0027 and ADR 0008 §Decision 3 (AC #8 cross-reference; added by the code review — this file was missing from the original File List, which is how the gap went unnoticed).
...
| 2026-07-29 | **Code review (`/bmad-code-review 22.5`), scoped by hunk** — 3 layers, 3 decision-needed (all resolved by owner), 18 patches applied, 2 deferred, 3 dismissed; status `review` → `done`. Headline fix: the narrow routes' `RefreshSourceInventory()` walked the live DISK, collapsing `ClassifyRebuildScope`'s comparison to disk-vs-disk, so a sibling save in the same debounce window absorbed a pending topology change and the added page landed with no nav entry — the exact class the classifier exists to escalate. Inventory now records what a pass RENDERED (per-path in the narrow routes, from the rendered set in `GenerateAll`, installed AFTER the render rather than before the output-root wipe). Also: `RefreshCodeSurfaces` contained + its two false doc claims corrected; `git-insights` no longer self-disables for a session on one transient write failure; nested ADRs no longer escalate non-convergently; file-level escalations labelled with the firing path instead of `<directory change>`; the reset covers the deep-git commit maps. AC #2's per-epic node/edge assertion — checked off but absent — now exists, with a vacuity guard; the two narrow classes claimed "proven" with no case (`content-adr`, `content-epics`) have cases; the gate now also runs with `--spa`, which measured Trap 3's IR-inheritance claim for the first time (and surfaced Trap 5 inside `spa/manifest.json`'s unfoldable per-page hash, handled locally). AC #8's cross-references closed in ADR 0008 §Decision 3 and `ARCHITECTURE-SPINE.md` § AD-5; ADR 0027 gained the proven-narrow class table and an explicit record that the epics teardown is unreachable from the production dispatch. Suite 2,817 passed / 4 failed / 3 skipped, `GoldenContentFingerprint` unmoved; the 4 failures proven to be a concurrent session's in-flight `specscribe.js`/`Charts.cs`/`HierarchyExplorer*.cs` work by bisecting a `git archive HEAD` tree overlaid with only this review's files (19/19 green there). |
