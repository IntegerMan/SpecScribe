...
- [x] ⊘ **Task 6 — Nuxt `/design-system` route** (AC: #3, #6) — **MOOT: the deliverable was DELETED.** Built as
  specified on 2026-07-25, then retired on 2026-08-07 by this story's own third-pass decision 3: under ADR 0034
  the design-system page a user gets is the C#-composed region rendered by Node through `PortalMetaSurface`,
  so a second Vue showcase route no user could reach was permanent duplication rather than pending
  convergence. AC #6's Vue half is recorded WITHDRAWN. The subtasks below are marked `⊘` rather than `[x]`
  because both name a file that no longer exists — following the `⊘ MOOT` convention this story already used
  for two patches on 2026-07-29. [ticked-but-moot corrected by the fourth review pass, 2026-08-07]
  - ⊘ `web/pages/design-system.vue` renders every primitive in every relevant status/motion state, using `PageShell.vue`. This is the worked example for the conventions doc and the future portal design-system surface. — **file deleted.** AC #5's `:deep()` worked example, which this page carried, was moved into `CONVENTIONS.md` §3 in the same change, so the demonstration AC #5 requires still exists.
  - ⊘ Verify live in a browser (`npm run dev` and/or `nuxt generate` + serve): tokens resolve (`var(--status-done)` → the moss value), reduced-motion honored, status readable by name with JS off. — **not re-verifiable: the route is gone.** ⚠️ This does NOT discharge the live-browser obligation, which is outstanding for the C# `design-system.html` and for the badge-border / skip-link changes; see the fourth-pass findings section.
...
- ~~`web/pages/index.vue`~~ — **no longer exists**: renamed to `web/pages/component-library.vue` by sibling
  Story 23.3, which moved the site root onto the IR manifest's entry page. That file still carries a 23.2 hunk
  (the `/design-system` retirement note and the re-pointed link), so it is 23.2's to review even though it is
  23.3's to own.
- ~~`web/pages/design-system.vue`~~ — **DELETED** 2026-08-07 by this story's own third-pass decision 3. Its
  AC #5 `:deep()` worked example moved to `web/CONVENTIONS.md` §3.
...
#### Backfilled: rounds that changed files and never got a File List entry

_Added by the fourth review pass, 2026-08-07. There were addenda for the 2026-07-29 follow-up and the
2026-08-07 dev-story round, but **none** for the 2026-07-28 re-review or the 2026-08-07 review follow-up
(4 decisions + 28 patches) — which is where most of the work below landed. CLAUDE.md makes the File List the
scoping instrument for the next reviewer, so the gap is what makes hunk attribution unreliable._

- `web/test/tokens-lib.test.mjs` — **never listed at all.** Carries the `findRootBlocks` grouped-prelude
  (`:root, :host`) and `@layer`/`@supports` cases proven red on 2026-08-07.
- `web/measurements/payload.json`, `web/measurements/payload.txt` — created by the 2026-07-28 round so AC #4's
  table is checkable rather than quoted; the record cites them, the File List never added them.
- `web/pages/measure/{async,island,static}.vue` — the `window`-prop hunks (2026-08-07), removing the prose the
  C# twin's re-review had already removed.
- `web/pages/component-library.vue` — a 23.2 hunk in a 23.3-owned file: the `/design-system` retirement note
  and the re-pointed link.
- `web/components/surfaces/IrSurface.vue` — likewise 23.3-owned, carrying 23.2's `provides-landmark`
  declaration from the fourth pass.
- `web/assets/shared-primitives.css`, `web/nuxt.config.ts`, `src/SpecScribe/assets/specscribe.css` — the
  `.skip-link` promotion and its `position: fixed` correction (2026-08-07 decision 2).

#### Added by the 2026-08-07 fourth review pass

**Modified — web:** `components/PageShell.vue` (the `providesLandmark` precondition replacing the inert
`onMounted` guard; `v-if` on title/brand; widened `chrome` warning) · `components/StatusBadge.vue` (`hasLabel`
render precondition) · `components/ChartPanel.vue` (`v-if` on the `h3`; `$slots.legend?.()?.length`; narrowed
anatomy claim; symbol-not-line citation) · `components/ListRow.vue` (`summaryText` guard; chip coercion;
`$slots.badge?.()?.length`) · `components/MeasureRows.server.vue` (`rowCount` clamp; list omitted when empty) ·
`components/surfaces/IrSurface.vue` (`provides-landmark`) · `pages/component-library.vue`, `package.json`,
`nuxt.config.ts`, `README.md`, `CONVENTIONS.md` (the four false "not wired into `specscribe generate`" copies;
allowlist count; ADR 0034 link).

**Modified — C#:** `DesignSystemTemplater.cs` (`retired` swatch class; derived motion count; `CanonicalRank`
ordering; withdrawn-plan doc) · `SiteNav.cs` (`DesignSystemLabel` const) · `Icons.cs`,
`HtmlRenderAdapter.Dashboard.cs` (consume the const) · `StatusStyles.cs` (`LegendWord` doc).

**Modified — tests:** `SiteGeneratorDesignSystemTests.cs` (pinned `ExpectedWords`/`ExpectedMeanings` +
completeness guard; whole-token `--status-*` match; derived motion coverage; join-shape "shared with";
two reverse loops guarded; new `DesignSystem_EveryBadgeStageClassResolvesToARuleInTheStylesheet`) ·
`StylesheetTests.cs` (stage-fill guard widened to every colour property and notation, plus a non-vacuity
bound — **both halves proven red**).

**Modified — docs:** `docs/adrs/0029-unscoped-shared-primitive-layer.md` (**Proposed → Accepted**),
`docs/adrs/README.md` (status + allowlist count).

## Change Log

| Date | Change |
| --- | --- |
| 2026-08-07 | **Fourth code-review pass (Group A) — 3 decisions answered, 21 of 23 patches applied, status `review` → `in-progress`.** Three-layer adversarial pass over ~2,050 of the story's 6,277 in-scope lines; **Groups B, C and D are explicitly NOT reviewed** and are named in the findings section. Headline, raised independently by two layers: **`PageShell`'s missing-landmark guard could never fire** — registered in `onMounted` only when `chrome !== 'full'`, while every `nav-only` route sets `noScripts: true` and so never hydrates, making a fix recorded as applied on 2026-08-07 inert. ⏸ **Its replacement is written, verified, and HELD**: `check:parity` is green on the clean tree and red on all 24 routes with it, and a **comment-only** edit to `PageShell.vue` reproduces that exactly — the content-derived scoped-style hash means ANY edit to a chrome SFC moves `pageSha`. Re-pinning is the designed answer (ADR 0033), but `pin:parity` rebuilds the corpus from a live `--deep-git` IR and would absorb sibling drift as well, so it was left for the owner rather than done quietly; the work is preserved as `decision-1-skip-link.patch`. Everything else was re-verified parity-clean with that one item reverted. Also applied: `v-if` guards so three primitives stop shipping the empty output their own headers forbid (`StatusBadge`'s colour-only badge, `ChartPanel`'s empty `<h3>`, `ListRow`'s empty summary) plus the `$slots`-truthiness wrapper bug; the `retired` swatch class no production caller emits; a derived motion count; `CanonicalRank` ordering; a `DesignSystemLabel` const replacing five hand-typed copies; four false "not wired into `specscribe generate`" claims including **one rendered on the running page**; the allowlist's "one entry" drift in both places a reader counts it; and **ADR 0029 ratified Proposed → Accepted**. Test quality was the largest cluster: the stage-vocabulary assertions were satisfied by construction (now pinned against independent `ExpectedWords`/`ExpectedMeanings` tables with a completeness guard), `--status-{stage}` was satisfied by its own `-bg` fill token, and `Stylesheet_StageFillsOnBadgeFamilies` had three holes — it passed when it inspected nothing, saw only `background`, and saw only `#` literals; **both halves proven red** (a literal `color:` is now caught; renaming the selectors now fails instead of silently retiring the guard). Suites: C# **2980/0/3**, web vitest **182 + 1 skipped**, `check:tokens` green, `check:parity` **green (24 routes / 14 families)**. ⚠️ The live-browser pass is outstanding for a **fifth** consecutive session. |
