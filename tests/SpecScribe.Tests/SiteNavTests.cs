...
public class SiteNavTests
{
    ...
    public void Build_OmitsMissingArtifactClasses()
    {
        ...
        Assert.Equal(new[] { "Home", "Epics", "Requirements", "Traceability", "Cadence", "How to use SpecScribe", "Design System", "About Spec-Driven Development", "About", "Logs" }, nav.Items.Select(i => i.Label).ToArray());
    }
    ...
    public void Build_IncludesSourceDerivedAndAdrLinksWhenAvailable()
    {
        ...
        Assert.Equal(
            new[] { "Home", "Epics", "Requirements", "Traceability", "Cadence", "GDD", "Narrative", "Game Architecture", "ADRs", "How to use SpecScribe", "Design System", "About Spec-Driven Development", "About", "Logs" },
    ...
    public void Build_IncludesReadmeInProjectGroupWhenAvailable()
    {
        ...
        Assert.Equal(new[] { "Home", "Epics", "Requirements", "Traceability", "Cadence", "Readme", "How to use SpecScribe", "Design System", "About Spec-Driven Development", "About", "Logs" }, nav.Items.Select(i => i.Label).ToArray());
    ...
    public void Build_AddsSprintItemAndQuickLinkNextToDeliveryViewsWhenAvailable()
    {
        ...
        Assert.Equal(new[] { "Home", "Epics", "Requirements", "Traceability", "Cadence", "Sprint", "How to use SpecScribe", "Design System", "About Spec-Driven Development", "About", "Logs" }, nav.Items.Select(i => i.Label).ToArray());
    ...
    public void Build_AddsCodeMapAndRiskQuadrantUnderInsights()
    {
        ...
        Assert.Equal(
            new[] { "Home", "Epics", "Requirements", "Traceability", "Cadence", "Code Map", "Risk Quadrant", "How to use SpecScribe", "Design System", "About Spec-Driven Development", "About", "Logs" },
            nav.Items.Select(i => i.Label).ToArray());
    ...
    public void Build_PutsBmadMethodDocsInNavAndQuickLinks()
    {
        ...
        Assert.Equal(
            new[] { "Home", "Epics", "Requirements", "Traceability", "Cadence", "PRD", "Architecture", "How to use SpecScribe", "Design System", "About Spec-Driven Development", "About", "Logs" },
            nav.Items.Select(i => i.Label).ToArray());
    ...
    public void Build_AddsSpecKernelToProjectGroupAndQuickLinks()
    {
        ...
        Assert.Equal(
            new[] { "Home", "Epics", "Requirements", "Traceability", "Cadence", "PRD", "Architecture", "Spec", "How to use SpecScribe", "Design System", "About Spec-Driven Development", "About", "Logs" },
            nav.Items.Select(i => i.Label).ToArray());
    }
    ...
    public void ToNavigationView_ProjectsGroupsAndFlattenedItems()
    {
        ...
        Assert.Contains(view.Groups, g => g.Label == "Help" && g.Children.Count == 5);
    }
    ...
    public void Build_HelpGroup_AlwaysContainsSddAboutAndLogs()
    {
        ...
        Assert.Equal(
            new[] {
                ...
                ("Design System", SiteNav.DesignSystemOutputPath),
    ...
    [Fact]
    public void RenderNavMarkup_BrandIsTheProjectNameAlone_WithNoToolMarkBesideIt()
    {
        // Owner feedback, 2026-08-01 (field use of the VS Code extension): the upper-left of the identity bar is
        // where the PROJECT name goes, so SpecScribe's own mark does not belong beside it — rendered together,
        // "<nib> TakeTheSky" invites the nib to be read as the project's logo. The mark previously sat inside this
        // span; this test is the inverse of the one that pinned it there.
        var nav = SiteNav.Build(new[] { "planning-artifacts/epics.md" }, "SpecScribe", hasAdrs: false);
        var markup = HtmlRenderAdapter.Shared.RenderNavMarkup(nav.ToNavigationView("index.html"));

        Assert.Contains("<span class=\"site-nav-brand\">SpecScribe</span>", markup);
        Assert.DoesNotContain("site-nav-mark", markup);
    }

    [Fact]
    public void RenderNavMarkup_AttributionBadgeStillCarriesTheNib_TokenColoredAndDecorative()
    {
        // The other half of the same decision: removing the mark from the brand must NOT remove the tool's identity
        // from the page. It keeps its place upper-RIGHT, where "Generated by SpecScribe" is a claim about who
        // produced the page rather than about whose page it is — and it keeps the shared NibPathData geometry, the
        // token-color rule (no hex in markup) and the decorative aria treatment the brand mark used to be pinned for.
        var nav = SiteNav.Build(new[] { "planning-artifacts/epics.md" }, "SpecScribe", hasAdrs: false);
        var markup = HtmlRenderAdapter.Shared.RenderNavMarkup(nav.ToNavigationView("index.html"));

        var svgStart = markup.IndexOf("<svg class=\"specscribe-badge-mark\"", StringComparison.Ordinal);
        Assert.True(svgStart >= 0, "the attribution badge carries the specscribe-badge-mark SVG");
        ...
        Assert.Contains("<span class=\"specscribe-badge-text\">SpecScribe</span>", markup);
    }
