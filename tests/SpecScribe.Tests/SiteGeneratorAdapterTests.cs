...
public class SiteGeneratorAdapterTests : IDisposable
{
    ...
    public void GenerateAll_GoldenContentFingerprint_IsStableAfterNormalizingVolatileTokens()
    {
        ...
        // Regenerated by the Story 20.2 CODE REVIEW patch pass. Markup deltas in this fixture: (a) the epics-index
        // sunburst no longer carries `data-node-id` at all — the hooks became opt-in (`nodeIds:`) so only the
        // dashboard, which actually mounts the explorer island, emits them; (b) each island node gained a `ring`
        // field (the ring a wedge sits on is now stated, not inferred from `kind` — an epic's aggregates sit on the
        // aggregate band but the orphan/unplanned ones sit on the story band); (c) the drillable wedges' aria-label
        // suffix now names the arrow-key affordance. Plus specscribe.js/.css and specscribe-spa.js asset bytes.
        // PROVENANCE (shared main): regenerated on a tree that also carried another session's in-flight Story 5.2
