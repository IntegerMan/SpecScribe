...
public class SiteGeneratorAdapterTests : IDisposable
{
    ...
    public void GenerateAll_GoldenContentFingerprint_IsStableAfterNormalizingVolatileTokens()
    {
        ...
        // Regenerated for journey-grouped global nav + uniform band cards: the single <nav class="site-nav"> is
        // now two tiers — a dark identity bar (project name + a "Generated by SpecScribe" attribution badge) above
        // a white menu band carrying the journey-grouped hierarchical menu (top-nav items merged with the former
        // dashboard "Explore Key Views" superset). The dashboard body dropped its own identity header + key-views
        // strip (now a visually-hidden H1); the summary band is a uniform 4-up grid with clickthrough "View →"
        // header links on the Epic Status / Overall Progress cards; and stat tiles link to their detail pages.
        // Every page's chrome changed, so the whole-site fingerprint shifts. [journey-nav-and-uniform-band-cards]
        const string expected = "26ff5db91cd4983beba054018a4d78bec82b689ffd12fa1b96b9c374afdfc1cd";
        Assert.True(
